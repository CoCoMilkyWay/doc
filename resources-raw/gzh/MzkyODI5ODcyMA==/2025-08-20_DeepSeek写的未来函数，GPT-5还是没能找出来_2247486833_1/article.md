...
量化君 量化君也 2025-08-20 07:07 广东

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=MzkyODI5ODcyMA==&mid=2247486833&idx=1&sn=3fbf3d081de1130b7b910810d5d0df0d&chksm=c33aa437ac128c1dfa7201394f5887c7b68de95c8bd41c748a87abf605f1e2b12f63270b434a#rd](https://mp.weixin.qq.com/s?__biz=MzkyODI5ODcyMA==&mid=2247486833&idx=1&sn=3fbf3d081de1130b7b910810d5d0df0d&chksm=c33aa437ac128c1dfa7201394f5887c7b68de95c8bd41c748a87abf605f1e2b12f63270b434a#rd)

![](assets/001.png)

  

DeepSeek-R1模型年初发布的时候，大伙儿玩得不亦乐乎，许多博主和个人宽客都让DeepSeek为自己构思和编写量化策略，并且把源码都po到公开平台与大伙儿分享，其中真的不乏有种有趣有料的“三有”策略，里面就有一个非常有种的策略。

  

当时在聚宽JoinQuant量化平台上，有用户上传了一个DeepSeek开发的收益非常炸裂的量化策略，2024年初至发布时点期间策略总收益6119.82%，年化收益4971.22%，但也在贴子里坦言“好像有未来函数”和“AI还得再加油”。
...
这个策略是DeepSeek写的，当时用DeepSeek进行拆解，也认为当中没有未来函数，死活找不到Bug所在，这也怪不得人家，毕竟对于人类程序员来说，自己写的Bug，自己也是很难找出来。

  

刚好这个月OpenAI不是发布了GPT-5模型嘛，AI又经过了大半年的发展进步，应该能力有所提升了吧，正好来试一下外来和尚这经念的好不好，按照当时的拆解流程再走一遍。

  

首先要让GPT-5先把策略读一遍，先确认一下有没有读懂，输入以下提示词。
...
-   宽基指数：纳指ETF、创业板ETF、沪深300ETF、上证50、国证2000
    
-   行业/主题：生物医药、新能源、消费、科技
    
-   海外市场：恒生科技、中概互联、德国DAX30、日经ETF、印度LOF、华宝油气
    
-   防御性资产：黄金ETF、可转债、国债ETF
    

2.调仓频率：

每个交易日早上 9:30检查，默认 每日调仓一次。
...
-   如果所有资产都下跌 → 买入 国债ETF（防御仓位）。
    
-   如果有上涨 → 选择 涨幅最大的ETF，并全仓买入。
...
这是一个 极简的“强者恒强”动量轮动策略：

-   用非常短的2日涨幅来判断市场最强资产。
    
-   逻辑简单粗暴：全仓买入短期最强ETF，否则躲到国债里。
    
-   核心思想是追逐短期强势资产，弱市则防御。
    
-   可以理解为 超短周期的ETF动量切换 + 国债兜底。
    

  

要不要我帮你画一个 流程图，直观展示从“计算收益率 → 选择目标 → 调仓”的完整逻辑？

\======回复结束线======  

  

简单来说，这就是一个侯选池固定的ETF轮动策略，利用2日的动量选择标的，每日早盘调仓，顺便接着让它画出流程图，这样整个流程更清晰。
...
到目前，已经确认它已经完全读懂了这个策略，接下来我就要向GPT-5问出最关键的那个问题，输入以下提示词。

  

_**Prompt：这个策略当中存在未来函数吗？会不会在当前时间取到了未来的数据？**_
...
GPT-5经过一通分析后最终的结论是：该策略没有未来函数，数据都是历史已知的_**。**_

  

好嘛，怎么会没有未来函数呢？只要在策略源码当中初始化的部分，加上这行代码set\_option('avoid\_future\_data', True)，再重跑一下就会立马报错，回测失败，报错信息：jqdata. exceptions. FutureDataError: avoid\_future\_data=True，get\_price取天行情时，盘中不能取当日的{'close'}字段数据。
...
刚才加的那条语句，就是用来打开平台的防未来函数功能，有未来函数的代码就会报错。**报错的原因就是在get\_price函数当中end\_date = context.current\_dt，也就是取数的结束日期设置为今天，相当于早盘就取到了当日收盘的数据，妥妥的未来函数。**

  

还是有一丢丢失望，这个资深宽客可以一眼认出的未来函数，GPT-5还是没认出来。对了，等等，还没确认用的模型是不是GPT-5呢，先别表错情了~
...
一问，你还真是GPT-5本尊，我现在可以继续失望了。

  

再仔细回顾GPT-5的回答，发现它是已经找到关键的地方了，已经很接近真相了。
...
它自己其实已经意识到策略早上9:30运行时，当天的收盘价还未存在，但它自己却说这样取到的是昨天的收盘价，不会拿到未来函数，是上山摘菌子回家吃了，前一秒清醒，后一秒小人就在天上飞了？

  

可惜啊，感觉像是前方明明已经空门，临门一脚却踢歪了。唉~~这也是AI编写量化策略经常会出现的一个通病，没有量化经验的萌新不容易看出来，量化策略有bug看不出来的话，那可能要付出真金白银的代价，AI虽好，切莫贪杯。
