...
量化君 量化君也 2025-07-09 20:00 广东

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=MzkyODI5ODcyMA==&mid=2247486762&idx=1&sn=0ebf5a95536d463ddb4080d9c5207561&chksm=c37ff78c70f3f9d437fcbbab49e1730db5be60102291bf890831b08c4de86394c7a91214587f#rd](https://mp.weixin.qq.com/s?__biz=MzkyODI5ODcyMA==&mid=2247486762&idx=1&sn=0ebf5a95536d463ddb4080d9c5207561&chksm=c37ff78c70f3f9d437fcbbab49e1730db5be60102291bf890831b08c4de86394c7a91214587f#rd)

![](assets/001.png)

  

在之前的文章里面讲到了，选用的量化交易软件QMT满血版，必须同时包含实现策略的两种方式，一种是【内置Python】，另一种是【原生Python】，【内置Python】是利用QMT软件中自带的Python环境，而【原生Python】则是基于miniQMT功能通过xtquant库利用电脑本地Python环境实现策略。

  

QMT软件一般都包含【内置Python】，也就是俗称的“大QMT”，miniQMT(俗称“小QMT”)则有一部分券商没有提供。因此，本文就来唠唠xtquant库如何安装和miniQMT功能如何开启以及验证，看看自己的QMT是不是满血版。

  

一、安装xtquant库的3种方式

xtquant库是迅投公司开发出来的一套完善的Python策略运行框架，对外以Python库的形式提供策略交易所需要的行情和交易相关的API接口。因此，要在自己电脑上用miniQMT功能，一定要安装xtquant库，它的安装一般有3种方式。

  

（1）方式1：通过pip安装

这种方式最为简单，就跟平常安装Python第三方库一样。

  

通过“Win + R”快捷键打开Windows系统的运行框，在当中输入“cmd”后按确认键，就可以打开命令行模式，在命令行当中输入“pip install xtquant”后敲回车键(Enter)即可。
...
如果你使用上述命令安装xtquant的速度太慢，可以在安装命令中添加清华镜像源，完整命令如下：

  

```
pip install -i https://pypi.tuna.tsinghua.edu.cn/simple xtquant 
```
...
方式1其实通过在线联网的方式把xtquant库下载到你本地电脑当中，存放在Python安装路径下的Lib\\site-packages文件夹底下，跟你平时安装第三方库存放的路径是一样的。因此，你只要能把xtquant库放在Lib\\site-packages文件夹底下就行了，接下来再讲能获取xtquant库的另外两种方法。
...
这种方式是通过QMT软件把xtquant先下载下来，再放到对应的路径当中去。

  

首先登录你的QMT软件，在主界面的中间正上方，点击【下载python库】。或者在主界面左上角，点击【设置】选项卡，在弹出的系统设置界面中选择【模型设置】。
...
在弹出的模型设置界面当中，点击【Python库下载】的蓝色按钮，等待下载完毕。切记不要去修改Python库路径，保持默认就可以了。
...
等Python库下载完毕，那xtquant库也会随之下载到本地的电脑上，但它是存放在QMT的内置Python环境当中的，咱要把它拷贝到电脑的原生Python环境当中去。

  

假设你是把QMT软件安装在“D:\\QMT极速策略交易平台”文件夹底下(每个券商的QMT软件名称都不一样)，那么你刚才下载下来的xtquant库就存放在“D:\\QMT极速策略交易平台\\bin.x64\\Lib\\site-packages”路径当中。
...
然后你把xtquant库这整个文件夹拷贝到你的Python安装路径下的Lib\\site-packages文件夹底下就可以了。

  

方式1是通过pip自动下载的，方式2是通过QMT软件下载再手动拷贝的，那么还有一个地方可以下载到xtquant这个库。

  

（3）方式3：通过迅投官网获取

xtquant库是迅投公司开发的，自然在其官网上也可以下载到，使用下方链接可以直达。

  

https://dict.thinktrader.net/nativeApi/download\_xtquant.html
...
点击【点击下载】按钮，便可以将xtquant的库以压缩包的形式下载下来，然后把解压出来的文件夹，放到你的Python安装路径下的Lib\\site-packages文件夹底下就可以了。

  

这里要注意两点，第一点是解压出来的文件夹名称是带日期戳的，你需要把日期戳去掉，例如把文件夹名称从“xtquant\_250516”重命名为“xtquant”。第二点是xtquant文件夹不能再套有文件夹(例如xtquant\\xtquant)，底下必须直接是源代码文件，如下所示。
...
通过以上任意一种方式将xtquant库安装好之后，便可以开启miniQMT功能了，miniQMT的开启有2种方式。
...
在运行QMT软件进入登录主界面时，勾选【独立交易】，不同券商的QMT软件在此处的勾选框名称不一样，有的可能叫“独立模式”或“极简模式”，大伙儿意会即可，勾选后点击登录，那就算开启miniQMT功能了。如果不勾选此处，开启的就是“大QMT”模式，进入的是【内置Python】。
...
假设你是把QMT软件安装在“D:\\QMT极速策略交易平台”文件夹底下(每个券商的QMT软件名称都不一样)，那么在“D:\\QMT极速策略交易平台\\bin.x64”文件里面就会有一个【XtMiniQmt.exe】可执行文件，双击运行就会出现miniQMT的专属登录界面，输入账号密码后即可开启miniQMT功能。
...
无论使用以上哪种方式，当出现以下类似界面时，便说明miniQMT功能已经正常开启了。
...
安装好xtquant库和开启miniQMT功能后，说明你现在就可以使用当中的行情和交易相关的API接口了。

  

现在打开自己电脑上的任何Python相关的IDE(如VSCode / PyCharm / Spyder / Jupyter Notebook等)，输入以下代码，并运行。
...
如果有类似于以下格式的数据结果打印出来，说明你的QMT软件是满血版的，带有的miniQMT功能正常使用，将来你就可以在自己电脑上的任何Python IDE中实时获取行情和下单交易。

  

```
{'430017.BJ': {'timetag': '20250709 15:00:04', 'lastPrice': 23.21, 'open': 23.6, 'high': 23.88, 'low': 23.01, 'lastClose': 23.76, 'amount': 131043500, 'volume': 55976, 'pvolume': 55976, 'stockStatus': 5, 'openInt': 15, 'settlementPrice': 0, 'lastSettlementPrice': 23.76, 
```

  

上面那几行代码的功能是获取指定的沪深京交易所3支股票当前的最新Tick数据，由于这是在2025年7月9日盘后运行的代码，所以返回的是3支个股在收盘时最后的一条数据。如果在盘中运行该代码，每次运行时，都是获得最新的Tick数据。

  

如果你的QMT软件不是满血版，没有miniQMT功能，则会在运行该代码时，得到类似于以下的信息，“not authenticated”就是没通过身份验证的意思，也就是说券商并没有授权miniQMT的功能给你。

  

```
RuntimeError: func:getFullTick, 
```

  

你的程序运行返回的结果是什么呢？希望都有timetag，没有error！
