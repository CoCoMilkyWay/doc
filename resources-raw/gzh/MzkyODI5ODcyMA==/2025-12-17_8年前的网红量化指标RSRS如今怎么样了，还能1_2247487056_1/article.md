...
量化君 量化君也 2025-12-17 17:37 广东

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=MzkyODI5ODcyMA==&mid=2247487056&idx=1&sn=7b53d77de887caef23c44af442597ac4&chksm=c35e7d5d675b6a1eaa19366978e45b29ac14109205b5c99ac8c0d7086159a7e4276d9200f88f#rd](https://mp.weixin.qq.com/s?__biz=MzkyODI5ODcyMA==&mid=2247487056&idx=1&sn=7b53d77de887caef23c44af442597ac4&chksm=c35e7d5d675b6a1eaa19366978e45b29ac14109205b5c99ac8c0d7086159a7e4276d9200f88f#rd)

![](assets/001.png)

  

2017年，有个量化指标在研报当中横空出世，它就是RSRS，随后风靡了各个投资领域，在股票、期货和大饼圈中有非常多的使用者，如今8年过去了，当初的那个指标变得怎么样了呢？

  

今儿个，咱一起来复现看一下，从数据获取、计算细节一直聊到策略构建、绩效回顾，掰开了，揉碎了~

  

1.RSRS的来源和思想

RSRS指标的全称是“阻力支撑相对强度（Resistance Support Relative Strength）”，它诞生于光大证券在2017年劳动节发布的金工研报《基于阻力支撑相对强度的市场择时》，这个系列的研报有好几篇，咱这次是复现最初的那一篇。
...
最初的想法和原理可以参照原版研报，如果只想听个大概的话，这里先插几句闲白。
...
那问题来了，怎么找到阻力位和支撑位呢？听网上那些“专家”的预测吗？当然不是~

  

其实我们每天看K线图，“公认”的阻力和支撑就蕴含在里面，那就是K线的最高价和最低价，简单来说，这两个价格是经过万千交易者充分交易后的博弈结果，所有的成交价格都包含在了最高价和最低价形成的空间里，在最高价这条阻力线之下，在最低价这条支撑线之上。当然了，光用1天的最高价和最低价当然不行，要用序列值。
...
可以换一个思路，这就是当时RSRS的创新点所在，不直接使用阻力位和支撑位这种绝对阈值方式，改为使用相对强度的方式。就好比是，绝对阈值方式就是预测清华北大的学生能否将来年入百万千万，相对强度方式则是预测清华北大的学生收入是否比普通人的收入均值要高，这两者都不是绝对事件，但两者的预测难易程度一目了然。
...
RSRS的计算步骤和流程说完了，撸起袖子开干吧，从数据获取、指标计算和策略构建全部用代码复现出来。

  

第一步，对照原版研报，获取沪深300指数从2005年至今的开高低收行情数据，这里使用的是常用量化开源库之一的akshare，“pip install akshare”安装后，直接就可以丝滑用起来了。
...
第二步，这里的关键是计算每一日的斜率值beta，这里先给萌新说一个简单具体的例子，懂最小二乘法OLS的小伙伴可跳过。
...
虽然有噪声的干扰，咱都知道它们的底层关系就是一条直线y=beta\*x+alpha，其中beta=2是斜率，alpha=1是截距，最小二乘法OLS的作用就是根据已知的坐标数值，计算出斜率和截距。
...
解单个序列的斜率值咱搞定了，在沪深300指数的行情数据上，咱只需要每个交易日滑动(rolling)计算18个交易日最高价vs最低价的斜率就可以了，为什么N=18呢，因为这是原版研报中在2017年定的最优参数，复现就以原版为准。
...
从最终收益看起来，RSRS标准分策略要比RSRS斜率策略要好，咱把它俩和基准画在一张图上进行对比，优化作用就明显了。
...
咱把研报中的RSRS策略对比图也找出来看看，研报中的数据是截止到2017年4月，当时RSRS斜率策略的累计净值是在11.57，RSRS标准分策略的累计净值是在14.37，无论是走势还是数值，总体上还是比较接近的，算是复现出个大概了。
...
看完整体的回测结果，大伙儿感觉RSRS这个量化指标怎么样呢？

  

2017年发布以来，距今已经8年了，相当于是有了8年的样本外数据，从绝对收益层面来看，发布后4年策略表现还是不错的，净值持续走高，但2021后，仿佛进入了一个下跌通道当中，近两年虽有所反弹，但仍未走出新高，难现当初12年13倍的辉煌；从相对收益层面来说，RSRS策略的净值走势还是好于基准的沪深300指数。

  

总体来说光大证券这篇研报还是瑕不掩瑜，RSRS指标在当时来说带有一定的创新性，不少小伙伴看了都觉得有启发，这也是它有众多拥趸的原因。

  

我当时也是在看完RSRS系列研报后，受此启发，开发出了『ETF斜率动量轮动策略』，这就是另外一个故事了，感兴趣的可以去看[《](https://mp.weixin.qq.com/s?__biz=MzkyODI5ODcyMA==&mid=2247486898&idx=1&sn=262dc8c02491d988ab4f258071ecc797&scene=21#wechat_redirect)[ETF轮动策略迁移到实盘后，幸好守住了十年10倍+](https://mp.weixin.qq.com/s?__biz=MzkyODI5ODcyMA==&mid=2247486898&idx=1&sn=262dc8c02491d988ab4f258071ecc797&scene=21#wechat_redirect)[》](https://mp.weixin.qq.com/s?__biz=MzkyODI5ODcyMA==&mid=2247486898&idx=1&sn=262dc8c02491d988ab4f258071ecc797&scene=21#wechat_redirect)。
...
一个量化指标，或是一个量化策略，都终将难免走向失效，但其中的创新和精华会时常给咱们以启迪，变为前进的助力。
