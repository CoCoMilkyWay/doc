# 曾经10年140倍，核心就7个字的策略，为啥能风靡量化圈3年？

量化君 量化君也 2026-01-14 17:07 广东

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=MzkyODI5ODcyMA==&mid=2247487135&idx=1&sn=f6eb41acdbaf01fc9df8252fce99e598&chksm=c3667fd8fb9f4f872c75972196a2dcbb1f012d9addb83ae14cf55e4b5449a6e695406140c180#rd](https://mp.weixin.qq.com/s?__biz=MzkyODI5ODcyMA==&mid=2247487135&idx=1&sn=f6eb41acdbaf01fc9df8252fce99e598&chksm=c3667fd8fb9f4f872c75972196a2dcbb1f012d9addb83ae14cf55e4b5449a6e695406140c180#rd)

![](assets/001.png)

  

大概是在3年多之前吧，在许多量化社区里面，发现有一类策略在个人宽客那里玩得飞起，那就是“大妈类”策略，有菜场大妈，还有皇城大妈、正黄旗大妈、通天纹大妈等，其中以菜场大妈的变种和衍生最多。

  

虽然这类名字听起来非常“民科”，一股野路子的味道扑面而来，但看过核心的策略思路后，还是存在一定的底层逻辑，对于量化萌新来说是一种性价比非常高的入门策略，逻辑简单代码少，思路清晰收益高。

  

目前从公开的资料看，菜场大妈的核心策略思想是聚宽社区用户ID为『开心果』的大神最先提出，虽然那篇帖子已经被作者删除了，但之前保存下源码的宽客延续了这个策略的生命，出现了各种“魔改”，将策略发扬光大，咱先来看一下当初的策略表现。
...
10年收益将近140倍，年化收益62.34%，夏普率高达2.25，最大回撤为30.88%，收益回撤比也超过了2.0，表现这么亮眼的策略，怪不得大伙儿都趋之若鹜，迅速在圈子里面风靡起来了。

  

为啥这个策略能有这么高的收益呢？在回答这个问题之前，先来解释一下这些策略名字里面为啥都含有“大妈”这个字眼，我说一个梗大伙儿就明白了。
...
价低，就是筛选质好股票里面价格低的股票，最初的版本中是股价不能超过9元，“9元”这个设置除了测试效果好之外，还有一个用意就是，方便小资金的个人宽客使用该策略，买入1手股票加上交易费用和滑点，最多才900块出头，选股10支，还不到1w，真的是非常滴亲民接地气。后来改进的版本当中就出现了动态的股价阈值，也就是不使用固定的股价数额了，而使用当时的分位价格，例如股价要比全市场80%的股票都要低。
...
你瞧，这个策略的正式学名其实应该是叫“绩优低价小市值选股策略”，但由于“菜场大妈”实在是太传神了，因此脍炙人口沿用至今，虽然这个名字看起来很土气，但却利用了两个被金融专家孜孜不倦研究的金融异象——小市值和低价股。

  

小市值就不用我过多介绍了，那是A股过去十几年来总体收益贡献最大的一类因子，早在Fama-French三因子定价模型中就已经提及，虽然不是一直都有效，但不可忽视它的整体收益贡献，小市值因子的超额收益来源主要有3方面。
...
低价股的“溢价”来源就没有小市值的那么明确了，可能的来源也是有3个，分别是低估值、低质量和个人投资者的价格幻觉，进一步详细的说明大伙儿可以看招商证券金工团队的研究《A股低价股溢价效应及其成因解析》，对了，小市值的可以看东方证券金工团队的《A股小市值溢价的来源》。

  

那现在关键的问题来了，这个策略目前还有效吗？里面有没有未来函数？有没有利用特定平台的Bug？策略逻辑是普适的吗？聚宽是不能直接实盘交易的，迁移到其他量化平台后，还能有这个收益吗？

  

刚好，这些问题可以一次性解决，验证有无未来函数，最靠谱的方法就是实盘跟踪，但这要等很长一段时间，最快的方法就是，将这个策略逻辑更换数据源重新实现，并在其他的量化平台上进行回测，并且这个量化平台是可以直接实盘的。

  

于是，为了验证逻辑和后续魔改，我使用QMT结合Tushare复现了这个菜场大妈选股策略，为了尽力避免未来函数的可能，在回测设定中，今日选股，市值、股息率等因子数据只用昨日的，并且在历史时点剔除涨跌停股票和ST股票等奇奇怪怪的股票，比方说根据当时股票的名称判断是否是ST股，而不是根据现在的股票名称，这样就能尽力避免幸存者偏差，复现策略的回测绩效如下。
...
复现策略的年化收益为47.44%，夏普率为1.54，最大回撤为42.55%，跟之前的绩效比要差了一截，年化收益下降了15%，最大回撤增加了12%。

  

需要补充说明的是，在质好方面，同时使用了股息率和PEG因子；在价低方面，除了最高限价9元之外，由于A股的“面值1元退市”规则，加入了最低限价2元，也就是股价要在2~9元之间；每周第一个交易日调仓，选择符合质好价低条件里面市值最小的10支股票，等权分配资金，正常费率含最低5元。

  

因为数据完整性的原因，回测是从2019年1月开始的，可以看到2024年之前的曲线走势基本是跟原版吻合的，但在2024年上半年遭受了两次重创，导致了收益下降和最大回撤提高，但在九二四行情之后，策略目前依然处于上升通道当中，从目前情况来看，菜场大妈选股策略总体上是有效的，只是收益比当初降低了，没有了当初10年100倍+的辉煌。

  

在复现策略当中，根据群友的日常需求，还加了一个月份过滤的功能，如果开启一四月过滤的话，回测绩效如下所示，年化收益为58.62%，夏普率为2.31，最大回撤为23.60%，收益回撤比又拉回了2.0以上。
...
根据两个平台的双重验证，说明这个菜场大妈选股策略表现还是不错的，是一个非常顶的入门级策略，而且非常适合小资金，1w就能入场，实在不行，5k也可以干。年化收益算是可以的，主要是现在的最大回撤还是不小，遇到极端行情，持有体验可能不够好，待后期加入更多细节控制，以期能降低回撤或者提高收益风险比。
...
本次菜场大妈选股策略的回测/实盘源代码，已经分享在『量化达摩院』社群当中，群友请原路径自取，还不会使用QMT进行策略回测和实盘的小伙伴，请参照社群知识库第二章的第3和第4部分进行操作。
