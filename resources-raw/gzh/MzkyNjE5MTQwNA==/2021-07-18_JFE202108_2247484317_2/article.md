# JFE202108

量化前沿速递 2021-07-18 16:29

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=MzkyNjE5MTQwNA==&mid=2247484317&idx=2&sn=9d5b24da7999b6763bd52da2baede115&chksm=c35784e70fa5bccd6d86e51eded3948e04087d730ab128848caed32a43ae464544277f645732#rd](https://mp.weixin.qq.com/s?__biz=MzkyNjE5MTQwNA==&mid=2247484317&idx=2&sn=9d5b24da7999b6763bd52da2baede115&chksm=c35784e70fa5bccd6d86e51eded3948e04087d730ab128848caed32a43ae464544277f645732#rd)

机器翻译，仅供参考！更多文献获取请关注公众号：量化前沿速递

获取原文请加入知识星球“量化前沿速递”

## 

文献汇总

\[1\] The local innovation spillovers of listed firms

上市公司的地方创新溢出

\[2\] Lucky factors

幸运因素

\[3\] Uncertainty, access to debt, and firm precautionary behavior

不确定性、获得债务和坚定的预防行为

\[4\] Angels and venture capitalists: Substitutes or complements?

天使和风险投资家：替代品还是补充品？

\[5\] Macro risks and the term structure of interest rates

宏观风险与利率期限结构

\[6\] Heterogeneous intermediary asset pricing

异质中介资产定价

\[7\] The rate of communication

通信速率

\[8\] Capital supply and corporate bond issuances: Evidence from mutual fund flows

资本供给与公司债券发行：来自共同基金流动的证据

\[9\] Pervasive underreaction: Evidence from high-frequency data

普遍反应不足：来自高频数据的证据

\[10\] Reciprocal lending relationships in shadow banking

影子银行中的互惠贷款关系

\[11\] Compensation disclosures and strategic commitment: Evidence from revenue-based pay

薪酬披露与战略承诺：基于收入的薪酬证据

\[12\] Out of sight no more? The effect of fee disclosures on 401(k) investment allocations

再也看不见了？费用披露对401（k）投资分配的影响

\[13\] Factors and risk premia in individual international stock returns

国际个股收益的影响因素与风险溢价

\[14\] Bank capital, government bond holdings, and sovereign debt capacity

银行资本、政府债券持有量和主权债务能力

\[15\] Network structure and pricing in the FX market

外汇市场的网络结构与定价

\[16\] The telegraph and modern banking development, 1881–1936

电报与现代银行业发展，1881-1936

\[17\] Regulatory effects on short-term interest rates

对短期利率的监管效应

\[18\] Ransomware activity and blockchain congestion

勒索软件活动与区块链拥塞

\[19\] Life after LIBOR

LIBOR后的生活

\[20\] Corporate immunity to the COVID-19 pandemic

对COVID-19大流行的公司免疫

## 

\[1\] The local innovation spillovers of listed firms

标题:上市公司的地方创新溢出

作者:Adrien Matray

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001367

DOI:https://doi.org/10.1016/j.jfineco.2021.04.009

Abstract : This paper provides evidence of local innovation spillovers (i.e., innovation by one firm fostering innovation by neighboring firms). First, I document that exogenous shocks to innovation by listed firms affect innovation by private firms in the same geographical area and that such local innovation spillovers decline rapidly with distance. Second, these local innovation spillovers stem from knowledge diffusing locally through two channels: learning across local firms and inventors moving from their employer to both existing firms and newly started spin-outs. Finally, I study the two-way relations between innovation spillovers and the availability of capital. I find that local innovation spillovers cause venture capital funds from outside the area to invest more in the local area, and that capital availability amplifies local innovation spillovers.

Keywords : Innovation spillovers;Knowledge diffusion;Patents;Financing of innovation

Abstract :本文提供了地方创新溢出的证据（即一个企业的创新促进了邻近企业的创新）。首先，我发现上市公司创新的外生冲击会影响同一地理区域内民营企业的创新，而且这种局部创新溢出效应会随着距离的增加而迅速下降。第二，这些本地创新溢出源于知识通过两个渠道在本地扩散：在本地公司和发明家之间的学习，从他们的雇主转移到现有的公司和新开始的螺旋输出（spin out）。最后，研究了创新溢出与资本可用性之间的双向关系。我发现，地方创新溢出会导致来自外部的风险投资基金更多地投资于地方，而资本的可获得性会放大地方创新溢出。

Keywords :创新溢出；知识扩散；专利；创新融资

## 

\[2\] Lucky factors

标题:幸运因素

作者:Campbell R. Harvey,Yan Liu

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001410

DOI:https://doi.org/10.1016/j.jfineco.2021.04.014

Abstract : Identifying the factors that drive the cross-section of expected returns is challenging for at least three reasons. First, the choice of testing approach (time series versus cross-sectional) will deliver different sets of factors. Second, varying test portfolio sorts changes the importance of candidate factors. Finally, given the hundreds of factors that have been proposed, test multiplicity must be dealt with. We propose a new method that makes measured progress in addressing these key challenges. We apply our method in a panel regression setting and shed some light on the puzzling empirical result that the market factor drives the bulk of the variance of stock returns, but is often knocked out in cross-sectional tests. In our setup, the market factor is not eliminated. Further, we bypass arbitrary portfolio sorts and instead execute our tests on individual stocks with no loss in power. Finally, our bootstrap implementation, which allows us to impose the null hypothesis of no cross-sectional explanatory power, naturally controls for the multiple testing problem.

Keywords : Factors;Factor selection;Variable selection;Bootstrap;Data mining;Orthogonalization;Multiple testing;Predictive regressions;Fama–MacBeth;GRS

Abstract :确定驱动预期收益横截面的因素具有挑战性，至少有三个原因。首先，测试方法的选择（时间序列与横截面）将提供不同的因素集。第二，不同的测试组合类别改变了候选因素的重要性。最后，考虑到已经提出的数百个因素，必须处理测试多重性。我们提出了一种新方法，在应对这些关键挑战方面取得了可衡量的进展。我们将我们的方法应用于面板回归设置中，并揭示了令人费解的实证结果，即市场因素驱动了股票收益率的大部分方差，但在横截面测试中经常被剔除。在我们的设置中，市场因素并没有消除。此外，我们绕过了任意的投资组合分类，而是在没有动力损失的情况下对个股进行测试。最后，我们的bootstrap实现，允许我们强加没有横截面解释力的空假设，自然控制了多重测试问题。

Keywords :因素；因子选择；变量选择；引导；数据挖掘；正交化；多次试验；预测回归；法玛-麦克白；GRS公司

## 

\[3\] Uncertainty, access to debt, and firm precautionary behavior

标题:不确定性、获得债务和坚定的预防行为

作者:Giovanni Favara,Janet Gao,Mariassunta Giannetti

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001379

DOI:https://doi.org/10.1016/j.jfineco.2021.04.010

Abstract : Better access to debt markets mitigates the effects of uncertainty on corporate policies. We establish this result using the staggered introduction of anti-recharacterization laws in US states. These laws enhanced firms’ ability to borrow by strengthening creditors’ rights to repossess collateral pledged in special purpose vehicles. After the passage of the laws, firms that face more uncertainty hoard less cash and increase payouts, leverage, and investment in intangible assets. Our findings suggest that better access to debt markets shields firms from fluctuations in uncertainty and decreases firms’ precautionary behavior, contributing to the deployment of cash and other internal resources to investment in intangible capital.

Keywords : Anti-recharacterization laws;SPVs;Creditor rights;Cash;Intangible assets;Geopolitical risk;Political uncertainty

Abstract :更好地进入债务市场可以减轻不确定性对公司政策的影响。我们通过在美国各州交错引入反再定性法来建立这一结果。这些法律加强了债权人收回以特殊目的工具抵押的抵押品的权利，从而增强了公司的借贷能力。法律通过后，面临更多不确定性的公司囤积的现金减少，支出、杠杆率和无形资产投资增加。我们的研究结果表明，更好地进入债务市场可以保护企业免受不确定性波动的影响，降低企业的防范行为，从而有助于将现金和其他内部资源用于无形资本投资。

Keywords :反再定性法；特殊目的公司；债权；现金；无形资产；地缘政治风险；政治不确定性

## 

\[4\] Angels and venture capitalists: Substitutes or complements?

标题:天使和风险投资家：替代品还是补充品？

作者:Thomas Hellmann,Paul Schure,Dan H. Vo

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001288

DOI:https://doi.org/10.1016/j.jfineco.2021.04.001

Abstract : We analyze the funding of start-up companies across financing rounds, focusing on the dynamic interactions between angel investors and venture capitalists. Using unique data from British Columbia, Canada, we show that angels and venture capitalists are dynamic substitutes. This substitutes pattern applies across the performance range of companies. It is less pronounced for serial angels. An instrumental variable analysis, based on available investor tax credits, suggests that the substitutes pattern is driven by company characteristics. Overall, the evidence points to the existence of parallel streams of angel and venture capital funding, with fewer transitions between streams than is traditionally assumed.

Keywords : Angel investors;Venture capitalists;Entrepreneurial finance;Investor dynamics;Start-up ecosystem

Abstract :我们分析了各轮融资中初创公司的融资情况，重点分析了天使投资者和风险资本家之间的动态互动。利用加拿大不列颠哥伦比亚省的独特数据，我们发现天使和风险资本家是动态的替代品。这种替代模式适用于所有公司的业绩范围。连环天使的发音就不那么明显了。基于现有投资者税收抵免的工具变量分析表明，替代模式是由公司特征驱动的。总体而言，证据表明存在平行的天使和风险资本资金流，资金流之间的转换比传统假设的要少。

Keywords :天使投资人；风险投资家；创业金融；投资者动态；启动生态系统

## 

\[5\] Macro risks and the term structure of interest rates

标题:宏观风险与利率期限结构

作者:Geert Bekaert,Eric Engstrom,Andrey Ermolov

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001252

DOI:https://doi.org/10.1016/j.jfineco.2021.03.011

Abstract : We use non-Gaussian features in U.S. macroeconomic data to identify aggregate supply and demand shocks while imposing minimal economic assumptions. Macro risks represent the variables that govern the time-varying variance, skewness, and higher-order moments of these two shocks, with ”good” (”bad”) variance associated with positive (negative) skewness. We document that macro risks significantly contribute to the variation of yields and risk premiums for nominal bonds. While overall bond risk premiums are countercyclical, an increase in aggregate demand variance significantly lowers risk premiums. Macro risks also significantly predict future realized bond return variances.

Keywords : Bond return predictability;Term premium;Macroeconomic volatility;Business cycles;Macro risk factors

Abstract :我们使用美国宏观经济数据中的非高斯特征来确定总供给和需求冲击，同时施加最小的经济假设。宏观风险代表控制这两次冲击的时变方差、偏度和高阶矩的变量，“好”（“坏”）方差与正（负）偏度相关。我们证明，宏观风险对名义债券收益率和风险溢价的变化有显著影响。虽然整体债券风险溢价是逆周期的，但总需求方差的增加显著降低了风险溢价。宏观风险也显著预测未来实现的债券收益率差异。

Keywords :债券收益的可预测性；定期保险费；宏观经济波动；商业周期；宏观风险因素

## 

\[6\] Heterogeneous intermediary asset pricing

标题:异质中介资产定价

作者:Mahyar Kargar

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001392

DOI:https://doi.org/10.1016/j.jfineco.2021.04.012

Abstract : I show that the composition of the financial sector has important asset pricing implications beyond the health of the aggregate financial sector. To assess the impact of massive balance sheet adjustments within the intermediary sector during the Great Recession and resolve conflicting asset pricing evidence, I propose a dynamic asset pricing model with heterogeneous intermediaries facing financial frictions. Asset flows between intermediaries are quantitatively important for both the level of and variation in the risk premium. An empirical measure of the composition of the intermediary sector negatively forecasts future excess returns and is priced in the cross-section with a positive price of risk.

Keywords : Intermediary asset pricing;Heterogeneous intermediaries;Leverage;Financial frictions

Abstract :我表明，金融部门的构成对资产定价的重要影响超出了总体金融部门的健康状况。为了评估大萧条期间中介行业大规模资产负债表调整的影响并解决资产定价证据之间的冲突，我提出了一个异质中介面临金融摩擦的动态资产定价模型。中介机构之间的资产流动在数量上对风险溢价的水平和变化都很重要。中介部门构成的实证指标对未来超额收益进行了负面预测，并以风险价格为正的横截面进行定价。

Keywords :中介资产定价；异质中介；杠杆作用；金融摩擦

## 

\[7\] The rate of communication

标题:通信速率

作者:Shiyang Huang,Byoung-Hyoun Hwang,Dong Lou

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001276

DOI:https://doi.org/10.1016/j.jfineco.2021.03.013

Abstract : We study the transmission of financial news and opinions through social interactions among retail investors in the United States. We identify a series of plausibly exogenous shocks, which cause “treated investors” to trade abnormally. We then trace the “contagion” of abnormal trading activity from the treated investors to their neighbors and their neighbors’ neighbors. Coupled with methodology drawn from epidemiology, our setting allows us to estimate the rate of communication and how it varies with the characteristics of the underlying investor population.

Keywords : Social interaction;Investor communication;Information diffusion

Abstract :我们研究在美国，通过散户投资者之间的社会互动来传递金融新闻和观点。我们发现了一系列貌似外生的冲击，这些冲击导致“被对待的投资者”交易异常。然后，我们追踪异常交易活动的“传染”，从受治疗的投资者到他们的邻居和他们邻居的邻居。结合流行病学的方法，我们的设置允许我们估计传播率以及它如何随潜在投资者群体的特征而变化。

Keywords :社会交往；投资者沟通；信息扩散

## 

\[8\] Capital supply and corporate bond issuances: Evidence from mutual fund flows

标题:资本供给与公司债券发行：来自共同基金流动的证据

作者:Qifei Zhu

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001264

DOI:https://doi.org/10.1016/j.jfineco.2021.03.012

Abstract : I examine how the investment behavior of bond mutual funds affects corporate financing decisions. Mutual funds that hold a firm’s existing bonds have a high propensity to acquire additional new issuances from the same firm. I utilize this stylized fact to construct a firm-specific bond capital supply measure by aggregating flows from a firm’s existing bondholders. Firms with a higher flow-driven capital supply are more likely to issue bonds, enjoy lower yields, and substitute away from equity financing and bank loans. Information acquisition costs and underwriter relationship likely contribute to the impact of flow-driven capital supply.

Keywords : Bond mutual funds;Corporate bond issuance;Capital structure

Abstract :本文研究了债券型共同基金的投资行为对公司融资决策的影响。持有一家公司现有债券的共同基金很有可能从同一家公司获得额外的新债券。我利用这个程式化的事实来构建一个企业特定的债券资本供应的措施，从一个企业的现有债券持有人的流量汇总。流动性驱动的资本供给较高的公司更有可能发行债券，享受较低的收益率，并替代股权融资和银行贷款。信息获取成本和承销商关系可能对流动驱动的资本供给产生影响。

Keywords :债券共同基金；公司债券发行；资本结构

## 

\[9\] Pervasive underreaction: Evidence from high-frequency data

标题:普遍反应不足：来自高频数据的证据

作者:Hao Jiang,Sophia Zhengzi Li,Hao Wang

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001306

DOI:https://doi.org/10.1016/j.jfineco.2021.04.003

Abstract : We propose a novel high-frequency decomposition of daily stock returns into news- and non-news-driven components, and uncover evidence of pervasive stock market underreaction to firm news. Prices tend to drift in the same direction as the initial market response for several days after the news arrival without reversals. A trading strategy exploiting the return drift generates high abnormal returns and remains profitable after transaction costs. To understand the economic mechanism, we find that the return drift is stronger when investors are distracted. Analysts’ slow adjustments of market expectations following firm news also contribute to the market underreaction.

Keywords : Underreaction;High-frequency;News;Attention;Expectation formation

Abstract :我们提出了一种新的高频分解每日股票收益分为新闻和非新闻驱动的组成部分，并揭示了普遍存在的股票市场反应不足的证据。在消息传来后的几天内，价格往往与最初的市场反应方向相同，不会出现逆转。利用收益漂移的交易策略会产生较高的异常收益，并在扣除交易成本后保持盈利。为了理解这一经济机制，我们发现当投资者分心时，收益漂移更大。分析师在确定消息后对市场预期的缓慢调整也造成了市场反应不足。

Keywords :反应不足；高频；新闻；注意；期望形成

## 

\[10\] Reciprocal lending relationships in shadow banking

标题:影子银行中的互惠贷款关系

作者:Yi Li

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001318

DOI:https://doi.org/10.1016/j.jfineco.2021.04.004

Abstract : Postcrisis regulations apply stricter liquidity rules to both money market funds (MMFs) and banks, requiring MMFs to do more overnight lending and banks to borrow longer-term. MMFs and banks resolve this dilemma by developing a “bundling” strategy across overnight and longer term markets. In particular, MMFs increase longer term funding and charge a lower rate to banks that have recently accommodated MMFs’ overnight depositing needs. Such cross-market reciprocity is stronger between MMFs and foreign banks, which depend on MMFs for dollar funding more than U.S. banks do. MMFs with lower liquidity buffers and higher flow volatility are more likely to engage in bundling.

Keywords : Reciprocal relationship;Money market funds;Liquidity;Regulation;Wholesale funding

Abstract :危机后的监管对货币市场基金（mmf）和银行都适用更严格的流动性规则，要求mmf提供更多的隔夜贷款，而银行则需要提供更长期的贷款。货币市场基金组织和银行通过在隔夜和长期市场制定“捆绑”战略来解决这一困境。特别是，货币市场基金增加了长期融资，并向最近满足货币市场基金隔夜存款需求的银行收取较低的利率。货币市场基金与外国银行之间的这种跨市场互惠性更强，它们比美国银行更依赖货币市场基金提供美元融资。具有较低流动性缓冲和较高流动性波动性的货币市场基金更有可能进行捆绑。

Keywords :互惠关系；货币市场基金；流动性；监管；批发融资

## 

\[11\] Compensation disclosures and strategic commitment: Evidence from revenue-based pay

标题:薪酬披露与战略承诺：基于收入的薪酬证据

作者:Matthew J. Bloomfield

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001331

DOI:https://doi.org/10.1016/j.jfineco.2021.04.006

Abstract : A 2006 rule change in the United States mandated that publicly traded firms provide more detailed disclosures about executives’ compensation plans. In response to the new disclosure requirements, Cournot firms with large market shares add revenue-based pay to their CEOs’ pay packages. This change in pay practices coincides with a shift towards more aggressive product market equilibria, characterized by greater production expenditures and lower margins. Jointly, these patterns are consistent with predictions from the theory of “strategic delegation,” and suggest that the new disclosure requirements enhanced the viability of committing through executive incentives. After adopting the new disclosure requirements, many firms appear to restructure their executives’ pay packages as strategic devices designed to make rivals curtail their competitive actions.

Keywords : Strategic delegation;Oligopoly;Commitment;Compensation;Disclosure

Abstract :美国2006年的一项规则修改规定，上市公司必须对高管薪酬计划进行更详细的披露。为了应对新的信息披露要求，市场份额较大的古诺公司在首席执行官的薪酬中增加了基于收入的薪酬。这种支付方式的变化与向更激进的产品市场均衡的转变相吻合，其特点是生产支出增加，利润率降低。总之，这些模式与“战略授权”理论的预测是一致的，并表明新的披露要求通过高管激励增强了承诺的可行性。在采用新的信息披露要求后，许多公司似乎将高管薪酬重组为战略手段，旨在让竞争对手缩减其竞争行为。

Keywords :战略授权；寡头垄断；承诺；补偿；披露

## 

\[12\] Out of sight no more? The effect of fee disclosures on 401(k) investment allocations

标题:再也看不见了？费用披露对401（k）投资分配的影响

作者:Mathias Kronlund,Veronika K. Pool,Clemens Sialm,Irina Stefanescu

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001355

DOI:https://doi.org/10.1016/j.jfineco.2021.04.008

Abstract : We examine the effects of a 2012 regulatory reform that mandated fee and performance disclosures for the investment options in 401(k) plans. We show that participants became significantly more attentive to expense ratios and short-term performance after the reform. The disclosure effects are stronger among plans with large average contributions per participant and are weaker for plans with many investment options. Additionally, these results are not driven by secular changes in investor behavior or sponsor-initiated changes to the investment menus. Our findings suggest that providing salient fee and performance information can mitigate participants inertia in retirement plans.

Keywords : 401(k) pension plans;Disclosure;Expense ratio;Financial regulation;Mutual funds;Pensions;Performance

Abstract :我们研究了2012年监管改革的效果，该改革要求对401（k）计划中的投资期权进行费用和业绩披露。我们发现，改革后，参与者对费用比率和短期绩效的关注度显著提高。在平均每位参与者贡献较大的计划中，信息披露效应较强，而在投资选择较多的计划中，信息披露效应较弱。此外，这些结果并非由投资者行为的长期变化或保荐人发起的投资菜单变化所驱动。我们的研究结果表明，提供显著的费用和绩效信息可以缓解参与者在退休计划中的惰性。

Keywords :401（k）养老金计划；披露；费用率；金融监管；共同基金；养老金；性能

## 

\[13\] Factors and risk premia in individual international stock returns

标题:国际个股收益的影响因素与风险溢价

作者:Ines Chaieb,Hugues Langlois,Olivier Scaillet

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001343

DOI:https://doi.org/10.1016/j.jfineco.2021.04.007

Abstract : We propose an estimation methodology tailored for large unbalanced panels of individual stock returns to study the factor structure and expected returns in international stock markets. We show that the local market is necessary to capture the factor structure in both developed and emerging markets. Neither the presence of multiple world or regional risk factors, systematic currency risk factors, nor a country-specific currency subsumes the importance of the local market factor. All factors, including the local market, carry significant risk premia across a large proportion of countries. The contribution of pricing errors to total expected returns is large and time-varying.

Keywords : Approximate factor model;Emerging markets;International asset pricing;Large panel;Time-varying risk premium

Abstract :本文提出了一种适合于股票收益大不平衡面板的估计方法，以研究国际股票市场的因子结构和预期收益。我们表明，本地市场是必要的，以捕捉在发达国家和新兴市场的要素结构。无论是多个世界或地区风险因素、系统性货币风险因素的存在，还是特定国家货币的存在，都不包含当地市场因素的重要性。包括本地市场在内的所有因素在很大一部分国家都具有显著的风险溢价。定价误差对总预期收益的贡献是巨大的，而且是时变的。

Keywords :近似因子模型；新兴市场；国际资产定价；大面板；时变风险溢价

## 

\[14\] Bank capital, government bond holdings, and sovereign debt capacity

标题:银行资本、政府债券持有量和主权债务能力

作者:Matteo Crosignani

链接:https://www.sciencedirect.com/science/article/pii/S0304405X2100132X

DOI:https://doi.org/10.1016/j.jfineco.2021.04.005

Abstract : I develop a model where the sovereign debt capacity depends on the capitalization of domestic banks. Low-capital banks optimally tilt their government bond portfolio toward domestic securities, linking their destiny to that of the sovereign. If the sovereign risk is sufficiently high, low-capital banks lend less to the productive sector to further increase their holdings of domestic government bonds, lowering sovereign yields. In this case, a government that regulates bank capital faces a trade-off. On the one hand, high-capital banks lend more to the productive sector. On the other hand, low-capital banks support the home sovereign debt capacity.

Keywords : Bank capital;Sovereign risk;Government bonds;Bank-Sovereign nexus

Abstract :我建立了一个模型，其中主权债务能力取决于国内银行的资本化。低资本银行最理想地将政府债券投资组合向国内证券倾斜，将其命运与主权国家联系起来。如果主权风险足够高，低资本的银行就会减少对生产部门的贷款，以进一步增持国内政府债券，从而降低主权收益率。在这种情况下，监管银行资本的政府面临着一种权衡。一方面，高资本银行向生产部门提供更多贷款。另一方面，低资本银行支持本国主权债务能力。

Keywords :银行资本；主权风险；政府债券；银行主权关系

## 

\[15\] Network structure and pricing in the FX market

标题:外汇市场的网络结构与定价

作者:Joel Hasbrouck,Richard M. Levich

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001409

DOI:https://doi.org/10.1016/j.jfineco.2021.04.013

Abstract : We construct the network, centrality measures, and attributions of trading profits for a sample of CLS Bank settlement data that spans diverse currency pairs, participants, and execution platforms. We define an average centrality differential as the return to the more-central counterparty in the trade. Estimates imply that the more-central counterparty receives a higher return, and that this differential increases as the counterparties’ centralities diverge. These two results are consistent with a pervasive centrality premium. This premium may reflect bargaining power, but we also find evidence that the premium is partially offset by losses that central agents incur in supplying liquidity.

Keywords : Foreign exchange;CLS bank;Dealer networks;Trading costs;Centrality

Abstract :我们以跨不同货币对、参与者和执行平台的CLS银行结算数据为样本，构建了交易利润的网络、中心性度量和属性。我们将平均中心性差异定义为对交易中更为中心的交易对手的回报。估计意味着，越集中的交易对手获得的回报越高，而且这种差异随着交易对手的集中度的不同而增加。这两个结果与普遍的中心性溢价是一致的。这一溢价可能反映了议价能力，但我们也发现有证据表明，溢价部分被中央代理机构在提供流动性时遭受的损失所抵消。

Keywords :外汇；CLS银行；经销商网络；交易成本；中心性

## 

\[16\] The telegraph and modern banking development, 1881–1936

标题:电报与现代银行业发展，1881-1936

作者:Chen Lin,Chicheng Ma,Yuchen Sun,Yuchen Xu

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001380

DOI:https://doi.org/10.1016/j.jfineco.2021.04.011

Abstract : The telegraph was introduced to China in the late 19th century, a time when China also saw the rise of modern banks. Based on this historical context, this paper documents the importance of information technology in banking development. We construct a data set on the distributions of telegraph stations and banks across 287 prefectures between 1881 and 1936. The results show that the telegraph significantly expanded banks’ branch networks in terms of both number and geographic scope. The effect of the telegraph remains robust when we instrument it using proximity to the early military telegraph trunk.

Keywords : Information technology;Telegraph;Bank;Finance;Modern China

Abstract :电报在19世纪末传入中国，当时中国也看到了现代银行的崛起。基于这一历史背景，本文论述了信息技术在银行业发展中的重要性。本文建立了1881年至1936年间287个地州的电报站和电报银行分布数据集，结果表明，电报在数量和地理范围上都显著地扩展了银行网点。当我们使用接近早期军用电报中继线的方法来安装电报时，电报的效果仍然很好。

Keywords :信息技术；电报；银行；财务；近代中国

## 

\[17\] Regulatory effects on short-term interest rates

标题:对短期利率的监管效应

作者:Angelo Ranaldo,Patrick Schaffner,Michalis Vasios

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001434

DOI:https://doi.org/10.1016/j.jfineco.2021.04.016

Abstract : We analyze the effects of prudential regulation on short-term interest rates. The European Market Infrastructure Regulation (EMIR) induces clearing houses (CCPs) to supply large amounts of cash in reverse repurchase agreements (repos). Basel III, in contrast, disincentivizes the borrowing demand by tightening banks’ balance sheet constraints. Using unique regulatory data of CCP investment activity and repo transactions, we find compelling evidence for both the supply and demand channels. The overall effects are decreasing short-term rates and increasing market imbalances in various forms, all of which entail unintended consequences due to the new regulatory framework.

Keywords : Repo;Clearing house;EMIR;Basel III;Leverage ratio

Abstract :我们分析了审慎监管对短期利率的影响。欧洲市场基础设施监管（EMIR）促使清算所（ccp）在逆回购协议（repos）中提供大量现金。相比之下，巴塞尔协议III通过收紧银行的资产负债表约束来抑制借贷需求。利用CCP投资活动和回购交易的独特监管数据，我们发现供求渠道都有令人信服的证据。总体影响是短期利率下降，各种形式的市场失衡加剧，所有这些都会因新的监管框架而产生意想不到的后果。

Keywords :回购；清算所；埃米尔；巴塞尔协议III；杠杆率

## 

\[18\] Ransomware activity and blockchain congestion

标题:勒索软件活动与区块链拥塞

作者:Konstantin Sokolov

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001422

DOI:https://doi.org/10.1016/j.jfineco.2021.04.015

Abstract : I examine blockchain congestion episodes caused by more than 4,400 triggers for ransomware attacks over a two-year period. When demand for settlement exceeds blockchain capacity, blockchain users engage in fee competition to prioritize their transaction settlement. A typical surge in ransomware activity causes transaction fees to increase by 2.1% and up to 28% in extreme cases. Consistent with theory literature, some users forgo blockchain settlement when transaction fees increase. An event study around an extreme spike in ransomware activity supports the findings of the main analysis.

Keywords : Blockchain;Congestion;Ransomware;Transaction fees

Abstract :我研究了两年内4400多个勒索软件攻击触发点引起的区块链拥塞事件。当结算需求超过区块链容量时，区块链用户会进行费用竞争，以确定其交易结算的优先级。勒索软件活动的典型激增会导致交易费用增加2.1%，在极端情况下会高达28%。与理论文献一致，当交易费用增加时，一些用户放弃区块链结算。围绕勒索软件活动的极端峰值的事件研究支持主要分析的结果。

Keywords :块链；拥塞；勒索软件；交易费用

## 

\[19\] Life after LIBOR

标题:LIBOR后的生活

作者:Sven Klingler,Olav Syrstad

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21001446

DOI:https://doi.org/10.1016/j.jfineco.2021.04.017

Abstract : We examine the alternative reference rates that are set to replace the London Interbank Offered Rate (LIBOR) as benchmark rate by the end of 2021. After providing the relevant background, we show that: (i) depending on the marginal lenders, tighter regulatory constraints can either increase or decrease the alternative benchmarks; (ii) increases in the amount of government debt outstanding increase the alternative benchmarks, more so for collateralized rates; and (iii) more central bank reserves lower the alternative benchmarks. In addition, we show that term rates based on the alternative reference rates can be detached from banks’ marginal funding costs.

Keywords : Benchmark rates;Financial regulation;LIBOR;Repo rates;Collateral

Abstract :我们研究了在2021年底取代伦敦银行同业拆借利率（LIBOR）作为基准利率的替代参考利率。在提供相关背景后，我们表明：（i）根据边际贷款人，更严格的监管约束可以增加或减少替代基准利率(ii）未偿政府债务金额的增加增加了替代基准，抵押利率更是如此；（iii）央行增加储备，降低替代基准。此外，我们还表明，基于替代参考利率的定期利率可以脱离银行的边际融资成本。

Keywords :基准利率；金融监管；伦敦银行同业拆借利率；回购利率；抵押品

## 

\[20\] Corporate immunity to the COVID-19 pandemic

标题:对COVID-19大流行的公司免疫

作者:Wenzhi Ding,Ross Levine,Chen Lin,Wensi Xie

链接:https://www.sciencedirect.com/science/article/pii/S0304405X21000957

DOI:https://doi.org/10.1016/j.jfineco.2021.03.005

Abstract : We evaluate the connection between corporate characteristics and the reaction of stock returns to COVID-19 cases using data on more than 6,700 firms across 61 economies. The pandemic-induced drop in stock returns was milder among firms with stronger pre-2020 finances (more cash and undrawn credit, less total and short-term debt, and larger profits), less exposure to COVID-19 through global supply chains and customer locations, more corporate social responsibility activities, and less entrenched executives. Furthermore, the stock returns of firms controlled by families (especially through direct holdings and with non-family managers), large corporations, and governments performed better, and those with greater ownership by hedge funds and other asset management companies performed worse. Stock markets positively price small amounts of managerial ownership but negatively price high levels of managerial ownership during the pandemic.

Keywords : Corporate resilience;Financial risk;Supply chain;CSR;Corporate governance

Abstract :我们利用61个经济体6700多家公司的数据，评估了公司特征与股票收益对COVID-19案例的反应之间的关系。在2020年前财务状况较好（现金和未提取信贷较多、总债务和短期债务较少、利润较大）、通过全球供应链和客户所在地接触COVID-19风险较少、企业社会责任活动较多、高管地位较不稳固的公司中，大流行导致的股票收益下降较为温和。此外，由家族控制的公司（特别是通过直接持股和非家族管理者控制的公司）、大公司和政府的股票收益表现更好，而那些由对冲基金和其他资产管理公司拥有更多所有权的公司表现更差。股票市场对少量管理层持股的定价为正，但对大流行期间高水平管理层持股的定价为负。

Keywords :企业弹性；财务风险；供应链；企业社会责任；公司治理