# HftBacktest框架：高频交易回测的终极指南

QuantML QuantML 2025-02-20 18:20 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489374&idx=1&sn=565a818235e7529710c8c8337f67416e&chksm=cf9035bfdfe7de24e723dc49888691bda4683df4fb06393474c88addf1837d7d2bca1688ab3a#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489374&idx=1&sn=565a818235e7529710c8c8337f67416e&chksm=cf9035bfdfe7de24e723dc49888691bda4683df4fb06393474c88addf1837d7d2bca1688ab3a#rd)

### 

高频交易（HFT）以其超高速和微秒级的决策能力在金融市场中占据着重要地位。然而，HFT策略的开发与优化离不开一个高效、精准的回测工具。**HftBacktest**框架正是为此而生，它专为高频交易和做市策略而设计，致力于提供更接近真实市场环境的回测体验。

###   
一、HftBacktest框架概述  

HftBacktest 是一个开源的高频交易回测框架，其设计理念源于对传统回测工具在高频率数据处理上的不足的深刻理解。该框架旨在解决以下关键问题：

1.  **延迟模拟**：高频交易中，微小的延迟都可能导致巨大的收益差异。HftBacktest  充分考虑了**数据馈延迟**和**订单执行延迟**，并支持用户自定义延迟模型，以更准确地模拟真实交易环境。
    
2.  **订单队列模拟**：在高频市场中，订单在队列中的位置对成交结果至关重要。HftBacktest  模拟了订单队列的位置，并支持用户根据不同的模型来预测订单的成交概率。
    
3.  **全订单簿重建**：基于**L2 市场深度（Market-By-Price）**和**L3 按订单市场深度（Market-By-Order）**数据，HftBacktest  能够重建完整的订单簿，为策略提供更详细的市场信息。
    
4.  **多资产和多交易所支持**：HftBacktest  支持跨资产和跨交易所的回测，帮助用户评估策略在不同市场环境下的表现。
    
5.  **实时交易机器人部署**：HftBacktest  允许用户使用与回测相同的算法代码部署实时交易机器人，目前支持 **Binance Futures** 和 **Bybit** 交易所。
...
-   **Rust 后端实现**：HftBacktest  的核心回测引擎采用 Rust 语言编写，充分利用了 Rust 的高性能和内存安全性优势，确保回测过程的高效性和可靠性。
    
-   **Numba JIT 加速**：在 Python 端，HftBacktest  利用 Numba 的即时编译器（JIT）加速关键代码路径，进一步提升了回测速度。
    

####   
2\. 精细化的市场模拟  

-   **逐笔模拟**：HftBacktest  支持**逐笔（tick-by-tick）**模拟，用户可以自定义时间间隔或基于数据馈和订单接收事件进行模拟，以更精确地反映市场动态。
    
-   **全订单簿重建**：基于 L2 和 L3 数据，HftBacktest  能够重建完整的订单簿，包括每个价格水平的挂单数量和订单 ID，为策略提供更详细的市场信息。
    
-   **延迟模拟**：用户可以使用内置的延迟模型或自定义模型来模拟数据馈延迟和订单执行延迟，从而更准确地评估策略在真实环境下的表现。
    
-   **订单队列模拟**：HftBacktest  模拟了订单在队列中的位置，并支持用户根据不同的模型来预测订单的成交概率，例如基于**Guant-Lehalle-Fernandez-Tapia 做市模型**的队列位置模型。
    

####   
3\. 灵活的策略开发接口  

-   **简洁的 API**：HftBacktest  提供了简洁易用的 API，用户可以通过继承 `Strategy` 类并重写 `on_tick` 或 `on_bar` 方法来开发自定义策略。
    
-   **多语言支持**：虽然主要使用 Python 进行策略开发，但 HftBacktest  也支持使用 Rust 编写策略，以满足对性能有更高要求的用户。
    
-   **事件驱动**：策略基于事件驱动模型，用户可以根据市场事件（如新订单簿、交易执行等）来触发策略逻辑。
    

####   
4\. 强大的数据处理能力  

-   **多数据源支持**：HftBacktest  支持多种数据格式，包括 CSV、JSON、数据库等，并支持从不同来源加载数据。
    
-   **数据融合**：HftBacktest  具备数据融合功能，可以将不同更新频率和市场深度范围的数据流融合在一起，提供最频繁和最精细的数据。
    
-   **时间戳处理**：版本 2  中，时间戳处理机制得到了改进，事件的有效性由 `EXCH_EVENT` 和 `LOCAL_EVENT`  标记，时间戳单位也从微秒改为纳秒，以提高精度。
    

####   
5\. 实时交易机器人部署  

-   **与回测代码一致性**：HftBacktest  允许用户使用与回测相同的算法代码部署实时交易机器人，确保策略在回测和实盘中的表现一致。
    
-   **外部连接器支持**：通过 IPC（进程间通信），HftBacktest  支持与外部连接器集成，例如使用 **Iceoryx**  进行高效的数据传输。
    
-   **多机器人协调**：HftBacktest  正在开发中央协调应用程序，并计划与 Telegram 集成，以实现对多个机器人的集中管理和控制。
    

###   
三、路线图与未来发展  

HftBacktest  的开发团队制定了详细的路线图，以下是一些关键的发展方向：

1.  **性能优化**：
    

-   持续改进性能报告工具，例如集成 **Quantstats**  和 **Prestolabs**  的研究成果。
    
-   添加更多性能指标和可视化功能，以提供更深入的分析。
    

3.  **功能扩展**：
    

-   增加对 **Level 3  按订单市场深度**  的支持。
    
-   支持 **修改订单**  功能。
    
-   添加 **费用模型**，包括按交易价值、数量和交易次数收费，以及基于方向的差异化费用。
    

5.  **实时交易支持**：
    

-   支持 **Level 3  按订单市场深度**  的实时交易机器人。
    
-   通过 IPC  支持外部连接器，实现多机器人统一连接。
    

7.  **数据源集成**：
    

-   计划集成更多交易所的 WebSocket  API，例如 **Binance Futures**、**Binance COIN-m 期货/现货/期权**、**OKX**、**Hyperliquid**、**Coinbase**、**Kraken**  等。
    
-   集成 **Databento**  作为数据馈来源。
    

9.  **其他改进**：
    

-   增加文档和测试覆盖率，并使用 GitHub  工作流进行持续集成和部署。
    
-   开发中央协调应用程序，并集成 Telegram  以实现对多个机器人的集中管理。
    

###   
四、示例与教程  

HftBacktest  提供了丰富的示例和教程，帮助用户快速上手：

-   **示例代码**：在 `examples`  目录和 Rust  示例中，用户可以找到各种策略的示例代码，例如基于 ARMA、ARIMA 或 GARCH  的做市策略示例。
    

![](assets/001.png)![../_images/tutorials_Market_Making_with_Alpha_-_Order_Book_Imbalance_4_0.png](assets/002.png)

-   **教程文档**：HftBacktest  提供了详细的教程文档，涵盖了从数据准备、策略开发到回测执行和结果分析的各个方面。
    
-   **迁移指南**：针对版本迁移，HftBacktest  提供了详细的迁移指南，帮助用户顺利完成版本升级。
    

  
`#回测代码   @njit   def market_making_algo(hbt):       asset_no = 0       tick_size = hbt.depth(asset_no).tick_size       lot_size = hbt.depth(asset_no).lot_size          # in nanoseconds       while hbt.elapse(10_000_000) == 0:           hbt.clear_inactive_orders(asset_no)              a = 1           b = 1           c = 1           hs = 1              # Alpha, it can be a combination of several indicators.           forecast = 0           # In HFT, it can be various measurements of short-term market movements,           # such as the high-low range in the last X minutes.           volatility = 0           # Delta risk, it can be a combination of several risks.           position = hbt.position(asset_no)           risk = (c + volatility) * position           half_spread = (c + volatility) * hs              max_notional_position = 1000           notional_qty = 100              depth = hbt.depth(asset_no)              mid_price = (depth.best_bid + depth.best_ask) / 2.0              # fair value pricing = mid_price + a * forecast           #                      or underlying(correlated asset) + adjustment(basis + cost + etc) + a * forecast           # risk skewing = -b * risk           reservation_price = mid_price + a * forecast - b * risk           new_bid = reservation_price - half_spread           new_ask = reservation_price + half_spread              new_bid_tick = min(np.round(new_bid / tick_size), depth.best_bid_tick)           new_ask_tick = max(np.round(new_ask / tick_size), depth.best_ask_tick)              order_qty = np.round(notional_qty / mid_price / lot_size) * lot_size              # Elapses a process time.           if not hbt.elapse(1_000_000) != 0:               return False              last_order_id = -1           update_bid = True           update_ask = True           buy_limit_exceeded = position * mid_price > max_notional_position           sell_limit_exceeded = position * mid_price < -max_notional_position           orders = hbt.orders(asset_no)           order_values = orders.values()           while order_values.has_next():               order = order_values.get()               if order.side == BUY:                   if order.price_tick == new_bid_tick or buy_limit_exceeded:                       update_bid = False                   if order.cancellable and (update_bid or buy_limit_exceeded):                       hbt.cancel(asset_no, order.order_id, False)                       last_order_id = order.order_id               elif order.side == SELL:                   if order.price_tick == new_ask_tick or sell_limit_exceeded:                       update_ask = False                   if order.cancellable and (update_ask or sell_limit_exceeded):                       hbt.cancel(asset_no, order.order_id, False)                       last_order_id = order.order_id              # It can be combined with a grid trading strategy by submitting multiple orders to capture better spreads and           # have queue position.           # This approach requires more sophisticated logic to efficiently manage resting orders in the order book.           if update_bid:               # There is only one order at a given price, with new_bid_tick used as the order ID.               order_id = new_bid_tick               hbt.submit_buy_order(asset_no, order_id, new_bid_tick * tick_size, order_qty, GTX, LIMIT, False)               last_order_id = order_id           if update_ask:               # There is only one order at a given price, with new_ask_tick used as the order ID.               order_id = new_ask_tick               hbt.submit_sell_order(asset_no, order_id, new_ask_tick * tick_size, order_qty, GTX, LIMIT, False)               last_order_id = order_id              # All order requests are considered to be requested at the same time.           # Waits until one of the order responses is received.           if last_order_id >= 0:               # Waits for the order response for a maximum of 5 seconds.               timeout = 5_000_000_000               if not hbt.wait_order_response(asset_no, last_order_id, timeout):                   return False          return True   `

###   
五、总结  

HftBacktest  是一个功能强大、性能优越的高频交易回测框架，适合需要高精度、低延迟回测的用户。其对延迟、订单队列和全订单簿的精细模拟，以及对多资产、多交易所的支持，使其成为高频交易策略开发和优化的强大工具。

尽管 HftBacktest  的学习曲线较陡，但其灵活性和高性能使其在高频交易领域具有显著优势。对于量化交易员和开发者来说，HftBacktest  是一个值得深入探索和尝试的工具。
...
更过资源见星球，加入QuantML星球，与750+专业人士一起交流学习：
