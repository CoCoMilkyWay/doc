# 半决策导向学习与深度集成：稳健投资组合优化框架

QuantML QuantML 2025-03-26 21:47 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489784&idx=1&sn=88ec66fdda5195f4f4399b3cfbd02b1c&chksm=cf2ce120460efef0a2a72e0ef610a6e5c94bf5d8718efd4a6f80948e5b261cc4319a342c1670#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489784&idx=1&sn=88ec66fdda5195f4f4399b3cfbd02b1c&chksm=cf2ce120460efef0a2a72e0ef610a6e5c94bf5d8718efd4a6f80948e5b261cc4319a342c1670#rd)

### 一、引言：机器学习重塑投资组合优化

近年来，机器学习技术，尤其是深度学习，正在彻底改变投资组合优化的传统方法。以下是一些关键发展：

1.  **深度学习赋能自动化与优化**:
    

-   大量研究利用深度学习技术来自动化投资组合管理流程，并提升其性能。
    
-   例如，Zhang 等人（2020 年）将**夏普比率（Sharpe Ratio）**作为目标函数，展示了决策导向学习（Decision-Focused Learning, DFL）在投资组合优化中的应用。
    

3.  **决策导向学习的优势与挑战**:
    

-   **非凸性**:  夏普比率相对于投资组合权重是非凸的，这导致优化过程不稳定，容易陷入局部最优解。
    
-   **模型复杂性**:  深度学习模型的参数与输出之间存在复杂的非线性关系，这进一步加剧了非凸性和不稳定性问题。
    

-   传统方法采用**预测-优化（Predict-then-Optimize）**的两阶段方法：首先预测模型参数，然后进行独立的决策优化。
    
-   DFL 通过将两个阶段整合到一个目标函数中，避免了预测误差在决策阶段的传播，从而提高了决策的准确性和一致性。
    

-   **优势**:  DFL 将预测和决策两个阶段统一到一个目标函数中，使得最终决策更符合实际任务需求。
    
-   **挑战**:  直接优化复杂的金融指标（如夏普比率）存在诸多困难。
    

7.  **强化学习（Reinforcement Learning, RL）方法的局限性**:
    

-   **算法选择与超参数调优困难**:  选择合适的 RL 算法并调整其超参数并非易事。
    
-   **不确定性**:  RL 方法本质上具有不确定性，导致不同运行之间的性能差异显著。
    

-   RL 方法也被应用于投资组合优化，但面临以下挑战：
    

鉴于上述挑战，开发一种更稳健、更实用的机器学习策略用于投资组合优化势在必行。

### 二、半决策导向学习框架：融合深度集成以增强稳定性

为了克服 DFL 的局限性，本文提出了**半决策导向学习（SDFL）**框架，并结合**深度集成（Deep Ensemble）**方法，以实现更稳定、更有效的投资组合优化。

#### 2.1  半决策导向学习：决策导向学习的实用化改造

SDFL 是对 DFL 的一种实用化改进，其核心思想是将决策任务转化为监督学习问题，具体方法如下：

1.  **目标投资组合的构建**:
    

-   与 DFL 直接优化复杂的金融指标不同，SDFL 采用预先计算好的、假设最优的投资组合作为训练目标。
    
-   本文探讨了两种目标投资组合：
    
-   最大化索提诺比率（Max-Sortino）投资组合:  该投资组合通过计算未来收益的均值和半方差来构建，旨在最小化下行风险。
    
-   **独热（One-Hot）投资组合**:  该投资组合将全部资金投资于未来收益最高的一只股票。
    

4.  **损失函数的选择**:
    

-   SDFL 采用**交叉熵损失函数（cross-entropy loss）**，该函数相对于模型输出是凸的，与 DFL 中使用的非凸目标函数形成对比。
    
-   之所以称为“半”决策导向，是因为尽管模型被训练用于执行决策任务，但损失函数并非该决策任务的直接性能指标。
    

6.  **深度集成的引入**:
    

-   深度学习模型参数初始化具有随机性，这会导致模型输出存在固有不确定性，进而影响投资组合优化的性能。
    
-   为了缓解这一问题，SDFL 采用深度集成方法，通过聚合多个独立训练的模型的预测结果来稳定输出。
    
-   具体而言，深度集成通过计算多个模型的预测结果的平均值来计算投资组合权重，使其对任何单个模型的初始化不敏感。
    

#### 2.2  深度集成：减少方差，提升鲁棒性

深度集成通过聚合多个深度学习模型的预测结果来提高预测性能、校准度和鲁棒性。其工作原理如下：

-   假设有多个模型 ，每个模型的预测方差为 ，不同模型之间的协方差为 （ 为相关系数）。
    
-   则集成预测器  的方差为：
    

-   当  时，模型之间的相关性小于 1，集成预测器的方差小于单个模型的方差，从而实现了方差减少。
    
-   在深度集成中，由于每个深度学习模型都是独立训练的，并且对于相同的输入会产生不同的输出，因此  的条件通常成立。
    

### 三、实验设计：验证 SDFL 的有效性

为了验证 SDFL 的有效性，本文进行了以下实验：
...
-   **投资组合 1**:  由 24 只股票组成，整体呈上升趋势。
    
-   **投资组合 2**:  由 84 只股票组成，整体呈区间波动。
    

3.  **目标投资组合**:
    

-   **Max-Sortino 投资组合**:  使用未来收益的均值和半方差进行计算。
    
-   **One-Hot 投资组合**:  投资于未来收益最高的股票。
    

5.  **模型架构**:
    

-   **本文采用了三种广泛使用的深度序列模型架构：  
    **
    
-   **长短期记忆网络（LSTM）**:  通过引入长期依赖关系扩展了基本的循环神经网络。
    
-   **Transformer**:  引入了注意力机制，可以在给定的时间范围内选择性地提取信息。
    
-   **Mamba**:  提供了一种线性时间复杂度的 Transformer 替代方案，降低了相对于序列长度的计算复杂度。
    

1.  **其他细节**:
    

-   使用每日 OHLCV（开盘、最高、最低、收盘和成交量）数据作为模型输入。
    
-   对特征进行标准化，并使用 252 天的窗口作为模型输入。
    
-   采用滚动预测程序每月更新模型，每个更新运行 100 个 epoch，并选择验证损失最低的 epoch 作为最终模型。
    
-   使用 Sophia Optimizer 进行模型训练。
...
-   图 1 展示了在 40 个基点的交易成本和两天的再平衡滞后情况下的回测性能。  
    

-   SDFL 方法在两个投资组合上都取得了比基线方法更高的累积收益。
    

-   夏普比率和索提诺比率也普遍提高，证明了所提出框架的整体有效性。
...
-   每条曲线代表回测期间开始时累积收益为 1 的累积收益。
    

-   结果反映了从 100 个独立训练的模型中聚合的集成权重。
...
-   图 2 重点分析了 Mamba 模型使用独热目标投资组合方案的情况。  
    

-   对于每个集成大小，随机选择模型样本进行 1000 次替换抽样，计算其权重的平均值，并记录聚合权重的回测性能。
    
    ![](assets/003.png)
    

-   通过生成 1000 个模型样本并对其进行自举抽样来计算集成权重。
    

-   由于其强大的回测性能、低计算成本和相对较高的可变性，Mamba 模型是分析深度集成影响的理想选择  
    

-   结果表明，随着集成规模从 1 增加到 64，累积收益持续上升，证明了集成在减少可变性、提高收益方面的优势。  
    
-   夏普比率和索提诺比率也呈现出明显的上升趋势。  
    
-   每个箱形图中四分位距的缩小表明，较大的集成规模进一步减少了方差，提高了稳定性。
    

### 四、相关工作：深入探讨投资组合优化与深度学习
...
-   讨论了基于梯度的夏普比率最大化方法，以及 RL 方法在投资组合优化中的应用。
...
-   回顾了 LSTM、Transformer 和 Mamba 等深度序列模型的研究进展。
    

7.  **深度集成**:
    

-   介绍了深度集成在分类任务和不确定性估计中的应用。
...
1.  **稳健性**:  SDFL 在两个不同的投资组合（一个整体呈上升趋势，另一个呈区间波动）上都表现出色，证明了其在不同市场条件下的稳健性和优越性能。
    
2.  **性能提升**:  与各种深度序列模型相比，SDFL 方法始终能取得更好的投资组合优化结果。
    
3.  **集成效应**:  深度集成的引入进一步提高了投资组合权重的质量。
    

#### 5.2  局限性

1.  **损失函数限制**:  SDFL 采用交叉熵损失函数，该方法仅适用于多头投资组合。 未来可以通过采用其他损失函数来扩展框架以包括空头头寸。
    
2.  **目标投资组合依赖性**:  框架的有效性取决于目标投资组合的特性。  虽然本文探讨了两种投资组合类型，但它们的精确度未必是最佳的，可能存在更适合特定投资目标的配置。
    
3.  **计算成本**:  尽管深度集成显著提高了回测性能，但也成倍增加了计算成本。  因此，从业者必须选择一种平衡性能提升和可用计算资源的集成规模。
