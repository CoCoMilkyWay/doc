# FinRL-DeepSeek：基于LLM的强化学习交易代理

QuantML QuantML 2025-02-13 22:16 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489251&idx=1&sn=a7be7c5be274ad88e6f16c4e4205b86f&chksm=cff1ac714d1fae4b8a084a7952972007b616df1d00ad52e39154646a5477241f1d4505f42aea#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489251&idx=1&sn=a7be7c5be274ad88e6f16c4e4205b86f&chksm=cff1ac714d1fae4b8a084a7952972007b616df1d00ad52e39154646a5477241f1d4505f42aea#rd)

### 

### 1\. 引言

自动化交易代理越来越多地利用强化学习（RL），但通常忽略了以下两个关键方面：
...
本文提出了一种新颖的混合RL-大语言模型（LLM）交易代理，在行动和风险层面整合了金融新闻的见解。本文的主要贡献是引入了基于LLM的新闻风险评估分数，以及基于LLM的交易推荐。这展示了LLM在新闻特征提取方面的潜力，超越了标准的情感分析，通过精心设计的提示词实现。

#### 2\. 相关工作

关于LLM增强的RL代理的研究正在增长，例如Unnikrishnan（2024）和Anonymous（2025）的研究。与这些研究相比，本文中的LLM对RL代理的影响方式更为简单。

在FinGPT（Yang et al., 2023）中，RL被应用于LLM在股票价格上的训练。这种方法比本文的方法更复杂，但具有利用现有LLM API的优势。

其他混合RL-LLM方法包括FinCon（Yu et al., 2024），它通过综合的多代理协作机制对信息进行复杂的提炼。本文仅使用简单的提示词。

这些RL-LLM方法也不同于仅使用LLM推荐的“纯LLM”代理，如Lopez-Lira和Tang（2024）以及Ding et al.（2024）的研究。

#### 3\. 数据和LLM提示词

我们使用FNSPID数据集（Dong et al., 2024），该数据集包含1570万条时间对齐的金融新闻记录，时间跨度为1999-2023年。为了减少LLM API成本，我们每天随机选择每只股票的代表新闻文章，将数据集缩减至200万条记录。使用LLM（DeepSeek V3（DeepSeek-AI et al., 2024）、Qwen 2.5 72B（Qwen et al., 2025）和Llama 3.3 70B（Grattafiori et al., 2024）），我们通过以下提示词提取股票推荐和风险评估：

**股票推荐提示词**（来自Dong et al., 2024）：

“你是一位具有股票推荐经验的金融专家。基于特定股票，打分范围从1到5，其中1为负面，2为稍微负面，3为中性，4为稍微正面，5为正面。”

**风险评估提示词**（新）：

“你是一位专门从事股票推荐风险评估的金融专家。基于特定股票，提供一个从1到5的风险评分，其中：1表示非常低的风险，2表示低风险，3表示中等风险（如果新闻缺乏任何明确的风险指示，则为默认值），4表示高风险，5表示非常高风险。”
...
OpenAI（Schulman et al., 2017）引入了近端策略优化（PPO）强化学习算法，Lele et al.（2020）和Liu et al.（2022）将其应用于股票交易。

PPO目标通过裁剪概率比来确保策略更新的稳定性：
...
###### 4.1.2 条件风险价值-近端策略优化（CVaR-PPO）

条件风险价值-近端策略优化（CVaR-PPO）（Ying et al., 2022）通过风险约束扩展了PPO，以惩罚高损失轨迹：
...
-   是CVaR置信水平（例如，对于最差的5%为0.05）。
    
-   是一个辅助惩罚参数。
    

据我们所知，我们是第一个将CVaR-PPO应用于股票交易的人。

##### 4.2 LLM增强的PPO

在LLM增强的PPO中，我们使用LLM从FNSPID数据集中计算特定股票的推荐分数 。这些分数会影响交易行为。受影响的行动是：
...
##### 4.3 LLM增强的CVaR-PPO（CPPO）

FNSPID数据集中的金融新闻通过LLM处理，为每只股票 和每天生成风险评分 。

扰动 也选择接近1以保持算法的稳定性。例如：
...
其中 是投资组合中股票 的财务权重（我们有：）。

![](assets/001.jpg)

图1. 在50万训练步骤后的回测，3年训练历史，1年交易

这些评分调整了Cvar PPO中的轨迹回报，以反映市场风险。调整后的回报定义为：

最终，金融新闻通过 和 两种方式影响交易行为。

#### 5\. 结果

##### 5.1 提前停止：40-50万训练步骤

在这些测试中，我们使用10%的LLM增强，这意味着原始PPO和CPPO高达10%。

###### 5.1.1 图1：

训练周期：2019-2022年 训练迭代：50万步（25个周期，每个周期2万步）

交易周期：2023年

图1中与Qwen 2.5一起的初步结果表明，整合LLM股票推荐在累积回报方面始终提高了PPO。然而，到目前为止，它还不足以击败纳斯达克100基准。

###### 5.1.2 图2：

训练周期：2013-2018年 训练迭代：40万步（20个周期，每个周期2万步） 交易周期：2019-2023年

结果表明，当PPO和CPPO采用更长的训练周期（6年对比3年）时，PPO的性能显著提高。然而，PPO仍然过于不稳定。

在其他条件相同的情况下，DeepSeek V3略好于Llama 3.3。在这个测试中，使用LLM总是会降低性能。

![](assets/002.jpg)图2. 在40万训练步骤后的回测，6年训练历史，3年交易

![](assets/003.jpg)

图3. 在训练了200万步（100个周期，每个周期2万步）后
...
在这两次运行中（RL代理是随机的），PPO和CPPO-DeepSeek的性能优于其他方法，包括纳斯达克100基准。

PPO在牛市期间表现更好，而CPPO-DeepSeek在熊市期间表现更好，过渡发生在2021年底乌克兰战争和随后的危机之前。

![](assets/005.jpg)图4. 在训练了200万步（100个周期，每个周期2万步）后的第二次运行

![](assets/006.jpg)图5. LLM增强对PPO的影响

##### 5.3 LLM增强强度的影响

在以下测试中，我们将LLM增强强度参数从10%调整到0.1%（即LLM扰动参数从0.9-1.1变化到0.999-1.001）。

###### 5.3.1 图5：

![](assets/007.png)

对于PPO-DeepSeek，更强的LLM增强主要会降低性能，即使是微小的（0.1%）扰动也是如此。

###### 5.3.2 图6：

对于CPPO-DeepSeek，更强的LLM增强会提高性能。
...
我们提出了用于算法交易的LLM增强的RL代理，整合了来自新闻的股票交易推荐和风险评估。未来的研究方向包括以下几个方面：

• **优化RAM使用**：更长时间的训练需要更多的RAM。例如，50万训练步骤使用了16GB+交换内存，而200万训练步骤需要128GB+交换内存。优化内存效率对于可扩展性至关重要。

• **减少时间尺度**：由于市场对事件反应迅速且突然，减少决策的时间尺度可以提高交易性能。

• **提高新闻信号质量**：提高FNSPID数据集中新闻信号的质量是确保更好市场表现的重要方向。
