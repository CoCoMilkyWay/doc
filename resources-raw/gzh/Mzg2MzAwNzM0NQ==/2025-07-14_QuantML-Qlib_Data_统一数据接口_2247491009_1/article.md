...
QuantML QuantML 2025-07-14 17:57 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247491009&idx=1&sn=ab64f8fe49a5b376a6f4379a80314a2a&chksm=cffa787a65c67dabcc52945bb36e4119f1200d065d1e5f267682d82b43678ee0836d248fa354#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247491009&idx=1&sn=ab64f8fe49a5b376a6f4379a80314a2a&chksm=cffa787a65c67dabcc52945bb36e4119f1200d065d1e5f267682d82b43678ee0836d248fa354#rd)

之前我们介绍过几种quantml-qlib的数据接入/更新方式，详见：

[QuantML-Qlib开发版 | 直接读取pg/mysql/mongodb数据库](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247484180&idx=2&sn=69ca761ab7d152740a3058b86ed02e6c&scene=21#wechat_redirect)

[QuantML-Qlib开发版 | 一键数据更新](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247484065&idx=2&sn=b2ad5cf74fcc452e49f7e4c4ec07439b&scene=21#wechat_redirect)

[QuantML-QlibDB | Clickhouse 行情存储与读取方案](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247484391&idx=1&sn=b56d54740da5d77bef608d787033e321&scene=21#wechat_redirect)

  

由于许多用户更多采用API数据导入方式，本次重构引入了全新的**统一数据接口系统**，实现了对多种主流数据源（RQData、TuShare、AKShare等，注意AKShare和TuShare会有调用频率限制还未全量测试，建议使用RQData）的无缝支持，并极大提升了系统的可扩展性、易用性和健壮性。

## 一、系统架构与设计理念

### 1\. 抽象与解耦

-   采用**抽象基类（BaseDataInterface）**，统一定义了数据接口的核心方法（如获取交易日历、股票列表、行情数据、停牌信息等）。
    
-   通过**工厂模式（DataInterfaceFactory）**实现数据接口的动态注册与实例化，便于后续扩展更多数据源。
    

### 2\. 策略与模板方法

-   各数据源实现类（如RQData、TuShare、AKShare）均继承自基类，分别实现自身的认证、数据获取、异常处理等细节。
    
-   统一的数据导出流程（如全量导出、增量更新）通过模板方法模式实现，保证流程一致、细节可定制。
    

### 3\. 配置与依赖注入

-   所有数据源的认证信息、参数、能力描述均集中在配置文件中管理，支持环境变量和动态切换。
    
-   支持灵活配置股票池、字段、频率等，满足不同业务需求。
...
-   支持**RQData**（专业、实时、付费）、**TuShare**（部分免费、日频、需Token）、**AKShare**（完全免费、日频、无需认证）等主流数据源。
    
-   用户只需在配置中切换数据源类型和参数，即可实现不同数据源的无缝切换，无需修改主流程代码。
    

### 2\. 统一API与数据格式

-   不论底层数据源如何，均通过统一的接口方法（如`get_trading_dates`、`get_stock_list`、`get_comprehensive_stock_data`等）获取数据，极大简化了上层业务逻辑。
    
-   所有数据最终导出为Qlib标准二进制格式，保证与原有系统的兼容性。
    

### 3\. 支持多频率、多股票池

-   支持日频、分钟级（1min、5min等）等多种数据频率。
    
-   支持主流指数成分股、全市场、用户自定义股票池等多种股票池配置。
    

### 4\. 增量与全量灵活更新

-   **全量模式**：一键导出全部历史数据，适合新环境初始化。
    
-   **增量模式**：自动检测已有数据的最新日期，仅补充缺失部分，极大提升数据更新效率，节省带宽和计算资源。
    

### 5\. 稳健的异常处理与日志

-   各数据接口均实现了详细的异常捕获和日志记录，便于问题追踪和维护。
    
-   支持自动重试、错误告警等机制，提升系统稳定性。
    

## 三、使用方法

目前提供全量更新和增量更新两种方式，默认使用rqdata作为更新数据源，可以替换为akshare以及tushare，如需支持其他数据接口，在data\_interfaces目录下增加对应的interface接口即可。

![](assets/001.png)

以增量更新为例，运行命令便可更新对应数据。

 `python .\unified_data_dumper.py dump_update  --update-days 10 --qlib-dir D:\\qlib_data\\qlib_cn_bak --include_fields open,close,high,low,volume,total_turnover,tradestatus` 

更新数据前请做好备份，以防出错。运行结果：
...
本次重构后的统一数据接口系统，极大提升了数据导出系统的灵活性、可扩展性和易用性。无论是个人研究还是企业级量化平台，都能根据自身需求灵活选择、切换和扩展数据源，显著提升了数据管理与策略开发的效率。

下次更新会增加airflow数据更新任务调度管理功能，敬请期待。
