# AAAI 25 | FactorGCL：基于超图的时序残差对比学习股票收益预测模型

QuantML QuantML 2025-02-12 18:05 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489234&idx=1&sn=63310a0d4f3d56e7064e93ce12698b82&chksm=cf82fa085486ade41f27cbb9cc8ce37ddc721c0181985709f3915fbf7833425fe867688c5e3d#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489234&idx=1&sn=63310a0d4f3d56e7064e93ce12698b82&chksm=cf82fa085486ade41f27cbb9cc8ce37ddc721c0181985709f3915fbf7833425fe867688c5e3d#rd)
...
1\. 引言：因子模型在股票投资中的重要性与挑战

在股票投资领域，因子模型一直是解释和预测股票收益的基石。该模型通过特定的变量（称为因子）来阐明股票价格的波动，在学术界和工业界都得到了广泛应用。因子模型的核心思想是，股票收益可以通过各种因子及其对股票的影响（即因子暴露）来解释。传统的因子模型，如Fama-French三因子模型，使用市场、规模和价值等手动设计的因子来解释股票收益。然而，这些基于人类经验设计的因子数量有限，可能无法充分解释股票收益的复杂变化。

近年来，股票价格趋势在不同行业之间表现出高度相关性，这些相关性无法仅通过基于人类经验的行业特定因子来充分解释。例如，在COVID-19疫情期间，电子消费品和医疗行业的股票价格趋势高度相关，这表明需要更复杂的模型来捕捉市场行为。此外，现有的大多数因子模型都是线性的，通过因子暴露的线性组合来解释股票收益。然而，越来越多研究表明，市场中因子与股票收益之间存在复杂的非线性关系。这种差异凸显了线性因子模型在捕捉实际市场行为方面的局限性。因此，当前的核心问题是如何挖掘出更有效的因子，以更好地适应真实的市场行为。

![](assets/002.png)

### 2\. 机器学习为因子模型研究带来的新视角

机器学习（ML）的发展为因子模型研究带来了新的视角。基于机器学习的方法能够以数据驱动的方式学习复杂且非线性的市场模式，并构建更具市场适应性的模型。然而，股票市场数据信噪比低，这可能会使基于机器学习的因子模型的学习过程复杂化。

当前大多数基于机器学习的方法直接从市场数据中提取因子，而没有有效利用人类先验经验。这可能导致提取的因子过度拟合市场噪声，而不是捕捉到有效的模式。这种局限性对机器学习在因子模型中的应用构成了重大障碍。

### 3\. FactorGCL：基于超图的时序残差对比学习因子模型

为了应对上述挑战，本文提出了一种基于超图的时序残差对比学习因子模型（FactorGCL），该模型利用超图结构更好地捕捉股票收益与因子之间的高阶非线性关系，并通过时序残差对比学习来指导有效且全面的隐藏因子的提取。

![](assets/003.png)

#### 3.1 超图结构：捕捉高阶非线性关系

超图是一种广义图结构，它允许一条边（称为超边）连接两个或多个节点。这种结构使得超图能够捕捉到超越成对连接的组间相关性。在FactorGCL中，股票被视为超图的节点，因子被表示为超边，隐藏因子的挖掘被框定为超边生成任务。

具体来说，FactorGCL采用了一种级联残差超图架构，将股票收益分解为三个组成部分：先验贝塔（prior beta）、隐藏贝塔（hidden beta）和个体阿尔法（individual alpha）。每个组成部分都是从去除前一个组成部分的影响后的残差中提取出来的。

![](assets/004.png)

#### 3.2 级联残差超图架构：逐步提取隐藏因子

**特征提取器**: 给定原始的序列市场数据，特征提取器对股票的时间特征进行编码，以捕捉丰富的时间信息。该过程定义为 ，其中 表示特征嵌入的维度。为了捕捉序列数据中的长期依赖关系，本文使用带有批归一化的门控循环单元作为特征提取器，并使用最后一个时间步的隐藏状态作为股票特征嵌入。

**先验贝塔模块**: 为了利用给定的 个先验因子 的专家知识，本文采用超图卷积神经网络（HyperGCN）来对股票收益与这些因子之间的非线性关系进行建模。具体来说，股票被表示为超图中的节点，因子被表示为超边。暴露于相同因子的股票通过相同的超边连接，关联矩阵代表因子暴露。本文认为，HyperGCN 中的信息传播机制有效地捕捉了因子对股票的非线性影响。该过程包括以下步骤：

-   **消息提取**: 对每个节点特征应用变换矩阵以提取有表现力的信息。
    
-   **消息聚合**: 聚合由同一超边连接的股票节点的信息，代表相应因子的共享信息。
    
-   **消息共享**: 将节点嵌入与共享因子信息整合，作为因子对股票的影响。
    

**隐藏贝塔模块**: 如前所述，基于人类先验知识的因子可能无法充分捕捉股票收益。为了解决这个问题，本文设计了一个隐藏贝塔模块，用于提取补充这些先验因子的隐藏因子。具体来说，将隐藏因子的提取视为一个超边生成任务：在去除先验因子信息后，隐藏贝塔模块从残差嵌入中生成新的超边，并构建一个新的超图来模拟这些隐藏因子对股票的非线性影响。

**个体阿尔法模块**: 除了先验和隐藏因子的影响外，股票本身的特质信息（即阿尔法）也对股票收益有显著影响。个体阿尔法模块处理去除先验和隐藏因子嵌入后的残差嵌入，以捕捉股票特定信息。本文通过应用带有 LeakyReLU 激活函数的线性层来计算个体阿尔法嵌入 。

![](assets/005.png)

#### 3.3 时序残差对比学习：引导模型提取有效且全面的隐藏因子

如前所述，数据驱动的因子模型面临着市场数据信噪比低的问题。具体来说，通过此类模型挖掘出的隐藏因子主要面临两个问题：

-   **有效性**: 从历史数据中提取的隐藏因子在将来应保持有效。然而，数据驱动方法提取的因子容易过度拟合市场噪声，从而缺乏预测未来股票收益的有效性。
    
-   **全面性**: 隐藏因子应补充先验因子，以提供对股票收益的全面描述。然而，市场噪声使因子挖掘复杂化，使得模型更有可能提取出简单的因子，同时忽略其他因子，从而无法充分代表股票收益。
    

为了解决这些问题，本文开发了一种自监督对比学习方法，称为时序残差对比学习。其动机是：对于一个具有有效且全面因子的因子模型，在去除所有因子信息后，残差（即阿尔法嵌入 ）应仅包含每个股票特有的特质信息，与其他股票无关。基于这一直觉，本文从 \[26\] 中汲取灵感，设计了一种在股票节点级别的跨时序对比学习方法。

![](assets/006.png)

### 4\. 实验结果：FactorGCL 优于现有方法

本文在真实股票市场数据上进行了广泛的实验，以证明所提出方法的有效性。实验结果表明，FactorGCL 在股票趋势预测任务中优于现有的最先进方法，并且在挖掘预测股票收益的有效隐藏因子方面也表现出色。

![](assets/007.png)

**主要结果**:

-   FactorGCL 在信息系数（IC）和信息系数比率（ICIR）方面均优于其他方法，表明其在预测股票趋势方面的有效性。
    
-   与一些同样能够从市场数据中提取隐藏关系的基线方法（如 HIST、FactorVAE 和 CI-STHPAN）相比，FactorGCL 能够更有效、更全面地挖掘隐藏因子，从而实现更高的预测性能。
    

**消融研究**:

-   通过构建四个模型变体，分别去除先验贝塔模块、隐藏贝塔模块、个体阿尔法模块和对比学习损失，本文验证了模型中不同模块的有效性。结果表明，模型中的每个模块都有助于整体性能的提升。
    
-   此外，实验还评估了隐藏因子数量 对模型性能的影响。结果表明，随着隐藏因子数量的增加，模型性能有所提高，这表明隐藏因子在增强模型的预测能力方面起着至关重要的作用。然而，超过一定限度后，隐藏因子数量的进一步增加会导致性能下降，这表明过多的隐藏因子会导致过度拟合，从而降低模型的效率。
...
-   为了进一步评估该方法在真实股票市场中的盈利能力，本文进行了投资模拟。投资策略采用简单的股票选择策略，即 TopK 策略。该策略涉及在每个交易日投资于预测得分最高的 TopK 股票，并在持有 天后卖出。在模拟回测中，本文从代表大盘股和中到小盘股的 CSI 300 和 CSI 500 指数中选择股票，对不同市场领域的投资策略进行平衡和全面的评估。
    
-   结果表明，FactorGCL 在所有指标上均取得了最佳表现，表明该模型能够在真实市场中实现盈利投资。
...
### 5\. 结论：超图因子模型在金融分析中的潜力

本文提出了一种新颖的基于超图的时序残差对比学习因子模型（FactorGCL），该模型利用人类设计的先验因子和数据驱动的隐藏因子来预测股票收益。该模型采用级联残差超图架构，其中隐藏因子是从去除先验因子信息后的残差信息中提取出来的。为了提高隐藏因子的有效性和全面性，本文设计了一种时序残差对比学习方法，该方法对比不同时间段的股票特定残差嵌入。广泛的实验表明，FactorGCL 在预测准确性和投资盈利能力方面均优于现有的最先进基线方法，消融研究进一步验证了该方法能够有效提取隐藏因子以提高模型性能。

### 6\. 总结

本文通过构建 FactorGCL 模型，为股票收益预测提供了一种创新的方法。该模型结合了超图结构、级联残差架构和时序残差对比学习，展现出强大的特征提取能力和预测性能。其主要优势在于：

-   **高效捕捉高阶关系**: 超图结构能够有效捕捉股票与因子之间复杂的高阶关系。
    
-   **有效利用先验知识**: 先验贝塔模块利用人类先验知识，为模型提供有价值的指导。
    
-   **挖掘隐藏模式**: 隐藏贝塔模块能够挖掘出对股票收益有重要影响的隐藏因子。
    
-   **增强模型鲁棒性**: 时序残差对比学习通过对比不同时间段的残差信息，增强了模型的鲁棒性和泛化能力。
