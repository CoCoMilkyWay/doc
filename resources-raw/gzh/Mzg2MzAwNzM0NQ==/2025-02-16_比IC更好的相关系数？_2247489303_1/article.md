...
QuantML QuantML 2025-02-16 22:01 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489303&idx=1&sn=8a13dd1e0ed87f7b1e1c9dbe4cb17b48&chksm=cf06438704f6a5690a8c0f5380aa83d7b688b195030a7fd09d09552e4747ec91d1b4e2f757d7#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489303&idx=1&sn=8a13dd1e0ed87f7b1e1c9dbe4cb17b48&chksm=cf06438704f6a5690a8c0f5380aa83d7b688b195030a7fd09d09552e4747ec91d1b4e2f757d7#rd)

### 

# 在数据科学中，量化不同变量之间的关系强度至关重要。相关系数是衡量这种关系的基本工具。2021 年，斯坦福大学的 Sourav Chatterjee 发表了一篇论文，介绍了一种新的相关系数，在统计学界引发了许多讨论。

传统上，Pearson 相关系数用于衡量线性关系，Spearman 和 Kendall 相关系数用于衡量单调关系。然而，这些传统方法无法捕捉到非单调关系。为了解决这个问题，Chatterjee 提出了一个新的相关系数，旨在检测更一般的双变量关系。

Chatterjee相关系数（Chatterjee's Correlation Coefficient，简称ξ，读作“xi”）是一种用于衡量两个变量之间依赖关系的非参数统计量。它由Sourav Chatterjee于2021年提出，旨在捕捉变量之间的单调关系，且不依赖于线性假设。

它看起来很有前景，并且目前已有多个实现版本。在本文中，我们将简要描述这个系数独特和有用的地方，以及它如何与最常用的皮尔逊相关系数（PCC）进行比较，后者常被用来展示皮尔逊相关系数的缺点。

![](assets/001.jpg)_不同数据依赖性下的皮尔逊相关系数。来源：维基百科_

# 数据创建

从最上面开始，我们有不同斜率的线条和逐渐增加的（随后减少的）随机性。第二行图表只是具有不同斜率参数的线性依赖性。我们使用以下代码处理这两行：

```
import numpy as npx = np.random.normal(size=500)y = np.random.normal(size=500)to_corrs_x = np.array(np.zeros((21, 500)))for i in range(14):    to_corrs_x[i, :] = xto_corrs_y = np.array(np.zeros((21, 500)))to_corrs_y[:14, :] = np.array([    x, x + y, 0.5 * x + y, y, -0.5 * x + y, -x + y, -x,    x, 7 * x, 3 * x, np.zeros(500), -0.3 * x, -0.7 * x, -x])
```

第三组数据集更有趣。第一个图表只是具有一些Y轴随机性的三角函数依赖性。第二个和第三个是旋转的方形。要绘制它们，我们将在所谓的曼哈顿度量中创建一个球体，并使用旋转矩阵来创建旋转。对于二维空间，它将简单地乘以一些三角函数。此外，还需要对X轴进行一些修改，以便更好地绘制它们。

```
to_corrs_x[14:, :] = np.array([    x,    x * np.sqrt(2) / 2 - np.sqrt(2) / 2 * y,    -y * 0.5 + 0.5 * np.sqrt(3) * x,    np.cos(x),    (x > 0) + np.random.normal(0, 0.1, 500) - 0.5])to_corrs_y[14:, :] = np.array([    np.sin(3 * x) + 0.5 * y,  # 第三行的第一个图表    x * np.sqrt(2) / 2 + np.sqrt(2) / 2 * y,  # 第三行的第二个图表 - 第一个立方体    x * -0.5 + 0.5 * np.sqrt(3) * y,  # 第三行的第三个图表 - 第二个立方体    x * 2 + y,    np.sin(x),    (y > 0) + np.random.normal(0, 0.1, 500) - 0.5])
```

请告诉我是否需要深入了解这些图表的创建过程。最后，让我们看看我们的数据！

```
fig, ax = plt.subplots(2, 7, dpi=300, figsize=(14, 4), sharey='row')for i in range(1, 15):    ax[(i-1)//7, (i-1)%7].plot(to_corrs_x[i-1, :], to_corrs_y[i-1, :], '.', color='navy')    ax[(i-1)//7, (i-1)%7].set_axis_off()fig, ax = plt.subplots(1, 7, dpi=300, figsize=(14, 1.5))for i in range(1, 8):    ax[-i].plot(to_corrs_x[-i, :], to_corrs_y[-i, :], '.', color='navy')    ax[-i].set_axis_off()
```
...
Chatterjee的相关系数（用希腊字母标记）验证变量是否可以表示为变量的可测量函数。类似于不同的非参数方法，我们创建计算系数值的过程。其属性和确切过程已被多次描述，因此我将跳过此过程。然而，有一件事在关于这个系数的使用方面至关重要。

正如Chatterjee所声称的，所提出的方法有一个缺点：对于小样本的系数功率。这个问题在最近的论文中得到了修正，但并非所有实现都考虑到了。

我将使用scipy.stats包获得的PCC与三种编译参数的方法进行比较：

1.  xicorpy包（版本0.6）（在图表上表示为和），
    
2.  xicorrelation包（版本0.3.0）（在图表上表示为），
    
3.  Tim Sumner在他的帖子中提出的纯实现（在图表上表示为）。
    

```
from scipy import stats  # 需要计算PCCfrom xicorpy import compute_xi_correlation  # xi ad. 1from xicorrelation import xicorr  # xi ad. 2from numpy import array, random, arangedef med_xicor(X, Y, ties=True):  # xi ad. 3    random.seed(42)    n = len(X)    order = array([i[0] for i in sorted(enumerate(X), key=lambda x: x[1])])    if ties:        l = array([sum(Y >= Y[order[i]]) for i in range(n)])        r = l.copy()        for j in range(n):            if sum([r[j] == r[i] for i in range(n)]) > 1:                tie_index = array([r[j] == r[i] for i in range(n)])                r[tie_index] = random.choice(r[tie_index] - arange(0, sum(tie_index)))        return 1 - n * sum(abs(r[1:] - r[:n-1])) / (2 * sum(l * (n - l)))    else:        r = array([sum(Y >= Y[order[i]]) for i in range(n)])        return 1 - 3 * sum(abs(r[1:] - r[:n-1])) / (n**2 - 1)
```

xicorrelation包有一个额外的参数，允许我们选择是否希望包括的修正。

![](assets/003.jpg)

如我们所见，所有方法在计算原始系数公式时返回相同的值。使用Xicorpy，我们可以使用修改后的版本，这在处理三角函数（PCC=0.0480对于三角函数图 vs. 修改后）和平方依赖性（PCC=-0.0168对于抛物线图 vs. 修改后）时表现相对较高，而皮尔逊甚至斯皮尔曼系数则失败。另一方面，对于第一行的点，我们可以看到一些依赖性，系数接近于零。相比之下，PCC仍然表明线性依赖性（例如，在第三个图表上，PCC=0.4420，而修改后）。这是因为系数对数据的噪声性敏感。另一方面，这个系数的解释是不同的。最大值1表明Y是X的函数，这在准备好的数据中显然不可能是这种情况。

然而，系数相对于PCC有一个很大的优势——没有关于X和Y分布的假设。唯一的假设是Y不能是常数——这就是为什么在我们的图表中心的常数图表上系数表现特殊的原因。这意味着它有更广泛的应用，为我们提供了更多关于变量之间关系的信息。

# 结论

系数比PCC更好吗？像往常一样，在数据科学中，这取决于。尽管更通用且更容易应用，但我们不知道系数所表明的依赖性类型。我们需要可视化数据以查看潜在关系的类型，但这仍然可能难以解释。它是周期性的吗？它是单调的吗？没有给出具体的答案。线性依赖性，虽然包含了一些假设，但很容易解释数据之间的依赖性类型。就我个人而言，我认为当无法应用更精确但也更具挑战性的已知和广泛使用的方法时，系数将非常适合使用。
