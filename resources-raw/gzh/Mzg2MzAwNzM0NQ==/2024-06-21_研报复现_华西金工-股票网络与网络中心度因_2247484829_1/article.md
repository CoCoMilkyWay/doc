...
张世良 QuantML 2024-06-21 17:06 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247484829&idx=1&sn=3da8197374301bab9610f7ed5d4b715b&chksm=cf37a7ba7905b87ba06beb371c6c926a8d2ff4393ae54d27a5d4cee51293dee5759f0f6be950#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247484829&idx=1&sn=3da8197374301bab9610f7ed5d4b715b&chksm=cf37a7ba7905b87ba06beb371c6c926a8d2ff4393ae54d27a5d4cee51293dee5759f0f6be950#rd)

Content

  

本文来自星球嘉宾张世良（微信号：joker19930309）的研报复现项目。  

**一、概述**

参考《华西证券-股票网络与网络中心度因子研究-20210314》研报中的逻辑，使用日频收益率数据，构建网络中心度因子，并进行历史数据样本内回测。
...
市场中不同股票之间存在互相影响，每只股票的价格波动并非仅受自身的基本面影响，同时也受到其他股票价格波动的影响。如果将股票市场看作一个复杂网络，那么不同股票可以视为网络中的节点，股票之间两两价格波动的相关性作为相连的边。一般认为，在网络中关联较强的公司，其股价往往也会呈现相似的走势，因而可以利用股票之间的相关性信息，使用关联股票最近一段时间的表现对个股未来的走势进行预测。

  

**2.股价波动关联的具体定义**

（1）使用Pearson相关系数来衡量股票之间价格波动的相关性，进而构建股票连接网络。对于给定股票i和j, 计算两只股票的Pearson相关系数：

![](assets/002.png)

（2）在此基础之上，计算不同股票节点之间的距离：

![](assets/003.png)

和负相关，以不同的方式刻画了股票间的网络连接。
...
SCC因子基于股票收益率时间序列的平均相关系数来构建股票网络。

首先，计算股票i和样本股票池中其他股票的近20个交易日收益率序列之间的平均Pearson相关系数:

![](assets/004.png)

随后，根据计算股票i和其他股票间的平均距离：

![](assets/005.png)

反映了单只股票与其他股票的平均距离，该值越小，说明t时刻股票与其他股票的距离越近，与其他股票互相影响程度越高，处在网络的相对中心位置。

最后，计算空间维度的网络相对中心度（SCC）：

![](assets/006.png)

**（2）时间网络中心度因子TCC**

TCC因子基于股票相对于市场的平均收益的偏离程度来构建股票网络。

首先，计算股票i在t时刻截面上，相对于样本股票池的平均收益的偏离程度：

![](assets/007.png)

表示t时刻样本股票池内全部股票收益率的标准差，越大说明股票i的收益与其他股票平均收益的偏离越大，从而相关性越不稳定，随时间的推移在网络中的位置变化越大。

随后，计算过去20个交易日股票i对其他股票收益的偏离程度的平均值：

![](assets/008.png)

最后，计算时间维度的网络相对中心度（TCC）：

![](assets/009.png)

**（3）网络中心度CC**

综合空间网络中心度SCC和时间网络中心度TCC，将SCC与TCC进行ZScore后等权合成，获得股票网络中心度指标CC:
...
在全A样本中，全历史区间内（2010.1至2024.5）:

SCC因子月频IC均值8.67%，年化ICIR为2.94, IC胜率79.77%；

TCC因子月频IC均值10.58%，年化ICIR为3.04，IC胜率82.08%；

二者等权合成的网络中心度CC因子，月频IC均值9.88%，年化ICIR为3.49,IC胜率86.71%。

TCC和SCC因子分10组单调性良好，且经过等权合成后，ICIR有所提高，稳定性提升，在全历史区间内总体表现优异。

2018年至今，SCC因子月频IC均值7.43%，年化ICIR为2.35；

TCC因子月频IC均值8.88%，年化ICIR为2.59；CC因子月频IC均值8.29%，年化ICIR为2.86。因子近年有一定衰减，但整体表现尚可。

  

**后续拟复现券商高频量价系列因子，敬请期待！**
