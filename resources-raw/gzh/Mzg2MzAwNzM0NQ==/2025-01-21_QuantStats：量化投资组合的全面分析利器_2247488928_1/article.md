# QuantStats：量化投资组合的全面分析利器

QuantML QuantML 2025-01-21 22:24 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247488928&idx=1&sn=58cdab8713f4647fcab47dc1757840e7&chksm=cf0f3073c906a8df47d5ace4661e777dcb0e5b6c9bdee83972e677271fbbc528ad486334171f#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247488928&idx=1&sn=58cdab8713f4647fcab47dc1757840e7&chksm=cf0f3073c906a8df47d5ace4661e777dcb0e5b6c9bdee83972e677271fbbc528ad486334171f#rd)

QuantStats 是一个开源的Python库，旨在简化量化投资策略的分析流程。它能够计算详细的统计和风险指标，并生成直观且信息丰富的可视化报告。与传统的分析方法相比，QuantStats 提供了以下优势：

1.  **全面性**：涵盖从基本收益分析到高级风险指标和策略优化的广泛功能。
    
2.  **易用性**：通过简洁的代码和直观的函数调用，用户可以快速上手并生成专业报告。
    
3.  **可视化**：提供多种图表选项，帮助用户更直观地理解数据和分析结果。
    
4.  **灵活性**：支持多种数据源和自定义分析，满足不同用户的需求。
    

### 项目地址：ranaroussi/quantstats: Portfolio analytics for quants, written in Python  
  
主要功能模块

QuantStats 主要包含以下三大功能模块：

#### 1\. **quantstats.stats：计算绩效指标**

QuantStats 提供了丰富的绩效指标，帮助用户全面评估投资策略的表现，部分指标列表：

-   **收益类指标**：
    

-   \*\*累计收益 (Cumulative Returns)\*\*：展示策略随时间的总体收益情况。
    
-   \*\*年化收益率 (Annualized Return)\*\*：将总收益转换为年化收益率，便于比较不同时间跨度的策略。
    
-   \*\*年化波动率 (Annualized Volatility)\*\*：衡量收益的波动性，反映策略的风险水平。
    

-   **风险调整收益指标**：
    

-   \*\*夏普比率 (Sharpe Ratio)\*\*：衡量每单位风险的超额收益，是评估投资策略的重要指标。
    
-   \*\*索提诺比率 (Sortino Ratio)\*\*：与夏普比率类似，但仅考虑下行风险，更适合评估对下行风险敏感的投资策略。
    
-   \*\*卡尔玛比率 (Calmar Ratio)\*\*：衡量收益与最大回撤之间的关系，反映策略的抗风险能力。
    

-   **风险指标**：
    

-   \*\*最大回撤 (Maximum Drawdown)\*\*：衡量策略在特定时间段内的最大亏损幅度，是评估风险的关键指标。
    
-   \*\*回撤持续时间 (Drawdown Duration)\*\*：分析策略从峰值回落到谷底并恢复到峰值所需的时间。
    
-   \*\*风险价值 (Value at Risk, VaR)\*\*：估计在一定置信水平下，策略可能遭受的最大损失。
    

-   **其他指标**：
    

-   \*\*胜率 (Win Rate)\*\*：策略盈利交易的比例。
    
-   \*\*盈亏比 (Win/Loss Ratio)\*\*：盈利交易平均收益与亏损交易平均损失的比值。
    

#### 2\. **quantstats.plots：生成可视化图表**

QuantStats 提供了多种图表选项，帮助用户更直观地理解数据和分析结果：

-   **收益相关图表**：
    

-   \*\*累计收益图 (Cumulative Returns Plot)\*\*：展示策略随时间的收益走势。
    
-   \*\*月度收益图 (Monthly Returns Heatmap)\*\*：以热力图的形式展示各个月份的收益情况，便于识别季节性趋势。
    

-   **回撤相关图表**：
    

-   \*\*回撤图 (Drawdown Plot)\*\*：直观展示策略的回撤情况，帮助识别策略的波动性和风险。
    
-   \*\*滚动回撤图 (Rolling Drawdown Plot)\*\*：展示不同时间窗口内的回撤情况，提供更细致的风险分析。
    

-   **风险相关图表**：
    

-   \*\*波动率图 (Volatility Plot)\*\*：展示策略收益的波动性变化趋势。
    
-   \*\*风险价值图 (VaR Plot)\*\*：展示不同置信水平下的风险价值变化。
    

-   **其他图表**：
    

-   \*\*收益分布图 (Returns Distribution Plot)\*\*：展示策略收益的分布情况，帮助识别收益的偏度和峰度。
    
-   \*\*滚动相关性图 (Rolling Correlation Plot)\*\*：展示策略与基准指数之间的滚动相关性变化。
    

#### 3\. **quantstats.reports：生成综合报告**

QuantStats 的报告功能可以将多个指标和图表整合到一个HTML文件中，生成全面且易于分享的策略评估报告：

-   \*\*完整报告 (Full Report)\*\*：包含所有主要指标和图表，提供对策略的全面评估。
    
-   \*\*自定义报告 (Custom Report)\*\*：用户可以根据需求选择特定的指标和图表，生成个性化的报告。
    

### 如何使用QuantStats？

以下是一个使用QuantStats分析苹果公司股票收益的简单示例：

```
# 1. 安装并导入QuantStats!pip install quantstats --upgrade --no-cache-dirimport quantstats as qs# 2. 获取苹果公司股票日收益数据stock = qs.utils.download_returns('AAPL')# 3. 生成收益快照图qs.plots.snapshot(stock, title='Apple Performance', show=True)# 4. 计算夏普比率sharpe_ratio = qs.stats.sharpe(stock)print(f"夏普比率: {sharpe_ratio}")# 5. 生成与标普500指数的对比报告sp500 = qs.utils.download_returns('SPY')qs.reports.html(stock, benchmark=sp500, output='report.html')
```
...
QuantStats 还可以用于策略优化，例如应用简单的移动平均线策略：

```
# 1. 计算50日移动平均线sma = stock.rolling(50).mean()# 2. 构建策略：价格高于移动平均线时买入，否则卖出strategy = qs.utils.make_index((stock > sma) * stock)# 3. 将策略与原始股票收益进行对比qs.plots.returns(strategy, benchmark=stock, title='SMA Strategy vs AAPL')
```

### 总结

QuantStats 是一个功能强大且易于使用的工具，能够帮助量化分析师和投资者深入分析投资策略的表现。通过其全面的指标计算、可视化图表和综合报告功能，用户可以更有效地评估风险、识别趋势，并优化投资策略。无论您是经验丰富的量化分析师，还是刚刚接触的新手，QuantStats 都能为您提供强大的分析能力，助力您在金融市场中取得成功。

希望这篇文章能帮助您更好地理解和使用 QuantStats。如果您有任何疑问或建议，欢迎在评论区留言。
