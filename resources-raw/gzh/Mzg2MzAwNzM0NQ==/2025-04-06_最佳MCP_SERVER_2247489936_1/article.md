...
QuantML 2025-04-06 21:59 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489936&idx=1&sn=f632aa0f72d71537e2f10bcc7f6c2c38&chksm=cf2ff5e3ee090d55a2c434f1611de97a94ffdfce99edfff9c23f78074fb101a5f54994975aac#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489936&idx=1&sn=f632aa0f72d71537e2f10bcc7f6c2c38&chksm=cf2ff5e3ee090d55a2c434f1611de97a94ffdfce99edfff9c23f78074fb101a5f54994975aac#rd)

#### Gmail MCP

让AI接管您的电子邮件，从发送、摘要到草拟邮件内容。

#### Discord MCP

将您的Discord服务器连接至AI，允许AI创建频道、发布消息、进行互动等。

#### Jupyter MCP

一个用于控制Jupyter Notebook的MCP服务器，能够编写代码并添加Markdown单元格，是数据科学家的理想工具，且免费使用。

#### Blender-MCP

该系统整合了Blender和MCP，包含一个Blender插件和一个MCP服务器。Blender插件在Blender内部创建了一个socket服务器来接收和执行命令，而MCP服务器则连接到这个Blender插件上。要使用Blender MCP，需完成几个步骤：安装Blender软件、更改Claude AI的MCP服务器配置、安装并上传插件文件至Blender。

#### GitHub-MCP

提供仓库管理、文件操作及GitHub API集成的MCP服务器。只需两步即可轻松添加GitHub MCP到Claude AI：生成GitHub个人访问令牌并修改Claude AI的config.json。

#### File System-MCP

基于Go语言实现的用于文件系统操作的MCP服务器，支持读写文件、创建/列出/删除目录、移动文件/目录、搜索文件及获取文件元数据。

#### Docker-MCP

社区开发的MCP服务器，用于管理容器、镜像、卷和网络。

#### WhatsApp MCP

这是最有趣的MCP服务器之一，可以帮助用户发送信息给朋友、加载聊天记录、加载联系人等，尽管集成过程比其他服务器复杂，但值得等待。

#### Puppeteer-MCP

提供浏览器自动化和网页抓取能力的MCP服务器，能控制Google Chrome或Firefox等浏览器，并可自动执行在线购物等任务。

#### SQLite-MCP

包括SQLite或Postgres Lite在内的MCP，专门用于Claude AI的数据库连接，使您可以使用AI与数据库交互。

#### Figma MCP

连接Figma与AI的MCP服务器，为平面设计师带来AI特性。

#### PowerPoint MCP

帮助创建任何类型的定制演示文稿的MCP服务器，仅需提示和其他功能即可实现。

#### Google Calendar (GSuite) MCP
