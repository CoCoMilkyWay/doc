...
QuantML QuantML 2024-07-03 23:00 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247485016&idx=1&sn=17598e7dbdfcb7e908b2a393c5dd28ad&chksm=cf569963ddb5f1546e86508dd89f5ee5ece91b62d918962cd143a80a46ea8a921cb930e39eff#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247485016&idx=1&sn=17598e7dbdfcb7e908b2a393c5dd28ad&chksm=cf569963ddb5f1546e86508dd89f5ee5ece91b62d918962cd143a80a46ea8a921cb930e39eff#rd)

Content

  

> 研报链接：[开源量化评论 | A股市场中如何构造动量因子？(qq.com)](https://mp.weixin.qq.com/s?__biz=MzI1NTYxMjE1Mw==&mid=2247486200&idx=1&sn=e369913f8a2c4bb2eb51ce488d3b3d8c&chksm=ea320f6edd458678f81860346d9762730a2150c6d27d33ae3d2ef974c9213b0c770e139e15b2&scene=21#wechat_redirect)
> 
> https://mp.weixin.qq.com/s?\_\_biz=MzI1NTYxMjE1Mw==&mid=2247486200&idx=1&sn=e369913f8a2c4bb2eb51ce488d3b3d8c&chksm=ea320f6edd458678f81860346d9762730a2150c6d27d33ae3d2ef974c9213b0c770e139e15b2

  

因子定义为：我们选择160交易日作为窗口期（N=160），同时引入切割比例λ，将振幅较低的λ比例交易日的涨跌幅加总，构造得到动量因子A(λ)。

  

由于需要滚动计算，采用pandas计算效率会非常低。本文介绍如何在QuantML-Qlib中如何优雅高效的构建因子。

  

首先需要了解qlib中因子计算的机制，qlib中采用表达式的方式构建因子，定义了一系列算子用于因子计算，并且采用了并行以及memory cache等方法加速因子计算，部分算子支持c以及流式计算，进一步提高了计算效率。  
...
在qlib/data/ops.py中加入切割算子定义：

```
class CutMoM(PairRolling):
```

  

注册之后就可正常调用，调用因子以及因子检验的代码在examples/alphazoo/研报复现/切割动量.ipynb中。

因子在沪深300中表现优异，IC达到了0.1
...
熟悉quantml-qlib中因子构建的方法之后，能够非常高效的大批量构建因子。大批量构建因子的效率相比于传统方法效率提升在百倍以上。而且算子和底层数据的复用，使得因子构建也极其优雅，无需再几十行甚至上百行构建一个因子，现在只需要一行表达式对应一个因子的计算。

Quantml-Qlib代码下载地址见星球
