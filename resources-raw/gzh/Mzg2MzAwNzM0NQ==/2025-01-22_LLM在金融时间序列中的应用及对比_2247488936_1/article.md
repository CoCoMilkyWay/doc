...
QuantML QuantML 2025-01-22 22:24 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247488936&idx=1&sn=6c056b102d118db593532177c2fe8237&chksm=cf9693ba1e33693bdfb7b3b5c5d4e3452b9b5b812cf412953f80111fcc0a0e8db970e7841144#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247488936&idx=1&sn=6c056b102d118db593532177c2fe8237&chksm=cf9693ba1e33693bdfb7b3b5c5d4e3452b9b5b812cf412953f80111fcc0a0e8db970e7841144#rd)

UVA-MLSys的金融时间序列仓库提供了一系列用于金融数据时间序列预测的模型实现，包括DLinear、iTransformer、TimeMixer、PatchTST和TimesNet等传统时间序列模型，以及GPT4TS、CALF和TimeLLM等基于大型语言模型（LLM）的时间序列模型。该仓库还包含了用于训练和测试这些模型的数据集，如美国各州的金融援助数据、IMF货币汇率数据库中的汇率数据、以及纳斯达克数据库中的股票价格和商品价格数据。此外，该仓库还提供了实验结果，展示了在少量训练数据（10%）下的模型表现，以及大型语言模型在零样本学习中的表现。

![](assets/001.png)

项目地址：UVA-MLSys/Financial-Time-Series: Time series models (Transformer, Foundation Models) for Financial data

论文地址：https://arxiv.org/pdf/2410.19025

### 模型结构和特点

本框架的核心在于评估和比较不同类型的模型在金融时间序列预测（FTSF）中的应用，特别是在金融援助（FA）领域的表现。

#### 1\. 模型分类与选择

我们选择了两种主要的模型类别进行比较：

1.  **传统深度学习模型**：这些模型主要基于Transformer架构，近年来在时间序列预测中表现出色。
    
2.  **基于大型语言模型（LLM）的基础模型**：这些模型利用了预训练的LLM，如GPT-2，并通过特定的技术将其应用于时间序列预测。
...
-   **结构**：DLinear是一个简单的线性模型，基于自回归（AR）模型的思想。它通过线性变换将输入序列映射到输出序列。
    
-   **特点**：由于其简单性，DLinear在计算效率上有优势，但在捕捉复杂的时间模式方面表现有限。
    

3.  **iTransformer**：
    

-   **结构**：iTransformer是Transformer架构的变体，采用倒置的注意力机制。它通过改变注意力机制的方向来增强模型对时间序列数据的处理能力。
    
-   **特点**：iTransformer在处理长距离依赖关系方面表现出色，但在某些情况下可能会受到计算复杂度的限制。
    

5.  **TimesNet**：
    

-   **结构**：TimesNet结合了时间卷积网络（TCN）和Transformer的优势。它通过多尺度卷积层来捕捉不同时间尺度上的模式，然后使用Transformer层进行进一步的特征提取和融合。
    
-   **特点**：TimesNet在捕捉多尺度时间模式方面表现优异，但在处理高维数据时可能会遇到内存瓶颈。
    

7.  **PatchTST**：
    

-   **结构**：PatchTST将时间序列数据分割成多个片段（patches），并使用Transformer处理这些片段。它通过局部注意力机制来捕捉片段内部和片段之间的依赖关系。
    
-   **特点**：PatchTST在处理局部时间模式方面表现出色，但在处理全局依赖关系时可能需要更多的计算资源。
    

9.  **TimeMixer**：
    

-   **结构**：TimeMixer采用了一种分解的多尺度混合策略，通过多个卷积层和混合层来捕捉不同时间尺度上的模式。
    
-   **特点**：TimeMixer在处理复杂的时间模式方面表现出色，但在某些情况下可能会受到参数数量的限制。
...
-   **结构**：TimeLLM通过重新编程LLM的能力来处理时间序列数据。它提出了一种“提示作为前缀”的技术，将时间序列数据转换为类似于自然语言的形式，从而利用LLM的预训练知识。
    
-   **特点**：TimeLLM在少样本和零样本学习场景下表现出色，但在处理极长的时间序列时可能会遇到计算瓶颈。
    

3.  **CALF**：
    

-   **结构**：CALF（Cross-modal Attention for Language and Forecasting）提出了一种跨模态注意力机制，通过将时间序列数据与文本数据进行联合处理来减少分布差异。它使用了一种新颖的微调框架来增强模型对时间序列数据的理解。
    
-   **特点**：CALF在处理多模态数据方面表现出色，但在某些情况下可能会受到数据预处理复杂度的限制。
    

5.  **GPT4TS**：
    

-   **结构**：GPT4TS（One Fits All）利用预训练的GPT模型进行时间序列预测，而不改变其重要层。它通过将时间序列数据转换为类似于自然语言的形式来利用GPT的预训练知识。
    
-   **特点**：GPT4TS在零样本学习场景下表现最佳，但在处理极长的时间序列时可能会遇到计算瓶颈。
...
-   **计算效率**：传统模型如DLinear和iTransformer在计算效率上有优势，适合处理大规模数据。
    
-   **多尺度处理**：TimesNet和TimeMixer通过多尺度卷积和混合策略，能够有效捕捉不同时间尺度上的模式。
    
-   **局部与全局依赖**：PatchTST通过局部注意力机制，能够有效处理局部时间模式，但在全局依赖关系处理上可能需要更多资源。
    

##### 2.2 基于LLM的基础模型的特点

-   **少样本与零样本学习**：TimeLLM和GPT4TS在少样本和零样本学习场景下表现出色，能够利用预训练知识进行有效预测。
    
-   **跨模态处理**：CALF通过跨模态注意力机制，能够有效处理多模态数据，增强模型对时间序列数据的理解。
    
-   **预训练优势**：基于LLM的模型能够利用大规模预训练数据，捕捉复杂的语言和时间模式。
    

#### 3\. 实验结果与比较

在少样本学习场景下，TimeLLM和PatchTST表现最佳，TimeLLM在多个数据集上取得了最佳或第二佳的结果，而PatchTST紧随其后。在零样本学习场景下，GPT4TS表现最佳，但由于缺乏微调，其整体性能不如少样本学习场景。
