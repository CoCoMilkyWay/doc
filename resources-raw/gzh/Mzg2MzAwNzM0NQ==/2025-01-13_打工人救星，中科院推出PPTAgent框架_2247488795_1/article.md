# 打工人救星，中科院推出PPTAgent框架

QuantML QuantML 2025-01-13 22:47 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247488795&idx=1&sn=dbc480b1f12428dc3b37e915b2222b4b&chksm=cfb136050fbe6a1ba8a37fa394bc7d2b176f923e9aac6f35db67d8e9913cd8bdee76ea234ce9#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247488795&idx=1&sn=dbc480b1f12428dc3b37e915b2222b4b&chksm=cfb136050fbe6a1ba8a37fa394bc7d2b176f923e9aac6f35db67d8e9913cd8bdee76ea234ce9#rd)

PPTAgent 是中科院推出的自动生成演示文稿框架，借鉴了人类制作演示文稿的方式，采用了一种**基于编辑的两阶段方法**，而非传统的端到端文本生成模式。这种方法更贴近人类的思维过程，能够更好地处理演示文稿的复杂性和交互性。![](assets/001.jpg)

### **两阶段工作流程：分析与生成**

#### **第一阶段：演示文稿分析**

-   **幻灯片聚类**：PPTAgent 将参考演示文稿中的幻灯片根据其功能进行分类，例如：
    

-   **结构性幻灯片**：如开场幻灯片、目录幻灯片等，使用大型语言模型 (LLM) 推断其功能并分组。
    
-   **内容性幻灯片**：如包含要点或图表的幻灯片，采用基于图像相似性的层次聚类方法进行分组。
    

-   **模式提取**：对聚类后的幻灯片进行内容模式提取，识别每个幻灯片的语义信息，包括：
...
-   **大纲生成**：根据参考演示文稿和输入文档，PPTAgent 生成一个结构化的大纲，指定每个新幻灯片的参考幻灯片、相关文档部分以及标题和描述。
    
-   **幻灯片生成**：基于大纲，PPTAgent 迭代地编辑参考幻灯片以生成新幻灯片：
    

-   **编辑操作**：使用五个专门的 API 来编辑、删除和复制文本元素，以及编辑和删除视觉元素。
    
-   **代码交互**：将幻灯片从原始 XML 格式转换为 HTML 表示形式，以便 LLM 更好地理解幻灯片结构，并生成可执行的编辑操作。
    
-   **自我修正机制**：在 REPL 环境中执行编辑操作，系统检测执行过程中的错误并提供实时反馈以进行自我修正。
    

### **全面评估框架：PPTEval**

为了解决现有自动化评估指标的局限性，PPTAgent 引入了 **PPTEval**，一个多维度的评估框架，从以下三个方面评估演示文稿质量：

#### **内容 (Content)**

-   **信息量**：评估幻灯片上呈现的信息量，避免信息过多或过少。
    
-   **文本清晰度和质量**：检查文本内容的语法错误、表达清晰度以及信息传达的有效性。
    
-   **视觉内容支持**：评估图像与文本内容的关联性，以及图像对信息的增强作用。
    

#### **设计 (Design)**

-   **颜色方案**：评估颜色对比度、色彩搭配的和谐性以及是否突出内容。
    
-   **视觉元素**：评估几何形状、图标等视觉元素的使用，以及其对设计的增强作用。
    
-   **整体设计**：评估幻灯片是否符合基本设计原则，如避免元素重叠、确保设计不干扰内容传达。
    

#### **连贯性 (Coherence)**

-   **逻辑结构**：评估演示文稿的逻辑结构是否合理，主题是否清晰，观点之间的过渡是否流畅。
    
-   **背景信息**：评估是否包含足够的基本背景信息，如演讲者信息、致谢或结论。
    

### **显著优势**

-   **内容修改为主，避免复杂样式操作**：PPTAgent 专注于修改内容，而不是处理复杂的样式操作，从而提高了生成效率。
    
-   **简洁的 API 设计**：PPTAgent 提供了易于使用的 API，使 LLM 能够轻松执行编辑任务。
    
-   **代码交互模块**：增强了 LLM 对幻灯片的理解，并提供了自我修正的机会，从而能够更稳健地生成准确的编辑操作。
    
-   **继承现有演示文稿的样式属性**：通过基于编辑的方法，PPTAgent 使新幻灯片能够继承现有演示文稿中设计良好的样式属性，从而提高整体设计质量。
    
-   **与开源 LLM 性能相当**：实验结果表明，Qwen2.5 在与 PPTAgent 结合使用时，其性能可与 GPT-4o 相媲美，展示了开源 LLM 在演示文稿生成领域的巨大潜力。
