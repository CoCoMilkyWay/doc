# ARMD：基于自回归滑动扩散模型的时间序列预测

QuantML QuantML 2025-01-20 20:57 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247488868&idx=1&sn=aa76019420bf31adc423ce36d629f446&chksm=cf0c732d8ff15b7db8a0641411768f0e48992a51a53dd7c4f8a4cd36a0ad28c90d045f1ac27a#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247488868&idx=1&sn=aa76019420bf31adc423ce36d629f446&chksm=cf0c732d8ff15b7db8a0641411768f0e48992a51a53dd7c4f8a4cd36a0ad28c90d045f1ac27a#rd)

**核心思想：基于时间序列连续演化的扩散模型**

ARMD（Auto-Regressive Moving Diffusion）模型受经典时间序列理论（ARMA）的启发，将时间序列的演化过程视为一个扩散过程，并将其与传统的扩散模型相结合，提出了一种全新的连续序列扩散模型用于时间序列预测（TSF）。

项目地址：daxin007/ARMD: \[AAAI 2025\] Official Implementation of "Auto-Regressive Moving Diffusion Models for Time Series Forecasting"

## 1\. ARMD 代码结构

ARMD（Auto-Regressive Moving Diffusion Models for Time Series Forecasting）的框架结构主要包括以下几个关键组成部分：

![](assets/001.png)

1.  **配置文件（Config）**：该目录包含了模型训练和测试所需的配置文件，以`.yaml`格式存储，用于指定模型的超参数、训练选项和数据预处理设置等。
    
2.  **数据文件（Data）**：这部分包含了数据预处理、加载数据增强代码。它还指定了数据集的存储路径，以及如何从原始数据集中提取时间序列数据。
    
3.  **模型文件（Models/autoregressive\_diffusion）**：这是ARMD框架的核心，包含了自回归移动扩散模型的实现代码。ARMD 模型主要用于时间序列预测任务，它通过定义一个自回归过程来模拟数据的生成过程。模型使用了两种噪声调度方法：线性调度和余弦调度，分别通过 linear\_beta\_schedule 和 cosine\_beta\_schedule 函数实现。这些调度方法定义了扩散过程中每一步的噪声水平。
    
4.  **实用工具文件（Utils）**：提供了一系列实用的辅助函数和类，用于辅助模型训练、评估和数据处理等操作。
    
5.  **引擎文件（engine）**：包含了模型训练，logger模块，以及学习率调整函数，主要定义了两个用于调整学习率的类：ReduceLROnPlateauWithWarmup和CosineAnnealingLRWithWarmup，它们分别用于在验证集上的性能停滞时减少学习率，以及在训练过程中根据余弦退火算法调整学习率，并且都支持学习率的暖启（warmup）功能。
    
6.  **主程序文件（main.py）**：作为项目的入口点，负于初始化模型、加载数据、执行训练和评估等操作。通过运行`main.py`可以启动整个ARMD框架。
...
-   与传统扩散模型将噪声作为最终状态不同，ARMD 将历史序列作为最终状态，体现了时间序列的连续性。
    

-   **中间状态生成：**
    

-   ARMD 采用基于滑动的方法生成中间状态 ，即通过将未来序列 逐步滑动 步来逼近历史序列 。
    
-   这种方法利用了时间序列的连续性特征，每个中间状态都对应时间序列演化过程中的一个中间序列。
    
-   具体公式为：
    
    其中 表示将序列窗口 向历史序列方向滑动 步。
    

-   **与 DDPM 的联系：**
    

-   ARMD 的前向扩散过程可以类比为 DDPM 的前向扩散过程，但 ARMD 的中间状态是确定性的，而非通过添加噪声生成。
    
-   ARMD 将滑动操作与 DDPM 的噪声添加过程对应起来，将滑动后的序列表示为：
    
    其中 表示从序列 到 的演化趋势，类似于 DDPM 中添加的噪声。
...
-   从历史序列 出发，利用反向去噪过程迭代生成未来序列 ，从而实现时间序列预测。
    

-   **线性逆演化网络：**
    

-   ARMD 采用基于线性的逆演化网络 ，该网络通过估计中间状态与目标序列之间的距离 ，并根据时间步长 自适应地调整中间状态和距离预测的权重，最终生成对目标序列的预测 。
    
-   具体公式为：
    
    其中 是随时间步长 递减的权重系数， 是用于平衡距离预测 和输入 的超参数。
    

-   **预测演化趋势：**
    

-   在获得预测结果 后，可以计算预测的演化趋势 ，并将其与真实值 进行比较，用于模型训练。
...
-   通过滑动操作生成中间状态，并计算演化趋势 。
    
-   利用逆演化网络生成预测结果 和预测的演化趋势 。
...
-   采样过程采用 DDIM 的采样方法，但去除了噪声项 ，因为 ARMD 的演化过程是确定性的。
...
-   ARMD 在 7 个常用基准数据集上进行了广泛评估，并与 5 种先进的基于扩散的 TSF 模型进行了比较。
    
-   在 14 种实验设置中，ARMD 在 12 种设置中取得了最优结果，表现出卓越的预测性能。
    
-   例如，在 ETTm1 数据集上，ARMD 的 MSE 和 MAE 分别比第二好的模型 D3VAE 降低了 47.7% 和 30.1%。
    
-   在 Stock 数据集上，ARMD 的 MSE 和 MAE 分别比第二好的模型 TSDiff 降低了 28.8% 和 26.3%。
    

-   **与其他 TSF 模型比较：**
    

-   ARMD 在多变量 TSF 任务中始终优于其他先进模型。
    
-   在 10 种实验设置中，ARMD 获得了最多的最优结果次数（7 次），进一步巩固了其作为领先扩散 TSF 模型的地位。
...
-   ARMA 理论启发的滑动操作使 ARMD 能够更好地捕捉时间序列的连续演化过程，避免了传统方法中噪声引入带来的不连续性。
    

-   **信息利用：**
    

-   通过滑动操作，ARMD 能够利用时间序列演化的中间信息，而传统方法直接将真实序列扩散为白噪声，忽略了这些有价值的信息。
    

-   **稳定性与准确性：**
    

-   逆演化网络采用基于距离的预测方法，并结合时间步长进行自适应调整，提高了预测的稳定性和准确性。
...
-   ARMD 的线性逆演化网络结构简化了模型复杂度，训练和推理速度更快。
    

## 总结

ARMD 是一种创新的基于时间序列连续演化的扩散模型，通过滑动操作和线性逆演化网络，实现了更准确、更稳定的预测性能。其在多个基准数据集上的优异表现证明了其在时间序列预测任务中的巨大潜力。
