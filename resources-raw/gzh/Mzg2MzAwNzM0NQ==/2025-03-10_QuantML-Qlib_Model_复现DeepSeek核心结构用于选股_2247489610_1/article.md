# QuantML-Qlib Model | 复现DeepSeek核心结构用于选股

QuantML QuantML 2025-03-10 22:04 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489610&idx=1&sn=3f76a4d98a488188df169085bd427d86&chksm=cfa895c940ee2c917fef17bdc935a3655dafb9cecafcdf517585e286e93fb88a8db54cb77f20#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489610&idx=1&sn=3f76a4d98a488188df169085bd427d86&chksm=cfa895c940ee2c917fef17bdc935a3655dafb9cecafcdf517585e286e93fb88a8db54cb77f20#rd)

### 

在之前的文章中，我们分别介绍了DeepSeek核心结构MOE以及MLA，以及如何将其融入QuantML-Qlib框架用于选股。
...
本文我们将MOE以及MLA结构进行结合：一方面采用改进型多头注意力机制（MLA），通过智能参数压缩和混合位置编码策略，使模型能更精准捕捉市场波动规律；另一方面引入专家决策系统（MoE），让多个专业子模型自主竞争，仅激活最优专家进行预测，大幅提升计算效率。该架构特别针对金融数据特性进行优化，包括处理突发数据缺失的填充策略、控制参数更新幅度的梯度裁剪技术，以及聚焦最新市场状态的预测机制。

## 核心架构解析

### 2.1 多层级特征处理框架

模型采用三级处理结构：基础特征层通过线性变换将原始特征映射到高维空间，位置编码层采用经典的正弦/余弦位置函数捕获绝对时序信息，核心处理层由多个MLA模块堆叠构成。特别设计的维度转换机制在保持批处理效率的同时，适配了金融数据的时序特性。

### 2.2 改进型注意力机制

MLA模块的创新体现在三个维度：

-   **参数效率优化**：引入LoRA（Low-Rank Adaptation）技术，对Key/Value矩阵进行低秩分解（kv\_lora\_rank=4），在保持表征能力的同时减少30%的参数量
    
-   **混合位置编码**：创新性地将旋转位置编码（RoPE）与无位置编码（NoPE）头相结合，其中RoPE头（rope\_head\_dim=8）捕获相对位置关系，NoPE头（nope\_head\_dim=4）保留绝对位置信息
    
-   **动态注意力聚焦**：通过潜在空间投影实现注意力权重的自适应调整，相比标准点积注意力，在金融波动数据上表现出更强的模式捕捉能力
    

### 2.3 专家决策系统

末端的MoE层采用"软选择-硬执行"机制：

`class MixtureOfExperts(nn.Module):       def forward(self, x):           gate_output = F.softmax(self.gates(x), dim=1)  # 软选择           max_expert_index = gate_output.argmax(dim=1)   # 硬执行           # 仅计算被选专家的前向传播           chosen_experts_outputs = expert_outputs[..., max_expert_index]` 

## 代码结构

核心代码（examples/benchmarks/DeepSeek/pytorch\_transformer\_moe\_mla.py）结构如下：

`pytorch_transformer_moe_mla.py   ├── MixtureOfExperts(nn.Module)          # 混合专家系统模块   ├── PositionalEncoding(nn.Module)        # 标准Transformer位置编码   ├── TransformerMoEMLA(nn.Module)         # 核心模型：结合MLA和MoE的Transformer   │   ├── MultiHeadLatentAttention         # 来自MLA的改进注意力机制   │   └── MixtureOfExperts                 # 最后的预测层   └── TransformerMoEMLAModel(Model)        # 完整的训练框架       ├── 训练配置（优化器/损失函数）       ├── fit()                            # 训练流程       └── predict()                        # 预测流程   `

代码已加入QuantML-Qlib，运行examples/benchmarks/DeepSeek/run.py即可启动模型。
...
QuantML-Qlib已包含40+SOTA模型，欢迎共同测试交流
