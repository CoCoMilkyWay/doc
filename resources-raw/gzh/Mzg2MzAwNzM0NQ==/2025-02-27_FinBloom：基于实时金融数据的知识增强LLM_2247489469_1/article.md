# FinBloom：基于实时金融数据的知识增强LLM

QuantML QuantML 2025-02-27 21:46 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489469&idx=1&sn=56d5df9f8418020bc78f825c856cf69a&chksm=cf87f0cc6bbad60a23e70c913ed18ff0b332c0acebb98dc62a089fb3b2fe58416a94796ce969#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489469&idx=1&sn=56d5df9f8418020bc78f825c856cf69a&chksm=cf87f0cc6bbad60a23e70c913ed18ff0b332c0acebb98dc62a089fb3b2fe58416a94796ce969#rd)

### 

#### 1\. 引言

大型语言模型（LLMs）擅长生成类人回答，但在需要实时信息访问的交互任务中常常表现不佳。这一局限性在金融领域尤为明显，因为金融模型必须访问最新信息，如新闻或价格变动，以支持决策。为了应对这一挑战，我们提出了FinBloom，一个知识增强的LLM方法，用于处理使用实时文本和表格数据的金融查询。

我们的贡献主要体现在三个方面：

1.  **开发了金融上下文数据集**：包含超过50,000个金融查询及其所需的上下文。
    
2.  **训练了FinBloom 7B**：一个定制的70亿参数LLM，使用来自路透社和德意志新闻社的1400万篇金融新闻文章，以及1200万份证券交易委员会（SEC）文件进行训练。
    
3.  **微调FinBloom 7B**：使用金融上下文数据集，将其打造成一个金融代理。该代理生成相关的金融上下文，实现高效的实时数据检索，以回答用户查询。
    

通过减少延迟并消除用户手动提供准确数据的需要，我们的方法显著增强了LLMs处理动态金融任务的能力。这种方法使实时金融决策、算法交易和其他相关任务更加高效，在高速度数据流环境中具有重要价值。

![](assets/001.png)

#### 2\. 文献综述

本节回顾了LLMs的知识增强、金融领域的LLMs贡献以及用于金融文本挖掘的流行金融数据集。我们还讨论了一些流行的知识增强方法在金融领域的局限性。

##### 2.1 LLM的知识增强

传统的LLM知识增强方法通常涉及对模型参数进行微调，这需要使用特定于下游任务的定制数据集对LLM的某些或所有层进行训练。然而，这些策略计算成本高，特别是当数据动态变化且模型需要频繁更新时。知识增强的基本原理在于使LLMs能够基于相关外部数据进行推理和上下文理解，而不是将它们视为静态知识的存储库。

近年来，出现了许多可行的替代方法，提供了对传统微调方法的潜在改进。例如，Wei等人（2022）引入了“思维链”（CoT），这是一种用于LLMs的少样本提示技术。CoT在提示中采用顺序示例，包括任务输入和中间步骤，以推动大模型的上下文理解和推理。

另一种方法是为LLMs配备检索模块，使其能够从数据库中访问与用户查询相关的信息。Lewis等人（2020）引入了检索增强生成（RAG）模型，他们将预训练的检索模块与预训练的seq2seq模型集成在一起。检索器提供基于输入的潜在文档，然后seq2seq模型基于这些潜在文档和输入生成输出。

尽管在LLM的知识增强方面取得了显著进展，但在实时处理高频流动的大量数据（如金融数据）方面，仍然面临挑战。

##### 2.2 传统RAG模型在检索金融数据方面的局限性

RAG系统在结合LLMs和信息检索技术方面具有巨大潜力，但它们在复杂和受监管的领域（如金融）仍然面临一些局限性。

1.  **金融数据表示的困难**：金融数据主要存储在结构化表格格式中，如电子表格和数据库。这些格式强调数据点的组织、分析和操作。虽然这对于金融任务有利，但对于RAG模型来说却是一个难题。表格缺乏RAG模型可以通过其自然语言处理能力轻易利用的内在关系和推理模式。将庞大的金融数据集转换为RAG模型的文本表示将是不切实际且低效的。
    
2.  **检索器准确性的挑战**：RAG模型的核心组件——检索器，负责从知识库中获取相关信息以回答用户查询。当处理金融数据时，检索器面临一个独特的挑战。金融查询通常涉及特定的金融指标、时间范围和金融公司。检索器在其检索过程中必须高度选择，确保只收集回答查询所需的精确数据点。
    
3.  **无法处理高速度、大量数据**：RAG模型通常依赖于静态数据库和预训练的检索机制，难以跟上金融数据快速涌入和动态特性的步伐。此外，金融环境中对频繁更新和高延迟响应的需求可能会使传统的RAG架构不堪重负，导致输出延迟和可能过时。
    

##### 2.3 金融数据集回顾

金融语言处理领域见证了注释数据集的显著发展，每个数据集在领域内都有不同的用途。我们通过表1突出了一些在金融领域广泛用于模型训练的例子。

![](assets/002.png)

#### 3\. 金融上下文数据集

对于任何给定的用户查询，金融代理应提供相关的上下文，包括数字和文本数据，这些数据可以附加到用户查询中以供进一步处理。本节讨论了本文的第一个核心贡献，即金融上下文数据集，由50,000个用户查询及其相应的上下文样本组成。

为了有效地训练模型，数据集应在句子结构上具有多样性，并在金融指标和公司方面具有全面性。具体来说，数据集应包括各种形式的查询，如简单的“是/否”问题、比较问题和开放式问题。这将帮助模型学习理解用户提出查询的不同方式。此外，数据集应包括大量的金融指标和公司，以它们许多可能的名称和其他通常的指代方式出现。这种数据集的多样性将使我们的模型能够很好地推广到各种用户查询，确保它能够准确识别所需的信息。

![](assets/003.png)

为了构建这个由自然语言用户查询及其相应的结构化数据请求组成的数据集，我们采用了多方法查询采样方法。第一种方法涉及收集通过技术渠道（如电子邮件或基于网络的表单等）发送给经纪公司的零售客户的查询。第二种方法涉及与10位金融顾问进行互动和咨询，了解零售投资者通常有哪些查询。第三种方法涉及对20位零售投资者进行访谈，了解他们在投资特定股票或形成投资组合之前通常会想到的常见问题。基于三种数据收集方法，我们创建了5000个查询模板，其中4000个模板是从技术渠道提取的，500个是与金融顾问共同创建的，另外500个是与零售投资者共同创建的。这种方法确保了广泛的金融查询的整合，从而形成了一个多样化的数据集。为了将数据集扩展到50,000个查询，我们通过随机改变每个模板中的各个方面来放大这5000个模板。

#### 4\. FinBloom 7B：一个用于金融领域的大型语言模型

我们的方法需要金融代理来形成金融数据的结构化请求，以便数据模块能够理解和响应。用户的查询通常以自然语言形式出现，这意味着它可能不会明确包含回答它所需的所有必要信息，如最近的数字和新闻。因此，代理需要分析用户的查询，并智能地识别回答查询所需的必要的金融指标，以及任何相关的指标和新闻。它还需要识别需要数据的公司以及所需的数据时间范围。最后，它需要确保这些要求以数据请求函数的形式呈现，以便数据模块能够解释。手动指定一组硬规则来处理所有可能的用户查询并执行这些任务将是非常繁琐和复杂的。因此，一个更好的方法是使用大量自然语言用户查询的数据集来训练文本到文本生成模型，以生成包含回答用户查询所需的所有信息的相应结构化查询格式。

![](assets/004.png)

为了创建金融LLM，我们选择一个具有强大通用语言理解能力并具有在金融领域特定专业化的潜力的基础模型。我们首先选择Bloom 7B参数模型作为基础，利用其强大的语言能力作为微调的基础。为了使模型具备金融专业知识，我们使用了从路透社和德意志新闻社采购的大量金融新闻文章数据集。该数据集涵盖了广泛的金融主题，包括市场趋势、经济指标、企业发展和监管变化。

为了评估FinBloom 7B在各种金融任务中的能力，我们采用了FinBen基准测试（Xie等人，2024）。这个全面的基准测试由多个数据集组成，每个数据集评估特定的技能，如信息提取、文本分析、问答、文本生成、预测、风险管理和决策。通过对这个严格的评估，我们的目标是衡量模型在处理各种现实世界金融应用中的有效性。

#### 5\. 用于上下文生成和数据检索的金融代理

本节描述了金融代理的结构和功能，解释了它如何与数据模块一起工作，以提供相关的上下文供LLM回答用户查询。金融代理是通过对FinBloom 7B模型进行微调而开发的，使用第3节中详细说明的自定义数据集。这种微调过程采用了参数高效的提示微调方法。

![](assets/005.png)

数据模块作为金融数据的存储库，包括表格财务信息以及与各种公司、部门和行业相关的实时新闻数据。它以动态方式运行，定期吸收和整合新数据，以确保它提供的信息保持最新和最新。该模块旨在处理金融数据请求。当接收到来自金融代理的数据请求时，数据模块构建一个包含相关财务数据的数据框，符合数据请求的规格。同时，该模块根据新闻项目与用户查询的语义匹配，提取对完整查询解析有益的相关新闻数据。我们使用基于RoBERTa的语义搜索来检测请求查询与新闻标题之间的相似性。

在接收包含公司/行业名称、财务指标和特定日期范围的数据请求时，该模块识别金融数据可用的最近日期范围。例如，净收入是按季度报告的，因此模块呈现请求中日期范围最近的可用的季度净收入。另一方面，股价是以更高的频率报告的，因此模块呈现请求中查询的确切日期范围的股价数据。如果请求中没有日期/日期范围，则选择数据可用的最新日期范围。然后，该模块检索该日期范围的相应指标数据，并另外传达该指标数据提供的相应日期范围和频率。因此，当这些数据与初始用户输入整合并提供给LLM时，LLM接收到有关指标及其相关日期范围的准确信息。

#### 6\. 结论

本研究强调了知识增强在LLMs中的作用，特别是对于需要实时信息访问的交互任务。首先，我们开发了金融上下文数据集，其中包括超过50,000个金融查询及其解决所需的上下文。在此基础上，我们使用来自路透社和德意志新闻社（DPA）的1400万篇金融新闻文章和1200万份SEC文件对FinBloom 7B进行了训练。随后，我们使用金融上下文数据集对FinBloom 7B进行了微调，创建了一个金融代理。该代理能够为用户查询生成所需的上下文，并有效地从结构化数据模块中检索它，以解决金融查询。
