...
QuantML QuantML 2025-03-16 18:32 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489693&idx=1&sn=91ef6e76764bd490ea183b1452089e51&chksm=cfabf57f6c4469df9e09d95fb82e6220291ae89d9ad97a07c9b2303c63a49473b0322dda4a62#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489693&idx=1&sn=91ef6e76764bd490ea183b1452089e51&chksm=cfabf57f6c4469df9e09d95fb82e6220291ae89d9ad97a07c9b2303c63a49473b0322dda4a62#rd)

### 

近日何恺明与LeCun联合发布了一项颠覆性的新技术——动态Tanh（DyT）。这项技术以其简单的实现方式和卓越的性能优势，引发了业内的广泛关注和讨论。DyT旨在替代传统的归一化层，在不减少性能的同时显著加速深度学习模型的训练过程。

归一化层在现代神经网络中扮演着关键角色，其主要目的是加速模型收敛、提高性能、增强泛化能力以及缓解梯度消失/爆炸问题。通过调整输入数据的分布，归一化层使得不同层之间的数据分布更加稳定，从而提升训练效率和模型表现。然而，归一化层也存在一些挑战，例如计算开销大、对 batch size 和超参数敏感，以及在推理阶段可能存在的不一致性。

DyT 模型是一种旨在替代 Transformer 中传统归一化层（如 Layer Normalization，LN）的创新方法。 其核心操作是动态 Tanh 函数，通过可学习的标量参数  动态调整输入的缩放比例，并利用  函数对输入进行非线性变换，模拟 LN 层对极端值的压缩效果。DyT 层可以直接替换 Transformer 中的 LN 或 RMSNorm 层，无需对网络架构进行其他修改，并且在视觉识别、语音处理、图像生成和语言模型等任务中表现出与 LN 或 RMSNorm 相当甚至更好的性能，同时训练稳定性强，通常无需额外调整超参数。
...
`pytorch_dyt_ts.py   ├── TransformerModel(Model)              # 模型训练框架   ├── PositionalEncoding(nn.Module)        # 标准位置编码   ├── DynamicTanh(nn.Module)               # 核心创新层   ├── DyTTransformerEncoderLayer(nn.Module) # 自定义编码层   ├── DyTTransformerEncoder(nn.Module)     # 编码器堆叠   └── Transformer(nn.Module)               # 完整模型结构   `
...
`class DynamicTanh(nn.Module):       def __init__(self, normalized_shape, alpha_init_value=0.5):           self.alpha = nn.Parameter(torch.ones(1) * alpha_init_value)  # 可学习缩放因子           self.weight = nn.Parameter(torch.ones(normalized_shape))     # 逐特征权重           self.bias = nn.Parameter(torch.zeros(normalized_shape))      # 逐特征偏置          def forward(self, x):           x = torch.tanh(self.alpha * x)  # 动态缩放后的Tanh           return x * self.weight + self.bias  # 特征级校准   `

-   **α参数**：可学习的缩放因子，自动调节激活函数饱和区
    
-   **权重/偏置**：特征维度的自适应校准，替代传统LayerNorm
...
运行examples/benchmarks/DyT/run.py即可将DyT用于选股模型，提升Transformer模型训练效率以及训练稳定性。
...
代码见星球，加入QuantML星球，与750+专业人士一起交流学习，限时发放一波优惠：
