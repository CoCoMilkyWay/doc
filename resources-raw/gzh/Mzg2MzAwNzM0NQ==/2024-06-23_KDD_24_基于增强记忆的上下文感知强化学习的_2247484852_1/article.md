...
QuantML QuantML 2024-06-23 20:47 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247484852&idx=1&sn=612490637ac96cfa86d829e9ed665c2b&chksm=cf0b13314a2a0871bfbc5f310405af03f59b44a7232c40d83674ac72e41be368f07bec0eac7a#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247484852&idx=1&sn=612490637ac96cfa86d829e9ed665c2b&chksm=cf0b13314a2a0871bfbc5f310405af03f59b44a7232c40d83674ac72e41be368f07bec0eac7a#rd)

Content

### **摘要 (Abstract)**

论文介绍了一种新的增强记忆的上下文感知强化学习方法（MacroHFT），用于高频交易（HFT）。高频交易在加密货币市场占据主导地位，强化学习（RL）因其处理高维金融数据和复杂顺序决策问题的能力而成为一种有吸引力的方法。现有RL方法存在缺陷，如过度拟合和单一代理的投资决策可能因市场快速变化而导致重大损失。MacroHFT通过两个训练阶段解决这些问题：首先训练多种子代理，每种代理根据市场趋势和波动性调整交易策略；然后训练一个超代理，利用记忆机制混合这些子代理的决策，以应对市场快速变化。

  

### **第1章 引言 (Introduction)**

引言部分主要介绍了金融市场的广阔和复杂性，尤其是加密货币市场因其高波动性而受到投资者的特别青睐。文中强调了高频交易（HFT）在加密货币市场中的重要性，并指出了传统量化交易策略和基于强化学习（RL）的方法在处理高维金融数据和复杂决策问题上的潜力。同时，作者也指出了现有RL方法在HFT中的局限性，如过度拟合和在极端市场条件下的决策偏差，这些局限性可能会限制策略的有效性和盈利能力。最后，引言部分提出了MacroHFT这一新的研究方向，旨在通过引入记忆增强和上下文感知的RL方法来克服这些挑战，以期在分钟级别的加密货币交易任务中实现更好的性能。  

  

### **第2章 相关工作 (Related Works)**

-   传统金融方法：基于历史价格和交易量信息可以预测未来市场条件的假设，技术分析在传统金融交易中得到了广泛应用。量化交易者设计了数百万种技术指标作为交易执行的信号，例如不平衡量（Imbalance Volume, IV）和移动平均收敛发散（Moving Average Convergence Divergence, MACD）。然而，这些基于技术指标的传统方法在非平稳市场（如加密货币市场）中容易产生错误的交易信号，导致性能不佳。
    

![](assets/001.png)

-   基于RL的方法：除了传统金融方法，基于强化学习的交易方法在量化交易领域也变得越来越受欢迎。直接应用标准的深度RL算法（如Deep-Q Network, DQN和Proximal Policy Optimization, PPO）以及其他增强技术（如CDQNRP和CLSTM-PPO）已经被探索。这些方法通过增强状态表示、增加稳定性或改善风险管理能力来提高交易策略的性能。此外，为了提高在包含不同市场动态的长期交易视野中的适应能力，分层强化学习（Hierarchical Reinforcement Learning, HRL）结构也被应用于量化交易。
    
-   分层RL方法的挑战：尽管分层RL方法在某些情况下表现出色，但它们在不同程度上遭受过度拟合问题的困扰，并且在基于金融背景进行有效策略调整方面存在困难。例如，MetaTrader和EarnHFT在每个时间戳只选择一个代理进行交易，通常导致决策执行偏向一方，高度有偏见。
    
-   MacroHFT的创新之处：作者提出MacroHFT，这是第一个分层RL框架，它不仅将宏观市场信息作为上下文以协助交易决策，而且还通过使用多个标准分解市场，提供混合策略，利用子代理的专业化能力，而不是选择单个代理。
    

###   

### **第3章 预备知识 (Preliminaries)**

-   金融定义：首先，论文详细定义了高频交易（HFT）中常用的金融术语，例如限价单（Limit Order）、限价订单簿（Limit Order Book, LOB）、开盘-最高-最低-收盘-成交量（Open-High-Low-Close-Volume, OHLCV）以及技术指标（Technical Indicators）。这些定义帮助读者理解交易的基本构件和市场数据的结构。
    
-   持仓和净值：介绍了持仓（Position）和净值（Net Value）的概念，这些是衡量交易者在特定时间持有的加密货币数量和其市场价值的关键指标。
    
-   MDP公式：将加密货币的高频交易问题表述为马尔可夫决策过程（MDP），这是一个数学框架，用于建模在不确定环境中的顺序决策问题。MDP由状态集合、行动集合、状态转移函数、奖励函数和折扣因子组成。
    
-   分层MDP结构：论文提出了一种与以往工作不同的分层MDP结构，专注于解决分钟级别的高频交易问题。这种结构允许在不同时间尺度上制定和执行交易策略，以适应市场的快速变化。
    
-   低级和高级状态与行动：详细描述了低级MDP和高级MDP的状态和行动。低级状态包括单状态特征、低级上下文特征和持仓状态，而高级状态在此基础上增加了高级上下文特征。行动则是基于这些状态做出的交易决策。
    
-   奖励函数：定义了低级和高级MDP的奖励函数，这些函数衡量了在特定状态下采取特定行动的即时回报，这对于训练RL代理来说是至关重要的。
    

  

### **第4章 MacroHFT方法 (MACROHFT)**

### 本章详细介绍了MacroHFT方法的工作原理和实现细节：

![](assets/002.png)

### **4.1 市场分解（Market Decomposition）**

-   论文提出了一种基于趋势和波动性指标的市场分解方法。首先，将市场数据分割成固定长度的序列数据块，然后为每个数据块分配趋势和波动性标签。
    
-   使用低通滤波器消除噪声，并通过线性回归模型确定市场趋势。波动性则是通过计算每个数据块的平均波动来确定。
    
-   数据块根据趋势和波动性的分位数被分为六个子集，代表不同的市场条件（如牛市、熊市、波动大的市场等）。
    

### **4.2 低级策略优化与条件适应（Low-Level Policy Optimization with Conditional Adaptation）**

-   为了解决过度拟合问题，并使策略网络对市场动态更加敏感，论文提出了一种条件适应方法。
    
-   使用双深度Q网络（DDQN）和对偶网络架构作为基础，结合上下文特征和当前持仓位置作为额外的条件输入。
    
-   通过全连接层和位置嵌入层提取状态特征，然后利用自适应层归一化和缩放/偏移向量调整隐藏状态，以适应不同的市场条件。
    

### **4.3 超代理策略优化与记忆增强（Meta-Policy Optimization with Memory Augmentation）**

-   在学习了应对不同市场条件的多样化策略后，论文进一步训练了一个超代理，它考虑所有子代理的决策，并输出一个能够灵活应对市场动态变化的高级策略。
    
-   超代理使用softmax权重向量来混合子代理的决策，并通过条件适配器根据市场趋势和波动性调整权重。
    
-   引入了记忆模块来增强超代理在面对加密货币市场急剧波动时的决策能力，记忆模块基于存储的关键向量进行查询和更新，以保持与当前决策最相关的经验。
    
-   MacroHFT的工作流程包括两个RL训练阶段：第一阶段是使用条件RL方法训练多种子代理，以应对不同的市场动态；第二阶段是训练超代理，以提供元策略，充分利用基于最近市场背景的多样化低级策略。
    
-   论文展示了MacroHFT在不同非平稳加密货币市场上的盈利能力，包括在牛市、熊市和波动市场中的表现。
    
-   通过图表展示了MacroHFT与其他基线方法相比的性能，证明了其在分钟级交易任务中的优越性。
...
### 本章详细描述了对MacroHFT方法进行评估的实验设置、使用的评估指标、基线比较以及实验结果

### **5.1 数据集 (Datasets)**

-   实验在四种加密货币市场上进行，包括BTCUSDT、ETHUSDT、DOTUSDT和LTCUSDT。数据集被分为训练集、验证集和测试集，具体的日期范围在文中的表2中给出。
    

### **5.2 评估指标 (Evaluation Metrics)**

-   使用了六个不同的金融指标来评估交易策略，包括：
    

-   总回报率（Total Return, TR）
    
-   年化波动性（Annual Volatility, AVOL）
    
-   最大回撤（Maximum Drawdown, MDD）
    
-   年化夏普比率（Annual Sharpe Ratio, ASR）
    
-   年化卡尔马比率（Annual Calmar Ratio, ACR）
    
-   年化Sortino比率（Annual Sortino Ratio, ASoR）
    

### **5.3 基线比较 (Baselines)**

-   选择了八种基线方法进行比较，包括六种最新的强化学习算法和两种广泛使用的基于规则的交易策略。这些基线方法包括DQN、DDQN、PPO、CDQNRP、CLSTM-PPO、EarnHFT、IV和MACD。
    

### **5.4 实验设置 (Experiment Setup)**

-   所有实验在配备4个4090 GPU的系统上进行。交易设置中，所有四种加密货币的交易手续费率统一设置为0.02%，遵循Binance的政策。
    
-   详细描述了子代理和超代理的训练参数，包括嵌入维度、策略网络维度、数据块长度、训练周期和优化器设置。
    

### **5.5 结果与分析 (Results and Analysis)**

-   展示了MacroHFT和其他基线方法在四种加密货币市场的性能对比。表1和图3展示了不同方法在各种评估指标下的表现。
    

![](assets/004.png)

-   通过详细的交易示例（如图4所示），分析了MacroHFT在不同市场条件下的交易策略和行为。
    

![](assets/005.png)![](assets/006.png)

-   讨论了MacroHFT在不同市场环境下的适应性和盈利能力，以及与其他方法相比的优势。
...
-   为了评估条件适配器（Conditional Adapter, CA）和记忆模块（Memory, MEM）的有效性，进行了消融实验。通过移除这些模块，展示了它们对MacroHFT性能的影响（如表3和图6所示）。
...
本文总结了MacroHFT作为一种创新的高频交易强化学习方法的有效性，强调了其通过市场分解、条件适配和超代理记忆增强机制，在多个加密货币市场上实现的盈利能力和风险管理方面的卓越表现，显著超越了现有方法，并展示了其在应对市场快速变化中的潜力和稳健性。
