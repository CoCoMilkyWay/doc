...
QuantML QuantML 2024-07-04 21:37 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247485027&idx=1&sn=fd7db21edfc2eabd5554d08b7fbd9df1&chksm=cfd5dcfec303db464c34702aa14513c8a59d052e50d362a264c2cb873df5a76e07f2f5467809#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247485027&idx=1&sn=fd7db21edfc2eabd5554d08b7fbd9df1&chksm=cfd5dcfec303db464c34702aa14513c8a59d052e50d362a264c2cb873df5a76e07f2f5467809#rd)
...
本文讨论了随着可获取特征数量的指数级增长，识别出能够最小化模型方差而不损害其偏差的关键特征对于训练成功的机器学习模型至关重要。此外，特征选择对于提高模型的可解释性、预测精度和计算效率同样关键。论文提出了一种基于强化学习的方法来解决特征选择问题，通过将状态空间构建为马尔可夫决策过程，并使用时间差分算法来选择最优的特征子集，以实现对特征的高效选择。  

  

### **第1章：引言**

-   讨论了统计机器学习中最小二乘法的使用，以及替代的拟合程序，如子集选择、收缩和降维，以提高预测精度和模型可解释性。
    
-   强调了特征数量增长对模型偏差和噪声效应的影响，以及特征选择的重要性。
    

### **1.1 特征选择方法**

-   特征选择旨在通过移除噪声或考虑更简单的假设空间来减少学习假设的泛化误差。
    
-   介绍了过滤（scoring）、包装器（wrapper）和嵌入式（embedded）三种特征选择方法。
...
本章介绍了将特征选择问题视为一个强化学习任务的方法。作者将所有可能的特征子集定义为状态空间，并将模型中包含的每个特征视为一个动作。通过设计一个基于机器学习算法评估当前状态（特征子集）预测能力的奖励函数，利用时间差分（TD）算法来评估和更新状态值，从而选择出最优的特征子集。此外，论文还提出了一种支持向量机（SVM）分类器来评估每个特征子集的状态值，以处理数据集中的非线性关系，并强调了这种方法在高维特征空间中的鲁棒性。

  

### **第3章：问题表述**

将特征选择问题形式化为一个马尔可夫决策过程（MDP），其中状态空间是所有特征的幂集，动作是从状态空间中选择一个未见过的特征，实现状态间的转换。该过程从一组随机特征开始，逐步添加选定的特征，直至所有特征都被考虑。目标是确定最优策略π\*，以最大化模型的泛化精度，最小化从训练集学到的假设的误差。论文进一步定义了奖励函数和最优策略的数学表达式，为使用强化学习方法进行特征选择提供了理论基础。

  

### **第4章：强化学习框架**

-   状态空间定义：状态空间被定义为所有特征的幂集，即所有可能的特征子集。
    
-   奖励函数：奖励函数由选择当前状态下任何特征的动作决定，基于机器学习算法评估的预测能力。
    
-   时间差分（TD）方法：使用TD(0)方法来更新当前状态的值，这涉及到选择最佳动作并转移到下一个状态，而不是等待最终阶段，如蒙特卡洛方法所做。
    

### **4.1 特征评分**

-   引入了平均奖励（Average of Reward, AOR）的概念，用于衡量在状态中选择每个特征时的平均奖励，并通过增量方式更新AOR值。
    

### **4.2 特征选择**

-   随机阶段：在遇到未见状态时，通过随机选择特征进行探索。
    
-   epsilon-greedy阶段：在已知节点上，根据之前获得的奖励选择最大化预测准确性的动作，以高概率进行选择。
    

### **4.3 Trajectory of an Episode**

-   描述了代理在每个Episode中的行为轨迹，包括随机动作选择和基于AOR值的贪婪动作选择，以及如何通过添加特征和更新AOR表来评估所选特征的奖励。
...
展示了基于强化学习的特征选择方法在不同规模数据集上的实验验证和结果分析。作者选择了三个数据集，调整了参数epsilon以观察其对代理探索行为的影响，并使用包装器方法评估了所选特征子集的分类精度。实验结果显示，随着epsilon值的增加，代理访问的状态数量增加，且所提出的强化学习方法在特征选择和分类精度方面表现良好，尤其是在澳大利亚数据集上达到了85.55%的准确率。此外，通过与过滤方法的比较，强化学习方法在特征排名准确性方面也显示出优势。
...
本章讨论了通过调整epsilon参数来增强代理的状态探索能力，并评估了所选数据集的分类精度。尽管强化学习方法在特征选择和分类精度方面表现出了优势，但作者指出目前还不能直接与其他方法进行比较，因为该方法是基于选择最佳特征子集并评估其精度。

  

作者还提到，未来工作将包括使用更高级的强化学习技术来估计未见状态的值，并考虑应用策略迭代技术如Q-learning和SARSA，结合过滤方法来开发最优策略，以控制每个状态下选择的动作。这表明作者对现有模型的改进和扩展持开放态度，并展望了未来的研究方向。

  

> 论文链接：\[2101.09460\] Feature Selection Using Reinforcement Learning (arxiv.org)
> 
>   
> 
...
利用RL对Alpha158因子筛选，测试采用了10个因子，各因子的重要性如下：
