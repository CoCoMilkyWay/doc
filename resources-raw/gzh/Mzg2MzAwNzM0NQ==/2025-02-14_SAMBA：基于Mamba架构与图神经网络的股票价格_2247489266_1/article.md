# SAMBA：基于Mamba架构与图神经网络的股票价格预测模型

QuantML QuantML 2025-02-14 19:48 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489266&idx=1&sn=321d2a1506b2e8e1c867d95c7cf1caf4&chksm=cf2e3e8c97c9171cf274ca5d40e3ebc933d6c6832356bbd8bdf2b70bfcb666b6b2935b1a6da4#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489266&idx=1&sn=321d2a1506b2e8e1c867d95c7cf1caf4&chksm=cf2e3e8c97c9171cf274ca5d40e3ebc933d6c6832356bbd8bdf2b70bfcb666b6b2935b1a6da4#rd)

### 

![](assets/001.jpg)

本文提出了一种名为 **SAMBA** 的创新性股票价格预测模型，该模型结合了 **Mamba** 架构和图神经网络（**GNNs**），旨在应对金融市场中股票价格预测的复杂挑战。

### 一、引言：金融市场的机遇与挑战

金融市场的规模和复杂性不言而喻，全球每天有数十亿美元的资金在其中流动。对股票市场走势的准确预测可以为投资者和机构带来巨大的经济收益。然而，市场行为固有的复杂性和难以预测性使得这一任务极具挑战性。

**1\. 市场预测的重要性与挑战:**

-   **重要性:** 准确的预测对于制定交易策略、管理风险和优化投资组合至关重要。
    

-   例如，预测市场上涨可以指导投资者买入股票以获取收益，而预测市场下跌则可以指导投资者卖出股票以规避风险。
    

-   **挑战:**
    

-   市场行为受到众多因素的影响，包括经济指标、政治事件、市场情绪等，这些因素之间存在复杂的非线性关系。
    
-   图1展示了道琼斯工业平均指数（DJIA）、纳斯达克（NASDAQ）和纽约证券交易所（NYSE）从2010年到2023年的收盘价波动情况，清晰地展现了市场的波动性和不可预测性。
    

**2\.  深度学习在市场预测中的应用:**

近年来，深度学习技术的进步为股票市场预测带来了新的机遇：

-   **多层感知机（MLP）模型:** 能够捕捉金融数据中复杂的非线性关系。
    

-   例如，MLP可以通过多层非线性变换来学习股票价格与各种经济指标之间的复杂关系。
    

-   **卷积神经网络（CNNs）:**
    

-   使用每日特征的二维输入或包含相关股票的3D输入，CNN可以提取时间和跨股票的模式。
    
-   例如，CNN可以识别股票价格时间序列中的趋势和周期性模式，以及不同股票之间的相关性。
    

-   **长短期记忆网络（LSTM）:**
    

-   LSTM能够学习股票数据中的长期依赖关系，从而提高预测性能。
    
-   例如，LSTM可以记住过去几个月甚至几年的市场趋势，并将其用于当前的预测。
    

然而，传统的深度学习模型在处理金融时间序列数据时存在局限性：

-   **自注意力机制:**
    

-   自注意力机制的引入进一步推动了股票市场预测的发展，因为它允许模型捕捉全局依赖关系。
    
-   基于此，Transformer模型在序列预测任务中表现出色，因为它能够捕捉不同时间步之间复杂的依赖关系，而不受距离限制。
    
-   Transformer已成功应用于股票市场预测，通过建模时间序列数据中的复杂模式来提高预测精度。
    
-   **局限性:**  Transformer在推理过程中具有二次复杂度，导致计算成本和内存使用量高，这在实时交易和涉及长序列的场景中限制了其实际应用。
    

### 二、  Mamba架构：高效处理长序列的解决方案

为了解决Transformer的局限性，本文引入了 **Mamba** 架构作为长程序列分析的高效替代方案。
...
-   Mamba基于状态空间模型（SSMs），其时间复杂度接近线性。
    
-   这意味着随着输入序列长度的增加，计算成本的增长是线性的，而不是像Transformer那样呈二次增长。
    

-   **选择性扫描算法:**
    

-   Mamba采用选择性扫描算法，可以动态地关注输入序列中最相关的部分，同时过滤掉不太有用的数据。
    
-   这种机制优化了计算和内存使用，使其能够高效地处理长序列数据。
    

**2\.  MambaStock模型:**

-   近期提出的MambaStock模型将单个Mamba模型应用于使用历史股票市场数据预测股票价格。
    
-   **局限性:**
    

-   单个Mamba模型仅在一个方向上执行选择机制，这可能会限制其捕捉全局依赖关系的能力。
    

### 三、  SAMBA模型：结合Mamba与图神经网络的优势

为了克服MambaStock的局限性，本文提出了 **SAMBA** 模型，该模型结合了 **双向Mamba (BI-Mamba)** 块和 **自适应图卷积 (AGC)** 块，旨在更有效地处理复杂的序列数据。

**1\.  问题定义:**

-   本文将股票价格预测问题定义为回归问题，目标不是预测股票价格的绝对值，而是预测股票价格的变化。
    
-   具体来说，给定过去L天的每日股票特征数据X，目标是预测第L+1天的1天回报率。
...
-   BI-Mamba块处理原始和反向输入序列，以改进性能。
    
-   如图2(b)所示，输入每日特征矩阵被馈送到Mamba模型中，分别得到正向输出Y1和反向输出Y2。
    
-   通过残差连接，将输入与输出相加并归一化，得到Y3。
    
-   Y3通过前馈网络（FFN）处理，以捕捉时间依赖性，并通过残差连接再次归一化，得到最终输出Y。
    
-   BI-Mamba块重复R层，以捕捉处理后的每日股票特征的时间依赖性。
...
-   采用图卷积操作，使用切比雪夫多项式的K阶近似进行图谱滤波。
    
-   为了减少参数数量并防止过拟合并提高模型的泛化能力，SAMBA采用矩阵分解技术，将滤波器权重和偏置项表示为嵌入矩阵的函数。
    
-   AGC块使用可学习的滤波器权重和偏置项，为每个每日股票特征提供一组独特的参数。
    
-   通过在所有每日特征之间共享这些参数，模型可以动态地学习每个特征随时间推移的底层模式。
    
-   **图结构构建:**
    
-   将每日股票特征表示为图中的节点，将它们之间的交互表示为图结构。
    
-   与预定义的图结构不同，SAMBA采用任务特定的自适应图构建方法，使模型能够以端到端的方式学习每日股票特征之间的依赖关系。
    
-   具体来说，SAMBA使用高斯核构建每日股票特征之间的依赖图，并通过可学习的缩放因子控制节点之间的相似性衰减速度。
...
-   如图2(b)所示，SAMBA模型的整体架构如下：
    

-   输入每日特征矩阵首先通过BI-Mamba块处理，捕捉时间依赖性。
    
-   然后，输出被馈送到AGC块，捕捉每日股票特征之间的交互。
    
-   最后，AGC块的输出通过线性变换生成最终的股票回报输出。
...
-   本文使用来自美国股市的三个真实世界数据集：NASDAQ、NYSE和DJIA，时间跨度为2010年1月至2023年11月。
    
-   使用前80%的数据用于训练，接下来的5%用于验证，其余15%用于测试。
    
-   模拟实验在配备Intel Silver 4216 Cascade Lake @ 2.1GHz CPU和四个NVIDIA Tesla V100 Volta GPU（32 GB内存）的计算服务器上进行。
    
-   选择了SAMBA模型的参数E=64、H=64、R=3、U=32、K=3和de=10。
...
-   基于MLP的模型：StockMixer、FreTS
    
-   基于GNN的模型：AGCRN、FourierGNN
    
-   Transformer、LSTM、MambaStock
...
-   信息系数（IC）：表示预测与实际结果的接近程度，通过平均皮尔逊相关系数计算。
    
-   排名信息系数（RIC）：基于股票短期盈利潜力的排名，通过平均斯皮尔曼系数计算。
    
-   计算复杂度：考虑每个epoch的训练时间和乘加（MAC）操作总数。
...
-   表1展示了预测模型在三个数据集上的实验结果，并比较了计算效率。
    
-   **预测性能:**
    

-   与第二好的模型相比，SAMBA在NASDAQ上的RMSE提高了11.72%，在NYSE上提高了10.07%，在DJIA上提高了6.90%。
    
-   SAMBA在IC和RIC方面也表现出色，与第二好的模型相比，在NASDAQ上的IC和RIC分别提高了85.38%和80.30%，在NYSE上分别提高了36.43%和50.32%，在DJIA上分别提高了33.62%和53.54%。
    
-   SAMBA模型在RMSE、IC和RIC指标上均显著优于所有基线模型。
    
-   此外，SAMBA模型受益于BI-Mamba和AGC块，与依赖单向Mamba模型的MambaStock方法相比，预测性能有所提高。
...
-   它的计算复杂度高于基于MLP的模型和MambaStock，但低于基于GNN的模型和Transformer架构。
    
-   与AGCRN模型相比，SAMBA将每个epoch的训练时间提高了6.70%，并且只需要0.11百万个MACs，这比Transformer和AGCRN模型要少得多。
    

### 五、  结论与未来展望

股票市场的复杂性使得预测股票回报具有挑战性。虽然基于Transformer的模型已经显示出有希望的结果，但它们需要大量的计算资源。本文提出了SAMBA，这是一种计算效率更高但仍然准确的股票回报预测解决方案。SAMBA利用Mamba架构的选择性扫描特性来捕捉长期模式，并使用自适应图卷积来推断每日股票特征之间的关系。实验结果表明，SAMBA在预测性能上显著优于多个最先进的基线模型，同时保持了较低的计算复杂度，使其成为实时交易和金融分析的有价值的工具。
...
论文，数据及代码见星球，加入QuantML星球，与700+专业人士一起交流学习：
