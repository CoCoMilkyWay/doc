# DishFT-GNN: 基于蒸馏的未来感知图神经网络用于股票趋势预测

QuantML QuantML 2025-02-19 18:34 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489365&idx=1&sn=f993c427c5bb11c30aff2a4e76d25ffd&chksm=cf9b38dfea7a7a0bb69b2d03e85f9bb6de6c97fa4ddf9f4172a1b27250281640cd8d4d8144af#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489365&idx=1&sn=f993c427c5bb11c30aff2a4e76d25ffd&chksm=cf9b38dfea7a7a0bb69b2d03e85f9bb6de6c97fa4ddf9f4172a1b27250281640cd8d4d8144af#rd)
...
股票市场是一个充满活力的金融投资平台，全球众多企业和投资者在此进行交易。2022年，全球股票市场的总市值已从10年前的54.6万亿美元飙升至94.69万亿美元。随着人工智能技术的飞速发展，深度学习（DL）技术为投资者提供了更多通过股票投资增加财富的机会。然而，在瞬息万变的股市中，做出有利可图的投资决策和制定有效的交易策略并非易事。
...
基于深度学习的股票交易方法主要基于以下假设：股票的未来走势可以通过其历史模式来揭示。因此，**深入分析历史股票价格指标中的潜在时间动态特征至关重要**。例如，基于循环神经网络（RNN）的方法，以及其变体长短期记忆网络（LSTM）和门控循环单元（GRU），都旨在捕捉时间动态特征。

-   **RNN及其变体**：这些方法利用时间动态特征提取能力，通过分析历史数据中的时间依赖性来预测未来趋势。
    
-   **图神经网络（GNN）**：为了进一步捕捉股票之间的空间依赖性，GNN构建了股票图，将行业信息和持股信息等关系纳入其中。在这种框架下：
    

-   **节点**：代表单个股票，其属性由上述基于RNN的模型生成。
    
-   **边**：表示股票之间的关系。
    

然而，**仅依赖显式关系进行特征聚合可能会导致偏差和不完整**。因此，越来越多的研究开始关注使用基于深度学习的图表示方法来探究隐式关系，以更好地理解股票之间的相互作用。

####   
1.3 现有方法的局限性  

尽管基于GNN的股票预测器在捕捉时空依赖性方面表现出色，但它们主要关注分析历史时空依赖性，而忽略了历史与未来模式之间的相关性。这种局限性导致预测性能提升有限。

-   **问题所在**：股票数据具有高度非平稳性，这意味着其统计特性会随时间发生变化。例如，一份超出预期的公司年度财报可能导致历史数据和未来数据之间的分布发生突变，而仅依赖历史价格指标的当代股票预测器无法及时捕捉到这种变化，其影响只有在事后才能显现出来。
...
为了解决上述问题，本文提出了一种新的基于蒸馏的未来感知图神经网络框架（DisFT-GNN），用于股票趋势预测。该方法的核心思想是：

-   **捕捉历史与未来模式之间的相关性**：不仅分析历史股票数据中的特征，还关注历史与未来数据分布变化之间的相关性。
    
-   **证据支持**：通过分析公司年度财报等事件对股票数据的影响，可以看出历史与未来数据之间存在显著的分布变化，这进一步证明了捕捉这种相关性的必要性。
    

####   
2.2 问题陈述  

本文将股票趋势预测公式化为一个**二元节点分类任务**。具体来说：

-   **动态股票图**：在交易日 t 的动态股票图可以定义为 ，其中：
    

-     表示 N 个单独股票集合。
    
-    表示过去 L 个交易日的 M 个价格指标。
    
-    是归一化关系矩阵，表示股票之间的关系。
    

-   **目标**：预测未来 T 个交易日的股票趋势，即 ，其中：
    

-     表示输出二元变量集合。
    
-   对于  ， 表示预测第 n 个股票将上涨，否则为 0。
    

-   **真实标签定义**：对于 ，真实标签定义如下：
    

其中  表示交易日  的股票收盘价， 是超参数。当  且  时，问题就变成了下一个交易日的股票趋势预测。
...
DisFT-GNN 的总体架构如图 2 所示，包含两个训练过程：分别训练教师模型和学生模型。

1.  **教师模型训练**：
    

-   **编码历史股票图和未来趋势信息**：生成历史时空嵌入和高层次未来嵌入。
    
-   **多通道特征融合**：提出了一种新的基于注意力的多通道特征融合方法，将上述嵌入整合起来，生成未来感知时空表示，描述历史-未来分布变化的多样性。
    
-   **预测与优化**：将表示输入预测模块以获得预测结果，并与真实标签进行比较，以优化教师模型。
...
-   **预测**：将表示输入与教师模型共享的预测模块进行预测。
    
-   **知识蒸馏**：为了根据历史时空嵌入推断未来模式，学生模型通过最小化未来感知时空表示与教师模型提取的未来感知时空表示之间的蒸馏损失来进行训练。
...
-   **未来趋势编码器**：使用前馈网络 (FNN) 对股票未来趋势进行编码，生成新的未来趋势嵌入 。
...
-   **多通道结果生成**：执行多次向量-矩阵-向量 (VMV) 乘法运算，每个通道代表历史和未来分布之间的一种潜在相关性。
    
-   **注意力机制**：通过分配注意力分数来评估每个通道的重要性。
    
-   **特征融合**：采用与 Transformer 类似的缩放点积注意力操作进行特征融合，生成高层次未来感知时空表示 。
    

#####   
3.2.2 预测与优化  

-   **预测模块**：将  输入到预测模块，该模块是一个带有 softmax 函数的 FNN，用于预测股票趋势。
    
-   **损失函数**：通过最小化交叉熵损失 (CEL) 来学习参数。
    

####   
3.3 基于蒸馏的学生模型训练  

-   **历史时空表示生成**：学生模型编码历史股票图，生成历史时空表示 。
    
-   **预测**：将  输入到与教师模型共享的预测模块以进行预测。
    
-   **知识蒸馏**：
    

-   **目标**：使学生模型能够基于历史时空嵌入推断未来模式。
    
-   **损失函数**：通过最小化  和  之间的蒸馏损失来进行训练。这里使用希尔伯特-施密特独立性准则 (HSIC) 作为蒸馏损失，以更好地捕捉  和  之间的非线性依赖性。
    

-   **最终目标**：学生模型的目标是生成基于历史股票数据的未来模式，同时尽可能准确地预测股票趋势。
...
-   **数据集**：使用来自美国股票指数的 S&P 100 和 NASDAQ 100 两个数据集，时间跨度为 2019 年 1 月 1 日至 2023 年 9 月 30 日。所有数据集分为三部分：85% 用于训练，7.5% 用于验证，7.5% 用于测试。
    
-   **实验设置**：为了评估 DisFT-GNN 的性能，将其与七种 GNN 股票预测方法进行比较，即 GCN、GAT、TGC、ADGAT、MGAR、VGNN 和 MDGNN。使用准确率 (ACC) 和马修斯相关系数 (MCC) 作为评估指标。
...
-   **性能提升**：实验结果表明，DisFT-GNN 显著提升了所有基线模型的预测性能。在大多数情况下，准确率提高了超过 2%，与 MGAR 集成时最高提高了 5.41%，MCC 在两个数据集上提高了 82.3% 到 218%。
    
-   **盈利能力验证**：通过回测分析，使用经典 GCN 作为骨干网络，比较了有无 DisFT-GNN 的投资利润。结果显示，两种方法的利润差距随着时间的推移逐渐扩大，进一步验证了 DisFT-GNN 的有效性。
    

####   
4.3 消融研究  

-   **消融实验**：为了研究每个组件的贡献，对两个经典基线 GCN 和 GAT，以及两个最先进的基线 VGNN 和 MDGNN 进行了消融研究。
    

-   **DishFT-GNN w/o H**：用 MSE 替换 HSIC。
    
-   **DishFT-GNN w/o F**：用简单连接替换基于注意力的多通道特征融合模块。
    
-   **DishFT-GNN**：原始的 DishFT-GNN 框架。
    

-   **结果分析**：消融结果表明，所有组件对 DishFT-GNN 的性能都有积极影响。HSIC 有助于学生模型学习更广泛的非线性特征，而基于注意力的多通道融合方法有效地模拟了历史-未来分布变化的多样性，从而提高了预测性能。
...
本文提出了一种名为 DishFT-GNN 的新型基于蒸馏的框架，用于股票趋势预测。为了解决当代股票预测方法无法从历史股票知识中提取丰富的未来模式的问题，本文引入了一个经过训练的教师模型，该模型生成信息丰富的历史-未来相关性，指导学生模型的学习。具体来说：

1.  **教师模型**首先提取历史时空和未来嵌入。
    
2.  **提出了一种新的基于注意力的多通道融合方法**，通过整合两种嵌入来挖掘历史与未来分布之间的关联。
    
3.  **将融合后的表示作为中间监督**，指导学生 GNN 学习未来感知的时空表示以实现准确预测。
    

在真实世界数据上的广泛评估展示了所提出方法的有效性。
