# ViL：通用图像识别框架的xLSTM应用

QuantML QuantML 2024-06-11 23:10 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247484694&idx=1&sn=749b65c0987fc7f47ec79ec780f4a690&chksm=cf14f935cd635d08ee448f485099c115c1354a707f3ad33342b76687d91e6bd9ccf698dc7eda#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247484694&idx=1&sn=749b65c0987fc7f47ec79ec780f4a690&chksm=cf14f935cd635d08ee448f485099c115c1354a707f3ad33342b76687d91e6bd9ccf698dc7eda#rd)

Content

本文主要介绍了一种新的计算机视觉架构——Vision-LSTM（ViL），它是对扩展长短期记忆网络（xLSTM）的一种适应性改进。

### **1\. 引言 (Introduction)**

-   背景：介绍了Transformers在计算机视觉领域的广泛应用，尽管它们最初是为自然语言处理设计的。也提到了最近对LSTM的扩展——xLSTM，它通过指数门控和可并行化的矩阵记忆结构克服了LSTM的长期限制。
    
-   ViL的提出：作者提出了Vision-LSTM（ViL），这是xLSTM构建块在计算机视觉中的适应性改进。ViL通过堆叠xLSTM模块，其中奇数模块从上到下处理图像块序列，而偶数模块则从下到上处理。
...
ViL架构：详细描述了ViL如何使用xLSTM模块构建一个通用的计算机视觉骨架。ViL首先将图像分割成非重叠的块，然后添加可学习的定位嵌入到每个块令牌中。

### ![](assets/001.png)

-   mLSTM块：核心是交替的mLSTM块，这些块完全可并行化，并配备了矩阵记忆和协方差更新规则。
...
-   数据集介绍：ImageNet-1K 是一个广泛使用的图像识别数据集，包含1.3百万训练图像和50K验证图像，覆盖1000个类别。
...
-   训练周期：ViL模型在224x224分辨率下训练了800个周期（对于tiny和tiny+模型）或400个周期（对于small、small+和base模型）。
    
-   学习率：使用1e-3的学习率和余弦衰减调度。
    

#### 与Vision Mamba (Vim) 的比较

-   参数匹配：为了公平比较，作者增加了额外的块以匹配Vim的tiny和小规模变体的参数数量。
    
-   计算效率：ViL在计算上比Vim更高效，尽管Vim使用了优化的CUDA内核，ViL仍然在运行时间上快了最多69%。
    

#### 与ViT模型的比较

-   性能对比：ViL在小规模上显示出与经过超参数调整的ViT模型（如DeiT、DeiT-II、DeiT-III）相比具有竞争力的结果。
    
-   优化周期：ViL-S+在训练周期上比DeiT-III-S短，但性能相当。
    

#### 长序列微调

-   微调设置：通过增加序列长度（729个patch tokens）和50%的重叠patch tokens进行30个周期的微调。
    
-   性能提升：长序列微调进一步提高了模型的性能。
...
-   ViL的优势：总结了ViL如何通过在ImageNet-1K分类任务中超越基于SSM的视觉架构和优化的ViT模型来证明其有效性。
    
-   未来应用：讨论了ViL在需要高分辨率图像以获得最佳性能的应用中的潜在用途，例如语义分割或医学成像。
    
-   改进方向：提出了改进预训练方案、探索更好的超参数设置或从Transformers转移技术（例如LayerScale）作为ViL未来可能的发展方向。
...
感谢对于QuantML的支持，放送10张优惠券，某书也有618活动，数量均剩余不多，欢迎加入：
