# 利用LLM Agent和知识图谱自动生成研报

QuantML QuantML 2025-03-03 20:48 上海

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247489525&idx=1&sn=f82f72bbf257ef6ef30bd6ed3fc550b4&chksm=cf51e8a52325da3877f4e57df58d6735d027e11d50a2e7915216a03bf9dcfb652815ce8aa536#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247489525&idx=1&sn=f82f72bbf257ef6ef30bd6ed3fc550b4&chksm=cf51e8a52325da3877f4e57df58d6735d027e11d50a2e7915216a03bf9dcfb652815ce8aa536#rd)

### 

# 在当今信息爆炸的时代，撰写高质量的行业研报需要大量的数据收集、分析和整理工作。幸运的是，随着大语言模型（LLM）和人工智能（AI）技术的进步，我们现在可以利用LLM Agent来自动化这一过程。本文将详细介绍如何利用LLM Agent和知识图谱数据库（如Neo4j）来构建一个自动化的研报生成系统。

## 1\. 系统架构概述

我们将构建一个由多个LLM Agent组成的团队，每个Agent负责特定的任务，最终协同生成一份详细的行业研报。整个系统架构如下：

1.  **数据研究员Agent（Data Researcher Agent）**：负责收集和分析特定城市和行业的公司数据。
    
2.  **新闻分析师Agent（News Analyst Agent）**：负责提取和总结相关公司的最新新闻。
    
3.  **报告撰写员Agent（Report Writer Agent）**：负责将前两个Agent的输出整合成一份结构化的Markdown报告。
...
我们将使用Neo4j演示服务器上的公司数据库，该数据库包含有关组织、个人以及一些组织的最新新闻的详细信息。这些数据通过Diffbot API获取。

### 2.2 Neo4j连接设置

首先，我们需要设置Neo4j的连接URI和认证信息：

`URI = "neo4j+s://demo.neo4jlabs.com"   AUTH = ("companies", "companies")   driver = GraphDatabase.driver(URI, auth=AUTH)   `

### 2.3 OpenAI API 密钥

我们还将使用GPT-4o模型，因此需要设置OpenAI API密钥：

`import os   import getpass      os.environ["OPENAI_API_KEY"] = getpass.getpass("OpenAI key: ")   llm = LLM(model='gpt-4o', temperature=0.7)   `

## 3\. 知识图谱工具

### 3.1 获取城市信息工具

该工具用于获取特定城市中特定行业的公司统计数据：

`from pydantic import BaseModel, Field      class GetCityInfoInput(BaseModel):       city: str = Field(..., description="City name")       industry: str = Field(..., description="Industry name, available options: Software Companies, Professional Service Companies, etc.")      class GetCityInfo(BaseTool):       name = "Get information about a specific city"       description = "You can use this tool when you want to find information about companies in a specific city and industry."       args_schema = GetCityInfoInput          def _run(self, city: str, industry: str) -> str:           query = """           MATCH (c:City)<-[:IN_CITY]-(o:Organization)-[:IN_INDUSTRY]->(i:Industry)           WHERE c.name = $city AND i.name = $industry           RETURN count(o) AS organizationCount,                   sum(CASE WHEN o.isPublic THEN 1 ELSE 0 END) AS publicCompanies,                   sum(o.revenue) AS combinedRevenue,                   collect(CASE WHEN o.nbrEmployees IS NOT NULL THEN o END)[..5] AS topFiveOrganizations           """           result = driver.execute_query(query, {"city": city, "industry": industry})           data = [el.data() for el in result["records"]]           return str(data)   `

### 3.2 获取新闻工具

该工具用于获取特定公司的最新新闻：

`class GetNewsInput(BaseModel):       company: str = Field(..., description="Company name")      class GetNews(BaseTool):       name = "Get the latest news for a specific company"       description = "You can use this tool when you want to find the latest news about a specific company."       args_schema = GetNewsInput          def _run(self, company: str) -> str:           query = """           MATCH (c:Chunk)<-[:HAS_CHUNK]-(a:Article)-[:ABOUT]->(o:Organization)           WHERE o.name = $company AND a.date IS NOT NULL           WITH c, a           ORDER BY a.date DESC           LIMIT 5           RETURN a.title AS title, a.date AS date, a.sentiment AS sentiment, collect(c.text) AS excerpts           """           result = driver.execute_query(query, {"company": company})           data = [el.data() for el in result["records"]]           return str(data)   `

## 4\. Agent定义

### 4.1 Agent角色和目标

我们将使用crewAI平台来定义和运行Agent。每个Agent都有明确的角色、目标和背景故事：

`from crewai import Agent      data_researcher = Agent(       role="Data Researcher",       goal="Gather comprehensive information about specific companies in a given city and industry.",       backstory="You are an expert data researcher with deep knowledge of business ecosystems and city demographics. You excel at analyzing complex data relationships.",       tools=[GetCityInfo()],       llm=llm,       verbose=True,       allow_delegation=False   )      news_analyst = Agent(       role="News Analyst",       goal="Find and analyze recent news about relevant companies in a given city and industry.",       backstory="You are a seasoned news analyst with expertise in business journalism and market research. You can identify key trends and developments from news articles.",       tools=[GetNews()],       llm=llm,       verbose=True,       allow_delegation=False   )      report_writer = Agent(       role="Report Writer",       goal="Create comprehensive, well-structured reports combining the findings from data research and news analysis.",       backstory="You are a professional report writer with experience in business intelligence and market analysis. You excel at synthesizing information into clear, actionable insights. Do not include any information not provided.",       llm=llm,       verbose=True,       allow_delegation=False   )   `

### 4.2 Agent任务定义

每个Agent的任务如下：

`from crewai import Task      city_research_task = Task(       description=f"""Research and analyze {city_name} and its business ecosystem:   1. Get city summary and key information   2. Find organizations in the specified industry   3. Analyze business relationships and economic indicators""",       agent=data_researcher,       expected_output="Basic statistics about the companies in the given city and industry."   )      news_analysis_task = Task(       description=f"""Analyze recent news about the companies provided by the city research task.""",       agent=news_analyst,       expected_output="Summarization of the latest news for the company and how it might impact the industry.",       context=[city_research_task]   )      report_writing_task = Task(       description=f"""Create a detailed markdown report about the results you got from city research and news analysis tasks. Do not include any information that isn't provided.""",       agent=report_writer,       expected_output="Markdown summary.",       context=[city_research_task, news_analysis_task]   )   `

## 5\. 运行Agent团队

最后，我们将所有Agent和任务组合成一个团队并运行：

`from crewai import Crew, Process      crew = Crew(       agents=[data_researcher, news_analyst, report_writer],       tasks=[city_research_task, news_analysis_task, report_writing_task],       verbose=True,       process=Process.sequential   )      city = "Seattle"   industry = "Hardware Companies"   report = generate_report(city, industry)   print(report)   `

## 6\. 示例输出

以下是一个示例报告的输出：

`# Seattle Computer Hardware Industry Report      ## Overview      The Computer Hardware Companies industry in Seattle comprises 24 organizations.      ## Top Performers      1. **Microsoft Corporation**      - **Revenue**: $198.27 billion      - **Employees**: 221,000      - **Status**: Public Company      - **Mission**: To empower every person and organization on the planet to achieve more.      2. **Nvidia Corporation**      - **Revenue**: $26.97 billion      - **Employees**: 26,196      - **Status**: Public Company      - **Formerly Known As**: Mellanox Technologies and Cumulus Networks.      3. **F5 Networks**      - **Revenue**: $2.695 billion      - **Employees**: 7,089      - **Status**: Public Company      - **Focus**: Multi-cloud cybersecurity and application delivery.      4. **Quest Software**      - **Revenue**: $5857.415 million      - **Employees**: 4,055      - **Status**: Public Company      - **Base**: California.      5. **SonicWall**      - **Revenue**: $300 million      - **Employees**: 1,600      - **Status**: Private Company      - **Focus**: Cybersecurity.      These companies significantly contribute to Seattle's economic landscape.      ## Recent News and Developments      - **Microsoft Corporation**: Faces legal challenges with its Activision Blizzard acquisition.   - **Nvidia Corporation**: Experiences strong demand for GPUs in China, highlighting its market dominance.   - **F5 Networks**: Gains recognition for its cybersecurity solutions, enhancing its market position.   - **Quest Software**: Launches a new data intelligence platform aimed at improving data management.   - **SonicWall**: Undergoes leadership changes and releases a threat report, emphasizing its commitment to cybersecurity.      These developments are poised to influence market dynamics, investor perceptions, and industry trends.      Note that the demo dataset is outdated; we don't import news regularly.   `

## 7\. 结论

通过使用LLM Agent和Neo4j知识图谱数据库，我们可以构建一个高效的自动化研报生成系统。该系统不仅能够快速收集和分析数据，还能生成结构化的报告，节省大量的人力和时间。未来，随着AI技术的不断进步，这种方法有望在更多领域得到广泛应用。
