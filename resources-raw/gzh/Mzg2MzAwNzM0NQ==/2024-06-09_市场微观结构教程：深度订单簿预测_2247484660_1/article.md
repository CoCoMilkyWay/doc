...
QuantML QuantML 2024-06-09 22:33 陕西

> 原文地址: [https://mp.weixin.qq.com/s?\_\_biz=Mzg2MzAwNzM0NQ==&mid=2247484660&idx=1&sn=fc191107da65068ab9a5cd01135a5f1d&chksm=cff65b6249ba792229f1bd28d86d130a5bfaa5b76c423475d222c53807deb68b4ac02f9befdf#rd](https://mp.weixin.qq.com/s?__biz=Mzg2MzAwNzM0NQ==&mid=2247484660&idx=1&sn=fc191107da65068ab9a5cd01135a5f1d&chksm=cff65b6249ba792229f1bd28d86d130a5bfaa5b76c423475d222c53807deb68b4ac02f9befdf#rd)

Content

  

本文主要探讨了利用深度学习方法预测高频交易中限价订单簿（Limit Order Book, LOB）中间价格变化的可行性。

### **1\. 引言 (Introduction)**

-   背景: 金融市场是一个高度随机的环境，具有低信噪比，由不同时间尺度、不同信息水平和不同技术能力的交易者组成。
    
-   交易所系统: 现代交易所依赖计算机化系统来处理订单，这些系统通过限价订单簿（LOB）提供市场供需匹配机制。
    
-   高频交易（HFT）: 论文特别关注高频交易，这是一种通过速度获得优势的策略，允许某些交易者在其他人之前行动。
    
-   深度学习的应用: 近年来，在单变量和多变量时间序列预测方面取得的深度学习成就为高频LOB预测开辟了新的可能性。
    

### **2\. 限价订单簿 (Limit Order Book)**

-   定义: LOB是一个数据结构，记录了每个证券的买卖订单，包括价格、数量和时间戳。
    
-   订单类型: 包括限价订单、市价订单和取消订单。
    
-   价格形成: 通过订单的提交和取消，LOB驱动了证券价格的形成。![](assets/001.png)
    

### **3\. 相关工作 (Related Work)**

-   市场微观结构分析: 研究交易者意图如何转化为价格和交易量。
    
-   交易成本: 研究交易成本的驱动因素，包括价格影响和最优订单执行策略。
    
-   自动化学习模型: 论文将相关文献分为三个主要领域：线性模型、非线性深度学习模型和强化学习方法。
    

### **4\. 数据 (Data)**

### 本章节为读者提供了对使用数据集的全面了解，包括数据的选择、处理和组织方式，这对于评估论文中提出的模型和方法的有效性至关重要。

### **数据来源**

-   股票选择: 研究涵盖了15个不同行业和领域的股票，所有这些股票都在纳斯达克交易所交易。
    
-   数据提供者: 使用的数据来自LOBSTER提供商，提供了高质量的逐笔（tick-by-tick）LOB数据。
...
-   行业和领域: 根据纳斯达克交易所的分类体系，作者们确定了这些股票的行业和领域归属。
    
-   市值: 研究考虑了2017年至2019年期间，市值在大型（10B-200B美元）到超大型（≥200B美元）的股票。
    

### **数据集结构**

-   时间范围: 数据集涵盖了2017年至2019年的整个交易时段。
    
-   训练、验证和测试集: 数据被分为训练集、验证集和测试集，每个集都包括连续的交易日。例如，训练集可能包含45天的数据，验证集5天，测试集10天。
...
-   去除交叉报价（crossed quotes）。
    
-   合并同一时间戳发生的多个状态（collapse states）。
    
-   排除拍卖调用的影响，只考虑特定交易时间内的事件。
    

-   标准化: 采用5天滚动窗口的z-score标准化方法，以适应LOB数据的非平稳特性。
    
-   数据格式: 数据被处理成两个文件类型：'message file' 和 'orderbook file'。'message file' 记录了每个市场事件，而 'orderbook file' 描述了事件发生后市场的即时状态。
...
-   数据集划分: 作者们为每年的数据集指定了特定的训练、验证和测试时间段，并确保了这些时间段不重叠，以避免数据泄露。
    
-   样本大小: 论文中提到了使用的数据样本大小，例如在训练DeepLOB模型时，使用了特定数量的样本，并根据类别的代表性进行了采样。
    

### **数据集的局限性**

-   论文也讨论了使用的数据集可能存在的局限性，例如数据集的规模可能影响模型的泛化能力，以及数据的非平稳性可能对模型性能产生影响。
    

### **5\. 方法 (Methods)**

### 本章详细介绍了论文中使用的方法论，包括数据的处理方式、模型的训练和验证过程，以及评估模型输出质量的方法

### **数据处理**

-   数据转换和处理: LOBFrame 提供了一个综合性的数据转换和处理流程，将原始的LOB数据转换成适合深度学习模型输入的格式。
    

### **模型训练和验证**

-   DeepLOB模型: 使用了一种称为 "DeepLOB" 的先进深度学习模型，该模型专门设计用于处理LOB数据。
    
-   卷积神经网络（CNN）: 利用CNN来模拟LOB的层级和空间动态。
    
-   长短期记忆网络（LSTM）: 使用LSTM来处理输入数据的时间维度。
...
-   输入: DeepLOB模型的输入是LOB的快照，包括价格和交易量信息。
    
-   时间序列: LOB被表示为多变量时间序列，每个时间点的LOB记录包含了多个价格和交易量级别。
    

### **训练细节**

-   数据采样: 在训练过程中，采用随机和平衡的采样方法，确保每个类别的样本数量均衡。
    
-   批量大小: 使用32个样本的小型批量进行训练，每个样本包含100×40的输入尺寸，代表100个连续的LOB更新。
    

### **性能评估**

-   评估指标: 通过交易模拟来评估模型输出的质量，尽管论文指出实际的交易模拟结果并未包含在内。
    

### **技术细节**

-   优化器: 使用了 "AdamW" 的优化器，它是Adam优化器的一个变种，包含了权重衰减。
    
-   学习率: 根据训练流程和数据特性，选择了特定的学习率和其他训练参数。
    

### **实验设置**

-   GPU计算: 实验在伦敦大学学院计算机科学系的高性能计算集群上进行，使用了多种类型的GPU。
    

  

### **6\. 微观结构先验 (Microstructural Priors)**

### 本章专注于分析股票的微观结构特性，这些特性对于理解深度学习模型在预测限价订单簿（LOB）中间价格变化方面的有效性至关重要

### **股票的微观结构特性**

-   定义: 微观结构特性涉及股票的交易行为和订单簿的动态，包括价格形成机制、交易成本、市场流动性等。
    

### **股票分类**

-   基于tick size的分类: 论文根据股票的平均价差（spread）与最小价格变动单位（tick size）的关系，将股票分为小tick、中tick和大tick三类。
    

-   小tick股票：平均价差远大于tick size。
    
-   中tick股票：平均价差接近tick size。
    
-   大tick股票：平均价差远小于tick size。
...
-   分析: 论文分析了三年期间股票的平均价差与tick size之间的关系，并根据这一关系对股票进行了分类。
...
-   最佳报价的成交量: 研究了在最佳买卖报价（即最佳报价）上的成交量分布，这反映了市场的流动性水平。
    

### **实际LOB深度（Actual LOB Depth）**

-   定义: 为了衡量LOB结构的均匀性，论文引入了“实际LOB深度”（Ξ）这一指标，用于衡量LOB中价格水平之间的距离。
    

### **信息丰富度（Information Richness, IR）**

-   概念: IR指标衡量了股票在LOB最佳报价水平上的活动频率，与股票的预测能力有关。
    

### **股票的交易活动**

-   分析: 论文还分析了股票在不同时间尺度上的交易活动，包括LOB更新在物理时间中的分布。
    

### **微观结构特性与预测能力**

-   关联性: 论文探讨了股票的微观结构特性如何影响深度学习模型的预测能力，特别是对于不同类别的股票（小tick、中tick、大tick）。
    

### **7\. 结果 (Results)**

-   模型性能: 使用Matthews相关系数（MCC）评估了DeepLOB模型的性能。
    
-   预测实用性: 提出了一种新的实用框架，评估预测在实际交易中的可行性。
