...
//
// T2 一次子进程把 tag_loop.py 真正要用到的 SDK 接口全查一遍 (只 import 与签名, 不联网, 不起 bridge, 不建 agent):
// 0=齐全 1=没装 其余=装了但与本项目用法不符 (原因由 python 自己打到 stderr)。
// 查的是 tag_loop.py 对 SDK 的几条假设, 与其说是"检查依赖", 不如说是把假设钉在这里 —— 万一某版 SDK
// 改了这些, 在环境检查就失败, 而不是跑到第一篇 (或第几百篇) 才炸:
//   1) AsyncClient.launch_bridge 是 async classmethod 且收 workspace 关键字 (云端 agent 也得先起本地 bridge)
//   2) AsyncClient 是异步上下文管理器 (我们用 async with 管它的生命周期, 退出自动关 bridge 子进程)
//   3) AsyncCursor.models.list 存在 (启动时校验 TAG_AGENT_MODEL 在模型表里)
//   4) CloudAgentOptions 有 repos (给 [] = 无仓库) 和 metadata (打标记, 供清理上次泄漏的 agent)
static const char *SDK_PROBE_PY = R"PY(
import dataclasses, importlib.metadata, inspect, sys
try:
    import cursor_sdk
...
if missing:
    ...
    sys.exit(3)
lb = getattr(cursor_sdk.AsyncClient, 'launch_bridge', None)
if not (lb and inspect.iscoroutinefunction(lb)):
    print('[环境] cursor_sdk.AsyncClient.launch_bridge 不是 async 方法', file=sys.stderr)
    sys.exit(4)
kw = inspect.signature(lb).parameters.get('workspace')
if kw is None or kw.kind not in (kw.KEYWORD_ONLY, kw.POSITIONAL_OR_KEYWORD):
    print('[环境] cursor_sdk.AsyncClient.launch_bridge 不收 workspace 关键字', file=sys.stderr)
    sys.exit(4)
if not all(hasattr(cursor_sdk.AsyncClient, n) for n in ('__aenter__', '__aexit__')):
    ...
    sys.exit(5)
if not hasattr(cursor_sdk.AsyncCursor, 'models') or not hasattr(cursor_sdk.AsyncCursor.models, 'list'):
    print('[环境] cursor_sdk.AsyncCursor.models.list 不存在', file=sys.stderr)
    sys.exit(5)
...
if not {'repos', 'metadata'} <= fields:
    ...
    sys.exit(6)
ver = getattr(cursor_sdk, '__version__', '') or importlib.metadata.version('cursor-sdk')  # 包没有 __version__, 退回 dist 元数据
print('[环境] cursor-sdk', ver, file=sys.stderr)
)PY";
