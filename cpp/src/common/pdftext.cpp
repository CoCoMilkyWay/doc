...
static poppler::document *load(const std::string &path) {
  static std::once_flag once;
  std::call_once(once, [&] {
    poppler::set_debug_error_function(silence, nullptr);
    pinned = poppler::document::load_from_file(path);
  });
  return poppler::document::load_from_file(path);
}

int pdf_pages(const std::string &path) {
  std::unique_ptr<poppler::document> doc(load(path));
