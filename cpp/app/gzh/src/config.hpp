...
// 文章库根目录，相对当前工作目录（run.py always cd 到本项目根）。文章是文档库
// 的原始输入，与研报/论文并列放在 doc 仓库的 resources-raw/ 下，程序本身留在
// cpp/app/gzh/，故要上跳三级。换目录是搬家而不是日常操作，真要换直接改这里重编译。
constexpr const char *kStoreDir = "../../../resources-raw/gzh";

// dump 落盘目录（见 kDump），相对同一个工作目录。放 tmp/ 而不是 kStoreDir 下：
// 它是所有域名的往返原始数据，属于本机调试产物，不是文章库的内容。
constexpr const char *kDumpDir = "tmp/dump";

// dump 开关：true 则对所有域名 MITM 并把每次往返落盘到 kDumpDir。
