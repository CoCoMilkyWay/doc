...
│   │                                       #   翻页每页条数与间隔, UA, 目标主机, 文章库目录 (kStoreDir), dump 开关
...
└── (文章库)                                # 不在本目录: 文章是文档库的原始输入, 与研报/论文并列放在 doc
                                            #   仓库的 resources-raw/gzh/ (gitignore, 路径写死在
                                            #   src/config.hpp kStoreDir); 一个子目录一个公众号
    └── <__biz>/
...
main(argv):
    ...
    run_sync(config::kStoreDir)                                   # 出网路由 config::kUseTunnel 编译期定死  (egress.cpp)

run_sync(root):
    creds = Credentials(~/.wxmd/credentials.json)     # 构造即读回, 过期的直接丢掉  (capture.cpp)
    known = list_accounts(root)                        # 扫文章库下每个子目录的 account.json  (store.cpp)
...
```bash
./run.py                                             # 增量同步文章库, 结束后起代理等着加新号
./run.py --uninstall                                 # 还原系统代理、移除 CA 与 ~/.wxmd (不动文章库)
./run.py --build                                     # 只编译
