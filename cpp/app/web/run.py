...
# 帖子是文档库的原始输入, 与研报/论文并列放在 doc 仓库的 resources-raw/ 下; 程序本身
# 留在 cpp/app/web/, 故上跳三级。
DOC_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(ROOT)))
assert os.path.isdir(os.path.join(DOC_ROOT, "resources-raw")), (
    "找不到 %s/resources-raw" % DOC_ROOT
)
...
# 缓存根目录。None = <doc 仓库>/resources-raw/web/<site>/
STORE = None
    ...
    try:
        for name in SITES:
            ...
            store_root = STORE or os.path.join(
                DOC_ROOT, "resources-raw", "web", site.name
            )
            run(
                site,
                store_root,
