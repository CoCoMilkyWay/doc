// stage1 scan: 校验 RAW_REPORT_DIR 下所有 PDF 的文件名与目录结构是否符合命名规范。
//
...
// 跨文件/结构检查 (structure.cpp / content.cpp), 目录层次 = RAW_REPORT_DIR/{券商}/{系列}/,
// TOPIC_DIR 下为 RAW_REPORT_DIR/0主题研报/{主题}/:
...
// 依赖: libpoppler-cpp (pkg-config poppler-cpp). 参数见 config.hpp。
#pragma once
