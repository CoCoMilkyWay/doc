...
def build_request_form_data(
    ...
    return _api_client.build_parse_request_form_data(
        ...
        start_page_id=start_page_id,
        end_page_id=end_page_id,
        # docpipe 剪裁: 只要 md + images, 不要 middle/model/content_list json 与 origin pdf (调试产物, 体积 100x)
        return_md=return_md,
...
async def run_orchestrated_cli(
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as http_client:
        try:
            ...
            # docpipe 剪裁: 不画 _layout.pdf/_span.pdf (依赖已关闭的 middle.json/origin pdf), 保持 None
