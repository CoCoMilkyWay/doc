...
2007-2024 年 4 月 5 日 A 股日频量价数据，包括高、开、低、收、均价、成交量 6 个序列，我们以每周的最后一个交易日t为截面，回溯截面上 $N_{t}$ 个股票过去 30 个交易日的数据，作为一个截面（对应 $N_{t}$ 个标签），以此滚动生成数据截面 $(\ N_{1},30,6\ )_{\cdot}\ (\ N_{2},30,6\ )\ \dots$ $(N_{t},30\small{,}6)$ ，在第一个维度上进行拼接得到全部训练数据。
...
$$
\alpha_{i}=softmax(MLP_{2}\left(SiLU(MLP_{1}(S_{i}))\right))
$$
