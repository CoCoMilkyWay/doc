...
在拟合好回归模型 $后$ ，根据逻辑需要，找到可以反映或代理该逻辑的量化数据，比如是需要 $\beta_{X}$ ，还是需要拟合后的最后一期的残差 $\varepsilon_{T}$ ，还是需要变量解释程度 $R^{2}$ 等等，甚至更进一步，这些描述线性关系的统计数据在时间序列上的性质等等。

## 最后，要说明的有以下几点：

1. 该回归模型是在滚动时间序列， $而$ 非在股票横截面上做回归。每个股票的因子仅由其自己各个不同财务数据时间序列回归得出。滚动窗口本身可以设置为参数，一般默认为 8 期，即使用过去 2 年的数据。同时默认做市值与行业中性。
