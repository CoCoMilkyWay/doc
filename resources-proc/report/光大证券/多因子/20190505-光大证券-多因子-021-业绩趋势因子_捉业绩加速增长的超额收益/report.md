...
$$
\mathrm{standard}(factor)_{jt}=\frac{factor_{jt}-\overline{factor_{t}}}{std(factor)_{t}}\tag{2}
$$
...
EBPT 因子为一种成长类的因子，在光大证券多因子系列此前的研究报告中，我们已梳理过预测能力较为强的成长类因子，包括传统的 NP_Q_YOY（单季度净利润同比增速）、创新性的 $\mathsf{OP\_SD^{1}}$ $\mathsf{LPNP^{2}}$ 等。本节我们主要比较EBPT 因子与其他成长因子的表现，并分析其与主流因子的相关性。
