...
$$
illiq=mean(\frac{|Return_{t}|}{Amount_{t}})
$$
...
主动买卖盘，分别以价类数据中的成交价、价类数据中的挂单价和量类数据中的成交量给出构造。从每日逐笔成交来看，当前成交价大于上一笔买一价时，交易以买方为主导，记成交量为vol_buyi；反之当前成交价小于上一笔卖一价时，交易以卖方为主导，记成交量为 $vol_{-}sell_{i}。$ 。则主买量为 $\begin{array}{r}{Vol\_Buy_{t}=\sum vol\_buy_{i}}\end{array}$ ，以衡量多空博弈双方多头力量，主卖量为 $\begin{array}{r}{Vol\_Sell_{t}=\sum vol\_sell_{i}}\end{array}$ ，以衡量多空博弈双方空头力量。
...
$$
Stren=\frac{\sum weight_{t}\times Vol\_Buy_{t}}{\sum weight_{t}\times Vol\_Sell_{t}}
$$
...
1. 确定窗口最大最小值。对于给定时间序列 $x_{t},t=1,2,3,\dots N$ ，遍历所有点，对于任何一个时间点t，考察时间序列x 在t为中心的前后各j个时间段内的最大值或最小值，具体为

$$
\begin{array}{r}{Status_{t,m}=\left\{\begin{array}{ll}{1}&{x_{t}\geq x_{t+i},i=-m,-m+1,\ldots,m-1,m}\\{-1}&{x_{t}\leq x_{t+i},i=-m,-m+1,\ldots,m-1,m}\end{array}\right.}\end{array}
$$
