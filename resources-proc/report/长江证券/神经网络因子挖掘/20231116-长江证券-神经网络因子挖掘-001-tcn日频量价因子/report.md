...
$$
vol_{t}=\frac{vol_{t}-\min(vol_{i})}{\max(vol_{i})-\min(vol_{i})},i=1,2,\ldots63
$$
...
$$
Loss_{3}=\frac{\sum_{i=1}^{n}-\mathrm{Corr}(\hat{y}_{i},y)}{n}
$$

第二种思路是将所有因子先取均值，再求得到的等权合成因子的有效性，即可反映多个因子的整体有效性，且同时可以反应多个因子的线性可加性。

$$
Loss_{4}=-\mathbb{Corr}(\sum_{i=1}^{n}\widehat{y_{i}},y)
$$
...
$$
\frac{Loss_{5}=\sum_{i=1}^{n}\sum_{j=1}^{n}Corr(\widehat{y_{i}},\widehat{y_{j}})^{2}}{n_{2}}
$$
...
$$
Loss_{final}=-\frac{\sum_{i=1}^{n}\operatorname{Corr}(\hat{y}_{i},y)}{n}-\lambda_{1}\operatorname{Corr}(\sum_{i=1}^{n}\hat{y}_{i},y)+\lambda_{2}\frac{\sum_{i=1}^{n}\sum_{j=1}^{n}\operatorname{Corr}(\hat{y}_{i},\hat{y}_{j})^{2}}{n_{2}}
$$
