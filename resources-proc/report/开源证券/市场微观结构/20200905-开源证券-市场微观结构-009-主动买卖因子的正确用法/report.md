...
## 主动买卖因子合成： $\mathbf{ACT}_{正向}$ 较 $\mathbf{ACT}_{负向}$ 具有更强的选股能力

基于切割分析，我们分别构建基于大单和中单的正向主动买卖因子 $\mathrm{ACT}_{正向}$ ，和基于小单的负向主动买卖因子 $\mathrm{ACT}_{负向}$

$\mathrm{ACT}_{正向}$ 因子在不同切割比例λ下的选股能力均优异，且λ越小，多空对冲的表现越佳。其中当λ=10%时，多空对冲的收益波动比达3.06;

$\mathrm{ACT}_{负向}$ 因子在不同切割比例λ下的收益波动比相对稳定，但近几年来收益逐渐变低（曲线收益变缓)，这与我们的直觉也是一致的：近年来，大资金的主导性越来越强，因此代表大户和中户的主动买卖因子 $\mathrm{ACT}_{正向}$ 相对于代表散户的 $\mathrm{ACT}_{负}$ 向因子，在选股能力上表现更加优异。
...
$$
\mathrm{ACT}=\frac{主动买入金额-主动卖出金额}{主动买入金额+主动卖出金额}
$$
...
## 3、主动买卖因子的合成： $\mathrm{ACT}_{正向}$ 较 $\mathrm{ACT}_{负向}$ 有更强的选股能力

从上述的切割化分析，我们更精细化的发现四类挂单水平下的主动买卖对市场带来的影响。具体来说：小单ACT因子在低收益端呈现负向选股效应，大单和中单ACT因子在高收益率端呈现正向选股效应。基于此，我们分别构建基于大单和中单的正向主动买卖因子 $\mathrm{ACT}_{正向}$ ，和基于小单的负向主动买卖因子 $\mathrm{ACT}_{负向}$ ，具体过程如下：

(1) 逐日计算大单和中单总的主动买卖因子 $\mathrm{ACT}_{正向,\mathrm{t}},$ 以及小单的主动买卖因子 $\mathrm{[ACC_{负向,t}]}$

$$
\mathrm{ACT}_{\mathrm{正向},t}=\frac{主动买入金额\big(大单+中单\big)-主动卖出金额\big(大单+中单\big)}{主动买入金额\big(大单+中单\big)+主动卖出金额\big(大单+中单\big)}
$$

$$
\mathrm{ACT_{负向,t}=\frac{主动买入金额\left(小单\right)-主动卖出金额\left(小单\right)}{主动买入金额\left(小单\right)+主动卖出金额\left(小单\right)}}
$$

(2) 回溯过去20个交易日，取收益率最高λ比例的交 $易$ 日，称为高收益日；取收益率最低λ比例的交易日，称为低收益日；

(3) 对高收益日的 $\mathrm{ACT}_{正向,\mathrm{t}}$ 因子取平均，记为 $\mathrm{ACT}_{正向}$ ；对低收益日的 $\mathrm{ACT}_{负向,\mathrm{t}}$ 因子取平均，记为 $\mathrm{ACT}_{负1}$ 向。

我们分别对 $\mathrm{ACT}_{正向}$ 和 $\mathrm{t}\mathrm{ACT}_{负向}$ 进行五分组回测。从图6的结果可以看到， $\mathrm{ACT}_{正向}$ 因子在不同λ的切割比例下选股能力均优异，且λ越小，多空对冲的表现越佳。其中当λ=10%时，多空对冲的收益波动比达3.06。另外，通过表2可以看出，该因子在多头端的选股能力优异，λ=10%下，多头的收益波动比达到0.87。

图6: $\mathrm{ACT}_{正向}$ 的多空对冲净值：各多空对冲净值曲线在λ维度区分明显
![](images/a953eacc6455b66876a4c3981419216b2de9d001f6027e90787a6b64247e1bfe.webp)
数据来源：Wind、开源证券研究所

表2: $\mathrm{ACT}_{正向}$ 的绩效表现：因子的收益波动比随着λ的增加而变小，多空和多头绩效均表现优异
...
从图7和表3可以看到， $\mathrm{ACT}_{负向}$ 因子在不同切割比例λ下的收益波动比相对稳定，但近几年来收益逐渐变低（曲线收益变缓)，这与我们的直觉也是一致的：近年来，大资金的主导性越来越强，因此代表大单和中单的主动买卖因子 $\mathrm{ACT}_{正向}$ 相对于小单$\mathrm{ACT}_{负向}$ 因子，在选股能力上表现更加优异。

图7: $\mathbf{ACT}_{负向}$ 的多空对冲净值：近几年来，该因子的多空对冲收益逐渐降低
![](images/93748f100027610fa119d283bd6e14c8b05d9d8e9c242f9f9d838af81ffc6991.webp)
数据来源：Wind、开源证券研究所

表3: $\mathrm{ACT}_{负向}$ 的绩效表现：在不同λ比例下，该因子的收益波动较稳定，但总体的选股能力表现不佳
...
通过上述因子的分析，我们认为 $\mathrm{ACT}_{正向}$ 因子更能代表股票的主动买卖性质。另外，我们对 $\mathrm{ACT}_{正向}$ 和常见的Barra风险因子进行相关性分析（这里我们取 $\lambda=10\%$ 从表4可以看出，该因子和流动性、波动性因子具有较强的负相关性。

表4: $\mathrm{ACT}_{正向}(\lambda=10\%)$ 与Barra因子的相关性：该因子与流动性、波动等因子的负相关性较高
...
为了剔除这些风格因子的干扰，我们对 $\mathrm{ACT}_{正向}$ 进行行业和Barra风格因子的中性化处理，提纯后的因子表现如表5所示。可以看到，提纯后的因子表现依然优异，在λ=10%下，因子的多空收益波动比达到2.40。

表5: $\mathrm{ACT}_{正向}$ 因子的绩效表现(行业风格中性化)：提纯后的因子在多空和多头端表现均优异
...
在前文的探讨中，我们使用了过去20天作为切分的时间长度，为了防止参数的过拟合，我们对其进行不同回看天数下的绩效遍历。表6展现了在40天和60天下的$\mathrm{ACT}_{正向}$ 因子的表现：在λ=10%下，回看天数为40天的因子，多空和多头收益波动比为2.56和0.90；回看天数为60天下的因子，多空和多头收益波动比为2.37和0.93。因子的多空总体绩效较回看20天而言，表现稍弱，但多头依然稳健。

表6：不同回看天数下的 $\mathbf{ACT}_{正向}$ 因子表现：回看天数的参数稳定性较强
...
我们进一步探讨因子在其他样本空间中的表现。以回看过去20天为例， $\mathrm{ACT}_{正}$ 向因子在沪深300和中证500的样本空间内依然具有稳健的选股能力：以λ=20%为例，沪深300内因子的多空和多头的收益波动比分别为1.32和0.63，中证500内因子的多空和多头的收益波动比分别为1.78和0.80。

表7: $\mathrm{ACT}_{正向}$ 因子在其他样本空间的表现：因子在沪深300和中证500内的选股能力依然稳健
