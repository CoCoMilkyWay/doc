...
$$
\mathop{max}_{w}\ :r^{T}\ :w
$$

$$
s.t.\quad\quad S_{d}\leq X_{s}(w-w_{b})\leq S_{u}
$$

$$
I_{d}\le X_{I}(w-w_{b})\le I_{u}
$$
...
2) $X_{I}$ 为行业暴露矩阵，基础模型中我们令行业中性，也就是 ${\cal I}_{d}=I_{u}=0;$
...
6) $I_{c}$ 为表示组合成分股是否为基准指数成分股的向量，不同增强 $\dot{\mathcal{P}}$ 品的合同规定不同，这里我们令组合占基准的比例γ至少为80%;

7) 为了降低组合因高换手导致额外的交易费用，这里 $.\rho\cdot$ 代表换手率约束，上限为50%。
