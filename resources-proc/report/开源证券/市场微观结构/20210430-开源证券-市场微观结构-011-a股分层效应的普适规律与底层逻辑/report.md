...
本节，我们用x代表振幅水平；F(x)代表综合影响； $f_{A}(x)$ 代表因素A 的影响；$f_{B}({\boldsymbol{x}})$ 代表因素B的影响；则：

$$
F(\mathbf{x})=f_{A}(x)+f_{B}(x),\ (x_{0}\leq x\leq x_{1});
$$

假设： $\begin{array}{r}{f_{A}(x)=\frac{1}{2}ax^{2}+bx;}\end{array}$

由于在振幅最低水平与振幅最高水平，基本面因子有效性的边际变化完全由因素A的边际变化决定，则可得条件：

$$
\operatorname*{lim}_{xx_{0}}F^{\prime}(x)=\ \operatorname*{lim}_{xx_{0}}{f_{A}}^{\prime}(x)\ ;
$$

$$
\operatorname*{lim}_{x\to x_{1}}F^{\prime}(x)=\ \operatorname*{lim}_{x\to x_{1}}{f_{A}}^{\prime}(x)\ ;
$$

$$
{f_{A}}^{\prime\prime}(x)=a={\frac{F^{\prime}(x_{1})-F^{\prime}(x_{0})}{x_{1}-x_{0}}};
$$
...
$$
{f_{A}}^{\prime}(x_{0})=ax_{0}+b=F^{\prime}(x_{0})
$$
