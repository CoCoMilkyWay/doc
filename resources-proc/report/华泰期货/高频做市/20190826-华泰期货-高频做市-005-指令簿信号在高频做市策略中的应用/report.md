...
其中 $\mathbb{V}_{t}^{b}$ 和 $\mathsf{V}_{t}^{a}$ 是t时刻在买一和卖一价位上所挂限价单的数量。
...
为了挖掘挂单不平衡信号的预测效果，这里把该信号分成 3 个区间。当 $\rho_{t}\in[-1,-{\textstyle\frac{1}{3}})$ 时，属于强卖区；当 $\rho_{t}\in[-\textstyle{\frac{1}{3}},\textstyle{\frac{1}{3}}]$ 时，属于中性区；当 $\begin{array}{r}{\rho_{t}\in(\frac{1}{3},}\end{array}$ 1]时，属于强买区。在这三个区间下可以定义两类事件，第一个事件是500毫秒截面数据中的(市价)卖(单)量<(市价)买(单)量，第二个事件是 500 毫秒截面数据中的(市价)卖(单)量>(市价)买(单)量。
...
CDJ 模型的参数涉及出现各类市场事件的密度、期望收益以及之后市场状态的转换概率，参数估计过程非常直接。假设 $N_{J,K}^{i}$ 表示市场状态在出现i事件后从市场状态J转换到市场状态K的次数，则限价单事件后的转换概率 $\cdot T_{J,K}^{l}$ 可以表示为

$$
T_{J,K}^{l}=\frac{N_{J,K}^{l}+N_{\widetilde{J},\widetilde{K}}^{l}}{N_{J}^{l}+N_{\widetilde{J}}^{l}}\tag{3}
$$

两类市价单事件后的转换概率 $\cdot T_{J,K}^{\pm}$ 可以表示为

$$
T_{J,K}^{\pm}=\frac{N_{J,K}^{\pm}+N_{\widetilde{J},\widetilde{K}}^{\mp}}{N_{J}^{\pm}+N_{\widetilde{J}}^{\mp}}\tag{4}
$$

限价单事件l在市场状态J下出现的密度 $\lambda^{l}(J)$ 可以表示为

$$
\lambda^{l}({\pmb J})=\frac{N_{J}^{l}+N_{\widetilde J}^{l}}{\tau_{J}+\tau_{\widetilde J}}\tag{5}
$$

市价单事件±在市场状态J下出现的密度 $\lambda^{\pm}(J)$ 可以表示为

$$
\lambda^{\pm}({\pmb J})=\frac{N_{J}^{\pm}+N_{\widetilde J}^{\mp}}{\tau_{J}+\tau_{\widetilde J}}\tag{6}
$$

限价单事件l在市场状态J下出现后的中间价变化可以表示为

$$
\epsilon^{l}(\pmb{J})=\frac{\sum_{k=1}^{N_{\pmb{J}}^{l}}y_{\pmb{J}}^{lk}-\sum_{k=1}^{N_{\pmb{\tilde{J}}}^{l}}y_{\pmb{\tilde{J}}}^{lk}}{N_{\pmb{J}}^{l}+N_{\pmb{\tilde{J}}}^{l}}\tag{7}
$$
...
$$
\epsilon^{\pm}(J)=\frac{\sum_{k=1}^{N_{J}^{\pm}}y_{J}^{\pm k}-\sum_{k=1}^{N_{\bar{J}}^{\mp}}y_{\bar{J}}^{\mp k}}{N_{J}^{\pm}+N_{\bar{J}}^{\mp}}.\tag{8}
$$
...
$$
H(t,x,q,S,{\pmb J})=\operatorname*{sup}_{\left(\gamma_{s}^{\pm}\right)_{t\leq s\leq T}\in A}\mathbb{E}\left[X_{T}+q_{T}S_{T}-|q_{T}|\frac{\Delta}{2}-\phi\int_{0}^{T}q_{u}^{2}du|\mathcal{F}_{t}\right]\tag{9}
$$
...
$$
H(t,x,q,S,{\pmb J})=x+qS+h(t,q,{\pmb J})\tag{10}
$$

这个分解可以比较直观地理解为未来最优策略的选取与已经取得的收益x无关，S是一个鞅过程，期望值为 $0,$ ，所以x和qS可以从H(t, x, q, S,J)中分解出去。

利用动态规划原理φi(t, y)可以通过以下 Hamilton–Jacobi–Bellman(HJB)偏微分方程求得

$$
\begin{align*}\frac{\partial h}{\partial t}-\phi q^2+\lambda^l(\pmb{J})\left(q\epsilon^l(\pmb{J})+\sum^l(t,q,\pmb{J})\right)\quad&\\+\sup_{\gamma^+\in[0,1]}\lambda^+(\pmb{J})\left(\gamma^+\frac{\Delta}{2}+(q-\gamma^+)\epsilon^+(\pmb{J})+\sum_{\gamma^+}^+(t,q,\pmb{J})\right)\quad&\\+\sup_{\gamma^-\in[0,1]}\lambda^-(\pmb{J})\left(\gamma^-\frac{\Delta}{2}-(q+\gamma^-)\epsilon^-(\pmb{J})+\sum_{\gamma^-}^-(t,q,\pmb{J})\right)=0.&\end{align*}\tag{11}
$$

其中

$$
\begin{array}{rlr}&{}&{\displaystyle\sum^{l}(t,q,{\pmb J})=\sum_{{\pmb K}}\bigl(h(t,q,{\pmb K})-h(t,q,{\pmb J})\bigr)T_{J,{\pmb K}}^{l}}\\&{}&{\displaystyle\sum_{{\pmb\gamma}^{+}}(t,q,{\pmb J})=\sum_{{\pmb K}}\bigl(h(t,q-{\pmb\gamma}^{+},{\pmb K})-h(t,q,{\pmb J})\bigr)T_{J,{\pmb K}}^{+}}\\&{}&{\displaystyle\sum_{{\pmb\gamma}^{-}}(t,q,{\pmb J})=\sum_{{\pmb K}}\bigl(h(t,q+{\pmb\gamma}^{-},{\pmb K})-h(t,q,{\pmb J})\bigr)T_{J,{\pmb K}}^{-}}\end{array}
$$
...
（3） 最低市价卖单成交价 $-Y_{s}$ 等于所挂买单价B，则根据把当前限价买单队列排名减去卖单成交量从而得到新的买单排名，如果这个新排名小于 0，则判断买单成交，否则继续挂单等待。
