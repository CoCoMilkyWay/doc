...
记 t 时刻, n 只股票收益率记为 $\pmb{x_{t}}=\left(x_{1,t},x_{2,t},\cdots x_{n,t}\right)^{T}$ ，常用因子模型假设

$$
x_{i,t}=\lambda_{\mathrm{i}}^{T}\cdot{\pmb f}_{t}+\epsilon_{i,t}
$$

其中 $\mathbf{\Psi}^{\prime}f_{t}=\left(f_{1,t},f_{2,t},\cdots,f_{r,t}\right)^{T}$ 是 r 个风险因子在 t 时刻的取值， $\lambda_{i}=(\lambda_{1},\lambda_{2},\cdots,\lambda_{r})^{T}$ 是 n个股票在r个风险因子上的风险因子暴露， $\scriptstyle f_{t}$ 需满足一些正则性条件来保证参数估计唯一性。如果风险因子r 已知，那么可以用 n 只股票收益率序列的前 r 个主成分作为 $\scriptstyle|f_{t}$ 的估计值，每一个主成分都是全市场股票收益率的线性组合，在一定模型假设下，可以证明主成分分析（PCA）获得的风险因子和因子暴露是上述模型的一致估计（Bai (2003)）。
