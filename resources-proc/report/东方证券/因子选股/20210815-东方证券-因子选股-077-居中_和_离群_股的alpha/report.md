...
$$
\tilde{C}_{1}=W\tilde{\varLambda}W^{\prime}
$$

$$
C_{1}=\tilde{C}_{1}\left[\left(diag\left[\tilde{C}_{1}\right]\right)^{\frac{1}{2}}\left(diag\left[\tilde{C}_{1}\right]\right)^{\frac{1^{\prime}}{2}}\right]^{-1}
$$
...
相似的算法比如斐波那契数列的变换矩阵 $\begin{bmatrix}1&1\\1&0\end{bmatrix}$ 的第一大特征值 1.618，其对应的特征向量为 $^{[0.851]}_{[0.526]}$ ，多次迭代之后两相邻元素的比例无限靠近此特征向量的比值。
