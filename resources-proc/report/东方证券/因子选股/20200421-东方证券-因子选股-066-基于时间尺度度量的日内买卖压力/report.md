...
其中， $RPP_{i,\imath}$ t表示股票 i 在某一时间区间内的时刻 t 的相对价格位置， $H_{i}$ 和 $\lvert L_{i}$ 为该时间区间内股票 i 的最高价和最低价。
...
$$
ARPP_{i}=\intop_{0}^{T}RPP_{i,t}\cdot dt
$$

由于 $RPP_{i,t}$ 定义中涉及的最高价 $H_{i}.$ 、最低价 $\cdot L_{i}$ 与具体时刻 t 无关，在时间区间 0 到 T 之间是恒定的，所以上述积分可以写成如下形式：

$$
ARPP_{i,t}=\frac{\left(\int_{0}^{T}P_{i,t}\cdot dt-L_{i}\right)}{\left(H_{i}-L_{i}\right)}
$$

其中， $\int_{0}^{T}P_{i,t}$ ∙dt表示股票 i 在时间区间 0 到 T 内的时间加权平均价格，即 TWAP， $H_{i}$ 和 $\textstyle|L_{i}$ 为时间区间 0 到 T内的最高价、最低价。由于一般的日行情数据不提供股票的 TWAP 价格，所以实际计算中我们采用分钟线高开低收均值在时间序列上的平均值作为区间的 TWAP。
