...
$$
\max_{w_{i}}\left(\sum_{i=1}^{N}w_{i}*\mu_{i}-0.5*fee*\sum_{i=1}^{N}\left|w_{i}-w_{i,0}\right|\right)
$$

其中， $w_{\mathrm{i}}$ 为组合中股票 i的权重， $\mathsf{w}_{\mathsf{i},0}$ 为组合中股票 i的初始权重， $\mu_{\mathrm{i}}$ 为股票 i的预期超额收益；fee作为惩罚系数，代表交易成本。为了使本文的结论贴近实践，如无特别说明，下文的测算均假定以次日均价调仓，同时扣除 3‰的交易成本。
