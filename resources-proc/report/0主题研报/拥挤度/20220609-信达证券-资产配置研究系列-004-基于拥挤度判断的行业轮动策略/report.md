...
$$
F_{i,t}=\frac{closeprice_{i,t}}{closeprice_{i,t-win_{-}len+1}}-1
$$

其中， $F_{i,t}$ 为第i个行业在t时刻的动量因子，满足 $\begin{array}{r}{.i=1{,}2,\cdots,N,\quad t=win\_len,\cdots,T}\end{array}$ ；N为截面行业个数；T为回测时段总天数； $ClosePrice_{i,t}$ 为第i个行业在t时刻的收盘价；win_len为计算累计收益率时使用的回看窗口的天数。
...
$$
C_{i}=\frac{\sum_{j=1}^{n}\left(AR^{j}\cdot\frac{|EV_{i}^{j}|}{\sum_{k=1}^{N}|EV_{k}^{j}|}\right)}{\sum_{j=1}^{n}AR^{j}}.
$$
...
（1）行业拥挤度分散度因子SDC：定义 $.C_{i,t}$ 为第i个行业在时刻t的资产集中度， $mean(C_{i,t})$ 为t时刻截面所有行业资产集中度的均值，N为截面行业个数。当期行业拥挤度分散度因子 ${\cdot}SDC_{t}$ 定义为资产集中度 $\cdot C_{i,t}$ 截面标准差的相反数，截面所有行业的拥挤度分化越小，说明当前市场越拥挤。
