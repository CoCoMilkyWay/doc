...
$$
\tilde{x}=\left\{\begin{array}{cc}{{x_{m}+n\cdot D,\qquad}}&{{if\ x>x_{m}+n\cdot D}}\\{{x_{m}-n\cdot D,}}&{{if\ x<x_{m}-n\cdot D}}\\{{x,}}&{{else}}\end{array},\right.\tag{1}
$$

其中，x是任意一个财务因子的值， $x_{m}$ 是因子值在横截面上的中位数，D是序列|x−$\mathbf{x}_{\mathrm{m}}|$ 的中位数，n是一个参数，本文中取3，而x̃为去极值后的结果。
...
其中， $\varepsilon_{T}$ 为股票的特质收益率，b为回归得到的系数。通常，基于风格因子的线性回归对收益的解释力度较低，即 $\varepsilon_{T}$ 为股票收益R的绝大部分。将有k个财务因子的矩阵记为 $X_{Tk}$ ，随机森林的输入为 $B_{T}\mathcal{\hat{\ H}}\mathbf{\Pi}X_{Tk}$ ，输出为 $\varepsilon_{T}$ ，用训练集数据训练一个随机森林模型。

接下来，将拟合得到的随机森林模型作用于验证集的风格和财务因子上，得到验证集的各个季度的预测值 $P_{1},P_{2},\ldots,P_{n_{v}}$ ，对应地，各期的实际特质收益率为$\varepsilon_{1},\varepsilon_{2},\ldots,\varepsilon_{n_{v}}.$ 。对验证集的各个季度，计算秩相关系数：

$$
\begin{array}{r}{\rho_{i}=\rho(P_{i},\varepsilon_{i}),\qquad i=1,2,\ldots,n_{v},}\end{array}\tag{3}
$$

其中， $\rho(\cdot,\cdot)$ 为计算秩相关系数的函数。因子池 $\scriptstyle\left\{B_{T},X_{Tk}\right\}$ 的最终得分即是验证集各期相关系数的代数平均数。
...
$$
F_{0}=\sum_{i}s_{i}\frac{x_{i}}{\sigma_{xi}},\tag{6}
$$
...
我们以验证集的三个季度的多头分位数的平均值λ̅作为模型的评价指标，该指标越大，说明模型在历史上有越好、越稳定的表现。对于任意一个模型 $\mathsf{M}_{i}(n)$ ，其中i代表模型的序号，n代表模型的输入因子数（例如，如果输入10个风格因子和1个财务因子，则 $n=11)$ ），那么 ${\bar{\lambda}}_{i}$ 是n的一个函数，我们找到使得 $\cdot\bar{\lambda}_{i}$ 最大的输入因子数 $n_{i}$ ，即：

$$
n_{i}=\underset{n}{\mathrm{argmax}}\ :\bar{\lambda}_{i}(n).\tag{7}
$$

我们把模型 $\mathbf{M}_{i}$ 在验证集上最大的多头分位数记为 $\hat{\lambda}_{i}$ ，即 $\hat{\lambda}_{i}=\bar{\lambda}_{i}(n_{i})$ ，如果该数值大于 0.5，则入选股票的表现总体上强于市场，如果数值小于 0.5，则入选股票总体上弱于市场。如果选股因子是一个噪音，那么 $\hat{\lambda}_{i}$ 的数值应该很接近0.5。基于此，我们定义模型 $\mathsf{M}_{i}$ 的权重 $w_{i}$ 为：

$$
w_{i}=\operatorname*{max}\bigl(\hat{\lambda}_{i}-0.5,0\bigr).\tag{8}
$$
...
$$
F=\sum_{i}w_{i}\frac{f_{i}}{\sigma_{i}},\tag{9}
$$

其中， $f_{i}$ 为模型 $\mathsf{M}_{i}$ 预测的因子值， $\sigma_{i}$ 为因子值的标准差。我们使用众多机器学习模型，对于这些模型中“差”的机器学习模型，有两种可能性：第一，其 $\cdot\hat{\lambda}_{i}$ 不超过0.5，则其权重为零，不对结果F产生影响；第二，其 $\hat{\lambda}_{i}$ 仅略高于0.5，则其权重非常低，对结果影响微乎其微。因此，因子值F对于低质量的模型并不敏感。
