...
$$
\mathrm{zscore(((rank(amount)\mathbf{t}s\_mean(amount,20)))}+\mathrm{rank(cashflow\_to\ profit))\cdot\mathrm{rank(capex\_to\_assets))})}
$$
...
这一定位决定了框架不宜把目标函数简单写成“最大化样本内 $\mathrm{IC}^{\prime\prime}$ 。在实际研究中，样本内 IC 较高但换手过高、表达式过深、字段含义混乱或与已有因子高度重合的候选，并不具备稳定的投资价值。相反，真正值得保留的因子通常是在预测能力、稳健性、可解释性、低冗余和可交易性之间取得相对均衡。因此，本文采用的是“受约束搜索、反馈迭代、两阶段评估、多目标排序、相关性入库”的组合式算法框架。
...
$$
S^{*}=\{e\in S\colon D(e)=D^{*},C_{dim}(e)=1,C_{sem}(e)=1,C_{time}(e)=1,K(e)\leq K_{max}\}
$$
...
$$
\displaystyle{\mathop{\mathrm{m}}_{e\in S^{*}}}(IC(e),-Turnover(e),-Complexity(e),-Redundancy(e),Robustness(e))
$$
