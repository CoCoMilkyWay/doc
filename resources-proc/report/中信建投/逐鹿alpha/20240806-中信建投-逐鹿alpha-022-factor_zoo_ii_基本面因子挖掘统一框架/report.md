...
![](images/4e170d8c88bff8f62f5d061e6b2aade5382535600df18882cce05d5f8e9f6f4b.webp)
...
图 1: AutoML-Zero 算法
![](images/c79dad62e2b1f7afe4e2ea26f1743d095a676a82b70f6ffa9a28ebb56acea57a.webp)
...
图 2: OpenFE 结构
![](images/85e51627329b7a6ab7496d8b06ee424b66d633790b1216d66a7fad0b9e01e6d7.webp)
...
图 3:领域知识因子挖掘框架
![](images/27691598b1080a6ce6e046a0cf0583b5d10a117249dd2c36bba43764cb585558.webp)
数据来源：中信建投证券
...
图 4:因子挖掘框架
![](images/e4b84cbe3150a46574b7bdf024e2e68d929cfddbb6c7b4fb832920fba28ef412.webp)
数据来源：中信建投证券
...
数据，而分母端的“P”代表日频数据。在处理不同频率的数据时，我们会首先对低频数据按照信息发布日期进行隐式的频率转换（ ），将其转换为高频数据，然后再与其他高频数据进行计算。
...
| div(m1, m2) | ml/m2 | 输入相同量纲，输出无量纲 |
| mul(m1, m2) | m1*m2 | 输入为不同量纲，输出为带量纲 |
| 时间序列运算符 |  |  |
| ts_mean(m,v) | 过去v期m的平均值 | 输出不改变量纲 |
| ts_std(m,v) | 过去v期m的标准差 | 输出不改变量纲 |
| ts_delay(m,v) | m滞后v期 | 输出不改变量纲 |
| ts_delta(m,v) | m与过去v期的差值 | 输出不改变量纲 |
| ts_pct(m,v) | m与过去v期的变化率 | 输出无量纲 |
| ts_max(m,v) | 过去v期m的最大值 | 输出不改变量纲 |
| ts_min(m,v) | 过去v期m的最小值 | 输出不改变量纲 |
| ts_min_max_diff(m,v) | 过去v期m的最大值与最 小值的差 | 输出不改变量纲 |
| yoy (m) | m的同比值 | 输出无量纲 |
| qoq(m) | m的环比值 | 输出无量纲 |
| ttm(m) | m的 TTM 值 | 输出不改变量纲 |
| ts_slope (m,v) | 过去v期m的斜率 | 输出不改变量纲 |
| ts_resi(m,v) | 过去v期m的残差 | 输出不改变量纲 |
| ts_rsquare (m,v) | 过去v期m的r2 | 输出无量纲 |
| ts_regres sion_slope (m1,m2,v) | 过去 v 期 m1 对 m2 回归 的斜率 | 输入相同量纲，输出无量纲 |
| 金融工程深度报告 |  |  |
| ts_regression_resi(m1,m2,v) | 过去 v 期 m1 对 m2 回归 的残差 | 输入相同量纲，输出不改变量纲 |
| ts_regression_rsquare (m1,m2,v) | 过去 v 期 m1 对 m2 回归 的r2 | 输入相同量纲，输出无量纲 |
| 横截面运算符 |  |  |
| cs_norm(m) | m的横截面标准化 | 输出无量纲 |
| cs_rank(m) | m的横截面排序 | 输出无量纲 |
...
以 ts_regression_slope 算子为例，如果使用传统的 scipy.stats.linregress 函数，对一只股票两个因子间进行 1000次 T 为 12 的滚动回归，耗时约 8.15 秒，采用 cython+流式计算技术，耗时约 0.07 秒，效率提升 116 倍。其中cython 带来的提升约为9.6倍，流式计算带来的提升约为 12倍。函数的核心定义如下：
...
![](images/325be9a8df71e59c23ab71b23052ec3260749db9e01bab18e19417ea5703e165.webp)
图 6:树状表达式
数据来源：中信建投证券
金融工程深度报告
图 7:逆波兰表达式（C）
Generating Synergistic Formulaic Alpha Collections via Reinforcement Learning， 中信建投证券
...
最后再将候选因子池与现有的因子池进行对比，计算它们之间的两两相关性。如果在候选因子中存在与现有因子池中因子相关性较低的新因子，这些新因子将被纳入现有的因子池。如果候选因子与现有因子池中的因子存在高相关性（超过 0.6），并且候选因子的表现优于现有因子，则进行替换；否则保持现有因子池不变，不做任何调整。
...
图 9:因子一 IC
![](images/642df16f5bafe26064be84b5bbff312d227492e977a30fe6852d2a74e1165a03.webp)
数据来源：WIND，中信建投证券

图 10:因子一分组收益
![](images/0394f59fa0a3c29d926e80560c015a04f34b27ae144c40bc6530624c09a6dfba.webp)
数据来源：WIND，中信建投证券

## 因子二：

ts_regression_resi(add(net_cash_flows_oper_act,s_stm_bs)/ttm(s_fa_fcff),add(div(tot_shrhldr_eqy_excl_min_int, val_mv), qoq(add(tot_oper_cost2,oper_rev))), 4)

因子解释：4 期回归残差（（经营活动产生的现金流量净额+固定资产合计）/TTM（企业自由现金流量（FCFF）），（股东权益合计（不含少数股东权益） 总市值 环比（营业总成本 营业总收入））
...
图 11:因子二 IC
![](images/8f914088537921e0cf4cf37358d6d9c19629af78ce3be9c542cdc052e8350fb0.webp)
数据来源：WIND，中信建投证券

图 12:因子二分组收益
![](images/4f1db861b4793b120774af15ada259cd969f064046fe7e6f2719ee53eecb53e7.webp)
数据来源：WIND，中信建投证券
因子三：div(ts_mean(surplus_rsrv, 8), val_mv)
...
图 13:因子三 IC
![](images/a7dc6d115fd9e9d8d7d6396f568f879f8cdc92f495e97f8c4204dbd0dde3e34c.webp)
数据来源：WIND,中信建投证券
金融工程深度报告

图 14:因子三分组收益
![](images/f5a37619723feac136f1de43a2cdf3858e92e41bd9a84a139b436a895085bcbd.webp)
数据来源：WIND，中信建投证券
