...
那么如何刻画一个因子分层效应的强弱呢？假设第i个因子作为分层因子 $F_{i},$ ，基于 $F_{i}$ 我们对每个Alpha因子fj测算其不同样本下的有效性，ICj代表Alpha因子fj因子在全样本中的有效性， ${IC_{ij}}^{+}$ 代表Alpha因子fj在高Fi样本中的有效性， $|IC_{ij}^{-}|$ 代表Alpha因子fj因子在低 $\boldsymbol{F}_{i}$ 样本中的有效性。
...
$$
f_{i}=\overline{F_{i}}\times C\times\overline{d_{i}}\tag{3}
$$

$\overline{{F}}_{i}^{\mathrm{{'}^{\prime}}}$ 个股 的原始因子暴露向量；i:

分层因子情景矩阵；C:

$\bar{d}_{i}$ 个股 的综合特征向量。i:
...
$$
S_{_A}=\begin{bmatrix}2.5&1&-2.1\end{bmatrix}\times\begin{bmatrix}14\%&48\%&34\%&21\%\\65\%&37\%&54\%&58\%\\22\%&15\%&12\%&50\%\end{bmatrix}\times\begin{bmatrix}50\%\\0\%\\0\%\\50\%\end{bmatrix}=0.59
$$

股票B 的情景得分为：

$$
S_{_B}=\begin{bmatrix}2.5&1&-2.1\end{bmatrix}\times\begin{bmatrix}14\%&48\%&34\%&21\%\\65\%&37\%&54\%&58\%\\22\%&15\%&12\%&50\%\end{bmatrix}\times\begin{bmatrix}0\%\\10\%\\90\%\\0\%\end{bmatrix}=1.14
$$
