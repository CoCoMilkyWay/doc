...
$$
s(t)=\sum_{i=1,2,\ldots n}IMF_{i}(t)+r_{n}(t)\tag{1}
$$
...
$$
h_{1}(t)=s(t)-m_{1}(t)\tag{2}
$$

步骤 5：检查 $h_{1}(t)$ 是否符合 IMF的条件。如果不符合，则回到步骤 1并且将 $h_{1}(t)$ 当作原始讯号，进行再次的筛选。亦即

$$
\begin{aligned}&h_{2}(t)=h_{1}(t)-m_{2}(t)\\&\cdots\cdots\\&h_{k}(t)=h_{k-1}(t)-m_{k}(t)\\\end{aligned}\tag{3}
$$
...
$$
R=\ln\frac{\mathrm{std}\{s(t)-r_n(t)\}}{\mathrm{std}\{r_n(t)\}}\tag{6}
$$
...
我们的目标是 $浐$ 深300股指期货的日内交易。假设在上午 $t_{s}$ 时刻考虑建仓，我们
...
$$
Signal=\left\{\begin{aligned}1&\quad if\quad p(t_s)>p(t_o)\\-1&\quad if\quad p(t_s)<p(t_o)\end{aligned}\right.\tag{8}
$$
...
$$
r_{long}=\frac{F_{2}(1-c)-F_{1}(1+c)}{F_{1}(1+c)}\times100\%\tag{9}
$$

$$
r_{short}=\frac{F_1(1-c)-F_2(1+c)}{F_1(1+c)}\times100\%\tag{10}
$$
...
$$
\left|p(t_s)-p(t_0)\right|>Threshold\tag{11}
$$

和 EMDT相似，根据 $p(t_{s})$ 和 $p(t_{o})$ 的价格大小确定建仓方向，顺势做多或者做空。为了使两者更具有可比性，我们考虑两种比较模式。第一种是将Threshold设置为0，直接比较开盘价格和建仓时价格的大小，进行做多或做空（基本上每天都交易，除非建仓时价格和开盘价格一样）；第二种模式是通过调整普通趋势投资建仓条件判断阈值Threshold的大小，使得该策略的交易频率和EMDT策略基本一致（日均交易次数约为0.5次）。对于第二种模式，我们确定了Threshold为股指期货5.5个指数点。
