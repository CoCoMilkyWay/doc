...
金融市场中，信息的不平衡特性体现在两个方面：信息的非及时扩散和信息的过度反应。这两种不平衡环境造就了价格的四种趋势：加速上涨与加速下跌（对应信息的非及时扩散），以及减速上涨与减速下跌（对应信息的过度反应）。这四种情况可以通过对离散的价格时间序列进行低阶多项式拟合，观察一次拟合函数的一阶导数f’及二次拟合函数的二阶导数 g’’进行界定。当 ${\mathfrak{f}}^{\prime}{\mathfrak{g}}^{\prime\ }{>}0$ 时，代表加速上涨与加速下跌行为，此时反转风险相对较小，可顺势建仓；当 $\boldsymbol{\mathrm{f^{\prime}\ g^{\prime\prime}}}{<}0$ 时，代表减速上涨与减速下跌行为，反转风险较大，不进行交易。
...
通过一阶多项式拟合，我们可以对价格的基本趋势做出判断，但更重要的是我们还要对趋势的变化情况做出界定，即需要研究如图2中由于金融市场的信息不平衡特性所带来的趋势加速或减速的情况。这点可以通过二阶多项式拟合完成。同样是对该段时间的价格序列 $\{p_{t}\}$ 进行二次拟合，拟合的目标函数形式为 $y_{2}=a_{2}t^{2}+b_{2}t+c_{2}$ 。当$\frac{d^{2}y_{2}}{dt^{2}}>0$ 时，价格曲线为凹；反之 $\frac{d^{2}y_{2}}{dt^{2}}<0$ 时，价格曲线为凸；结合一阶多项式拟合的结果，图2的四种情况分别可以用如下量化结果进行描述：

1、图2（a），加速上涨： $\frac{dy_{1}}{dt}>0\ :\ :\ :\frac{d^{2}y_{2}}{\Xi}>0$

2、图2（b），减速上涨： $\frac{dy_{1}}{dt}>0\mathrm{~}\frac{d^{2}y_{2}}{\mathrm{~\textbar{~}}dt^{2}}<0$

3、图2（c），加速下跌： $\frac{dy_{1}}{dt}<0\mathrm{~}\frac{d^{2}y_{2}}{\mathrm{~\textmu~}}<0$

4、图2（d），减速下跌： $\frac{dy_{1}}{dt}<0\frac{d^{2}y_{2}}{\textmd{E}}>0$我们交易策略的基本思路是，在第1种和第3种情况出现时，即时，对股指期货进行顺势建仓，获得趋势性价差收益；关于平仓条件，我们会在后面的实证模拟中详述。
...
假设用 $EF_{1}$ 数 $y^{*}{}=f(x)$ 拟合某组一维离散数据，它在离散点 $x_{i}$ 处与观察值 $y_{i\geq}$ 差
...
$$
\sum_{i}\lvert\delta_{i}\rvert=\mathrm{min}
$$

（2）使残差的最大绝对值最小，即

$$
\operatorname*{max}\mid\delta_{i}\mid=\operatorname*{min}
$$
...
具体来说，数据拟合的最小二乘问题是：根据给定的数据组 $\left(x_{i},y_{i}\right)_{\left(\mathsf{i}=1,2,\ldots,\mathsf{n}\right)}$ 选取近似函数形式，即给定函数类H，求函数 $\varphi(x)\in H$ ，使得

$$
\sum_{i=1}^{n}{\delta_{i}^{2}}=\sum_{i=1}^{n}\bigl[y_{i}-\varphi(x_{i})\bigr]^{2}
$$

为最小，即

$$
\sum_{i=1}^{n}\left[y_{i}-\varphi(x_{i})\right]^{2}=\operatorname*{min}_{\varphi\in H}\sum_{i=1}^{n}\left[y_{i}-\varphi(x_{i})\right]^{2}
$$
...
$$
\sum_{i=1}^{n}\delta_{i}^{2}=\sum_{i=1}^{n}\bigl[y_{i}-P_{m}(x_{i})\bigr]^{2}=F(a_{0},a_{1},\cdots,a_{m})
$$

最小，即选取参数 ${}^{a_{i}}(\mathfrak{i=}0,1,\ldots,\mathfrak{m})$ ，使得

$$
F(a_{0},a_{1},\cdots,a_{m})=\operatorname*{min}_{\varphi\in H}\sum_{i=1}^{n}\bigl[y_{i}-\varphi(x_{i})\bigr]^{2}
$$

这就是数据的多项式拟合， $P_{m}(x_{i})$ 称为这组数据的最小二乘m次多项式拟合，在我们下面的交易策略中，m=1或2。

对于多项式系数 $\{a_{0},a_{1},\cdots,a_{m}\}$ ，可以通过求解多元函数极值来得到，即求解方程组

$$
\frac{\partial F}{\partial a_{j}}=-2\sum_{i=1}^{n}[y_{i}-\sum_{k=0}^{m}a_{k}x_{i}^{k}]x_{i}^{j}=0
$$

$$
(\mathsf{j}{=}0,1,...,\mathsf{m})
$$

即

$$
\left\{\begin{array}{c}{{na_{0}+a_{1}\displaystyle\sum_{i=1}^{n}x_{i}+a_{2}\displaystyle\sum_{i=1}^{n}x_{i}^{2}+\cdots+a_{m}\displaystyle\sum_{i=1}^{n}x_{i}^{m}=\sum_{i=1}^{n}y_{i}}}\\{{a_{0}\displaystyle\sum_{i=1}^{n}x_{i}+a_{1}\displaystyle\sum_{i=1}^{n}x_{i}^{2}+a_{2}\displaystyle\sum_{i=1}^{n}x_{i}^{3}+\cdots+a_{m}\displaystyle\sum_{i=1}^{n}x_{i}^{m+1}=\sum_{i=1}^{n}y_{i}x_{i}}}\\{{\ldots\ldots\ldots}}\\{{a_{0}\displaystyle\sum_{i=1}^{n}x_{i}^{m}+a_{1}\displaystyle\sum_{i=1}^{n}x_{i}^{m+1}+a_{2}\displaystyle\sum_{i=1}^{n}x_{i}^{m+2}+\cdots+a_{m}\displaystyle\sum_{i=1}^{n}x_{i}^{2m}=\sum_{i=1}^{n}y_{i}x_{i}^{m}}}\end{array}\right.
$$

这是最小二乘拟合多项式的系数 ${}^{a_{k}}\left(\mathsf{k}{=}0,1,\ldots,\mathsf{m}\right)$ 应满足的方程组，称为正则方程组。由函数组 $\vert1,x,x^{2},\cdots,x^{m}\ \vert$ 的线性无关性可以证明，上述方程组存在唯一解，且解所对应的多项式必定是已给定数据 $(x_{i},y_{i})_{(\mathsf{i}=0,1,\ldots,\mathsf{n})}$ 的最小二乘m次拟合多项式。
...
$$
\underline{{dy_{1}}}\qquad\underline{{d^{2}y_{2}}}
$$
...
始）。在趋势交易中，该种情况需要进行平仓。另外一种情况是在建仓后 $dt$ 的 $\underline{{\boldsymbol{\pi}}}$ 负号
...
$$
\frac{dy_{1}}{dt}\textcircled{d}y_{2}
$$
...
$$
dy_{1}\quad\underline{{{d}}}^{2}y_{2}
$$

对比表2和表1，可以看出，在加入 $\textit{ \textbf { 6 } }dt$ 或 $dt^{2}$ 正负号发生变化”的平仓条件后，累积收益率由56.7%减小至47.1%，绝对值减小了9.6%；但同时最大回撤由32.9%减小至17.2%，绝对值减小了15.7%。总体来看，加入上述平仓条件后，在收益率小幅减小的情况下，交易的最大回撤大幅下降，有效起到了控制交易风险的作用。图5显示了几个采用上述策略进行建仓与平仓的实例。经过统计，我们发现如图5中几个实例一样，
