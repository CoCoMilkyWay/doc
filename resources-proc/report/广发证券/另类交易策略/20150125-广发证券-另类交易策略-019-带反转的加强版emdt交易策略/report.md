...
$$
R=\ln\frac{\mathrm{std}\{s(t)-r_{n}(t)\}}{\mathrm{std}\{r_{n}(t)\}}\tag{1}
$$
...
$$
\scriptstyle Signal=\left\{{\begin{array}{ll}{1}&{{\mathrm{if}}\quad p(t_{s})>\mathbf{p}(t_{o})}\\{-1}&{{\mathrm{if}}\quad p(t_{s})<\mathbf{p}(t_{o})}\end{array}}\right.\tag{2}
$$
...
$$
s(t)=\sum_{i=1,2,...n}IMF_{i}(t)+r_{n}(t)\tag{3}
$$
...
样本内数据：2010年至 2011年行情数据。我们先用 2010年数据和 2011年数据计算出最优判断阈值 $Threshold2=\lambda_{opt}R_{\mathrm{mean}}$
