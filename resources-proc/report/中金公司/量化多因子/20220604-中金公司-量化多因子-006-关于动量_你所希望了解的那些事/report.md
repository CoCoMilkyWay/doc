...
$$
SUE_{T}=(Earning_{T}-\mathbb{E}(Earning_{T}))/\sigma
$$
...
$$
momentum_{T}^{i}=return\_series_{T}^{i}\cdot weig\&\_mask\_series_{T}^{i}
$$
...
一年动量（MOM_1Y）的收益率项同样为 $[r_{T-240}^{c},~r_{T-239}^{c},\dots,r_{T-21}^{c},~r_{T-20}^{c},\dots,r_{T-1}^{c}]$ ，与一个月动量的收益率项一致；但一年动量的权重序列为长度为240的序列 $[1,1,\ldots,1,0,\ldots,0]$ ，其中前220个元素均为1，最后20个元素均为0。
...
| --- | --- | --- | --- | --- | --- |
| 计算方式 | Closeτ/Closer-1-1 | Openτ/Openτ-1-1 | Vwapτ/Vwapτ-1-1 | $\overline{Close_{T}}/\overline{Open_{T}}-1$ | $Open_{T}/Close_{T-1}-1$ |
...
剔除涨跌停收益的一年动量因子，其权重序列为 $[m_{T-239},m_{T-238},\ldots,m_{T-20},0,\ldots,0]$ ，其中 $m_{t}$ 根据股票在t日的涨跌停状态取值，若非涨跌停则取值1，否则取值0。

日内波动调整的一个月反转因子，其权重序列为 $\left[0,0,\ldots,0,m_{T-19},\ldots,m_{T}\right]$ ，其中，最后20 个因素 $m_{t}$ 根据股票在t日的日内波动率大小取值，若t日的股票日内波动率高于[T-19,T区间股票日内波动率样本的80%分位数则取值1，若低于20%分位数则取值-1，其余情况取值0。
...
权重序列： $[m^{i},m^{i},\ldots,m^{i},0,\ldots,0]$ ，其中前220个元素均为 $m^{i}$ ，最后20个元素均为$0,m^{i}$ 表示股票i在前220交易日的波动率的倒数。
...
权重序列： $\left[1,1,\ldots,1,0,\ldots,0\right]$ ，其中前220个元素均为1，最后20个元素均为 $0.$
...
权重序列： $[0,0,\ldots,0,m_{T-19},\ldots,m_{T}]$ ，其中前220个元素均为0，最后20个因素 $m_{t}$ 根据股票在t日的日度振幅大小取值，若t日的股票日度振幅高于[T-19,T] 区间股票日度振幅样本的80%分位数则取值1，若低于20%分位数则取值-1，其余情况取值0。
...
权重序列： $[m_{T-239},~m_{T-238},\ldots,m_{T-20},m_{T-19},\ldots,m_{T}]$ ，其中 $m_{t}$ 根据股票在t日的日度振幅大小取值，若t日的股票日度振幅高于[T-239，T] 区间股票日度振幅样本的80%分位数则取值-1，若低于20%分位数则取值1，其余情况取值0。
...
权重序列： $[m_{T-239},m_{T-238},\ldots,m_{T-20},m_{T-19},\ldots,m_{T}]$ ，其中，假设t日为 [T-239,T-1]区间内股票最近一个业绩报告日，则权重序列中， $m_{t-1},\ m_{t},\ m_{t+1}$ 取值1，其余所有元素取值0。
...
权重序列： $\left[m_{T-239},\;m_{T-238},\ldots,m_{T-20},m_{T-19},\ldots,m_{T}\right]$ ，其中，假设t日为[T-220,T]区间内股票最近一个业绩报告日，则权重序列中， $m_{t-19},m_{t-18},\ldots,m_{t}$ 这连续20个元素取值1，其余所有元素取值0。
