...
等权和市值加权计算的最高组减最低组（High-Low）的原始收益差分别为-0.51%和，并且显著异于零。这表明，在美国市场上，对宏观经济不确定性敏感度较低的股票，未来能获得显著的超额收益。对此，作者的解释是，风险和不确定性是不同的。第一组中的股票，其 小于零，意味着它在经济不确定性上升时的收益较低（由得到$\beta^{UNc}的$ 回归方程可知）。因此，风险厌恶的投资者若持有这部分股票，就会要求以更高的预期收益的形式得到额外的补偿。而第十组中的股票，可以被看作是在经济不确定性上升时更加安全的资产，故投资者愿意为其支付更高的价格，并接受其较低的预期收益。
...
本文仿照美国市场的做法，以中国的 指数作为对宏观经济不确定性的度量，并和中国市场的 Fama-French 三因子共同形成四因子模型，通过最近 60 个月的数据滚动估计个股在 $.EPU_{\tau}$ 上的回归系数 $\mathcal{{\beta}}_{i,t}^{\mathcal{EPU}}$ 。具体的回归方程如下：

$$
\mathrm{Re}_{i,t}=\alpha_{i,t}+\beta_{i,t}^{EPU}*EPU_{t}+\beta_{i,t}^{MKT}*MKT_{t}+\beta_{i,t}^{NBS}*SMB_{t}+\beta_{i,t}^{HML}*HML_{t}+\epsilon_{i,t}
$$

其中， $\mathsf{Re}_{\mathfrak{i},\mathfrak{r}}$ 为股票 第 月的收益。

$\beta_{i,t}^{\mathcal{EPV}}$ 高的股票表明其与 EPU 有较强的相关性，即经济较不确定性越高，股票收益越好，可以认为有比较强的抗风险能力。而当经济不确定性减弱的时候，这部分股票的收益往往会下降。
...
由上表可见，在滞后阶数为 0-4 时，检验的 p 值都小于 0.05。这说明序列存在单位根的原假设被拒绝，EPU 指数是平稳的。由此计算得到 $|\beta_{\mathrm{i},t}^{\mathcal{EPU}}|$ 因子，可以用做进一步的研究和分析。

除了平稳性，另一个问题也值得关注。每个股票的 $1\beta_{i,t}^{\mathcal{EPU}}$ 因子均是基于历史数据计算得到，但却被用作预测未来的股票收益。这一做法实际上暗含着一个基本假定，即， $\beta_{i,t}^{\mathcal{EPV}}$ 因子在时间序列上并不会发生很大的变动。为了验证这一性质，本文采用了横截面自回归的方式予以研究。

首先，构建单因子的自回归，分别提取未来 6 个月、12 个月、24 个月、36 个月的$\beta_{i,t+T}^{\mathcal{EPI}}$ 因子与当前的 $i\beta_{\mathrm{i},t}^{\pounds pv}$ 因子构建如下的回归模型。其中，T=6，12，24 和 36。
...
$$
\beta_{i,t+T}^{EPU}=\alpha_{t}+\rho_{t}^{1}*\beta_{i,t}^{EPU}+\rho_{t}^{2}*\mathrm{Size}_{i,t}+\rho_{t}^{3}*\mathrm{PB}_{i,t}+\cdots+\rho_{t}^{9}*\mathrm{IVOL}_{i,t}+\varepsilon_{i,t}
$$

下表列示了这两个模型中， $\beta_{i,t}^{\mathcal{EPV}}$ 因子前的回归系数估计值及其 t 值。

表 3 $\beta_{i,t}^{EPI}$ 的自回归系数（2013.1-2018.4）
...
由上表可见，不论是否控制其他 8个因子，代表 $\mathcal{\beta}_{\mathrm{i},t}^{\mathcal{EPU}}$ 因子在时间序列上稳定性的系数 始终显著大于零。而且，12 个月以内的回归系数保持在 0.7 以上。这表明， $\beta_{\mathrm{i},t}^{\varepsilon\bar{\rho}\bar{v}}$ 因子是股票比较稳定的属性，可以作为一个新的因子考察其选股效果。

## 3.2 $\beta^{EPU}$ 因子的选股有效性检验

## 3.2.1 单变量分组

在每个月末，按照 $\cdot\beta^{\varepsilon\bar{\rho}\bar{v}}$ 因子把 A 股市场上所有可交易的股票（剔除 ST、停牌、涨停及上市不满 6 个月的股票），从小到大分成 5 组，依次记为 D1，D2，…，D5。分别计算等权和市值加权两种方式下，每组的平均收益（见下图）。
...
从上表中可以发现一个有趣的现象，只有在市值最大的一组（Big）中， $\beta^{\varepsilon\bar{\rho}\bar{v}_{1}}$ 和预期收益才表现出明显的正相关，首尾组合的收益差高达 1.02%。而在其余的市值组别中，分组收益的单调性较弱，首尾组合的收益差在统计意义上也不显著异于零。这似乎意味着，A股市场上，宏观经济的不确定性只有在大市值股票中未被合理定价。

为了证实这一猜测，本文进一步在沪深 300 和中证 500 的成分股中，检验了 $\left[\beta^{\varepsilon\bar{\rho}\bar{v}}\right]$ 的分组收益。下表展示的是沪深 300 成分股中的结果。
...
以上对比证实了双变量分组的结果，即， $\beta^{\varepsilon\bar{\rho}\bar{v}}$ 因子在市值较大的股票中具有较强的选股能力。

下表列示的是 $\beta^{\varepsilon\bar{\rho}\bar{v}}$ 因子在不同指数成分股中的 IC、IC-IR和胜率。和分组得到的结论一致， $\beta^{\varepsilon\bar{\rho}\bar{v}}$ 因子在沪深 300 成分股中的 IC 为 5.87%，IC-IR 接近 2，胜率也达到了3/4。而在中证 500 成分股和所有 A股中，该因子的 IC 均未超过 2%。

表 10 $\beta^{\bar{\varepsilon}\bar{\rho}\bar{\upsilon}}.$ 在不同指数成分股中的 IC、IR 值（2013.1-2018.4）
...
$$
\begin{array}{r}{\mathtt{Re}_{\mathtt{i},\mathtt{f}+\mathtt{l}}=\alpha_{\mathtt{f}}+\beta_{\mathtt{f}}^{\mathtt{l}}*\beta_{\mathtt{i},\mathtt{f}}^{\mathtt{EPU}}+\beta_{\mathtt{f}}^{\mathtt{2}}*\mathtt{Size}_{\mathtt{i},\mathtt{f}}+\beta_{\mathtt{f}}^{\mathtt{2}}*\mathtt{PB}_{\mathtt{i},\mathtt{f}}+\cdots+\beta_{\mathtt{f}}^{\mathtt{q}}*\mathtt{IVOL}_{\mathtt{i},\mathtt{f}}+\epsilon_{\mathtt{f}}}\end{array}
$$

下表给出了每个因子的截面溢价（回归系数）的时间序列均值和 t 统计量。在不同的指数成分股中，常用因子，如，市值、反转、ROE、换手率、特质波动率的截面溢价均表现出和预期一致的方向。 $\beta^{EDU}的$ 截面溢价均为正，但数值大小和 t 值却存在明显的差异。在沪深 300 成分股的回归中， $\beta^{\varepsilon\bar{\rho}\bar{v}}$ 的截面溢价均值为 ，是其余两种情形下的 1 倍。t 值接近 3，表明估计的稳定性也较高。这与上一节中，因子较高的 IR 和胜率相吻合。

表 11 $\beta^{\bar{\varepsilon}\bar{\rho}\bar{\upsilon}}.$ 在不同指数成分股中的截面溢价（2013.1-2018.4）

|  | 截距 | $\beta^{\varepsilon\bar{\rho}\bar{v}}$ | 市值 | PB | 反转 | ROE | 换手率 | 非流动性 | 特质波动率 | $\beta^{MKT}$ |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
以下两图进一步展示了 $\left[\beta^{\varepsilon\bar{\rho}\bar{v}}\right]$ 因子在沪深 成分股中的月度溢价序列及累计净值。除了 2015 年 3 月-8 月之间的回撤以外， $\beta^{\varepsilon\bar{\rho}\bar{v}}$ 因子在其余时间内均表现出稳定的正向溢价，胜率高达 73.4%。因此，可以认为， $\beta^{\varepsilon\bar{\rho}\bar{v}}$ 因子和沪深 300 成分股预期收益的正相关性，即使在控制了常见选股因子的影响之后，依然是稳健的。
