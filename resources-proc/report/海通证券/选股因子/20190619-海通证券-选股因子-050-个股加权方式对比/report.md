...
（ ）单个因子对初始权重 $\widehat{W_{\iota}}$ 的倾斜为：
连续因子倾斜 $W_{i}=\frac{F(Z_{i})*\widehat{W_{i}}}{\sum_{j=1}^{N}F(Z_{i})*\widehat{W_{j}}}$ 
$W_{i}=\frac{F_{1}\big(Z_{1,i}\big)*\dots*F_{M}\big(Z_{M,i}\big)*\widehat{W_{\iota}}}{\sum_{j=1}^{N}F_{1}\big(Z_{1,j}\big)*\dots*F_{M}\big(Z_{M,j}\big)*\widehat{W_{\jmath}}}$
资料来源：海通证券研究所整理
...
$$
F(Z)={\left\{\begin{array}{ll}{1+Z\qquad,Z>0}\\{(1-Z)^{-1},Z<0}\end{array}\right.}.
$$
...
$$
Capacity_{Port}=\sum_{i=1}^{N}Capacity_{i},
$$
...
具体来看，若我们要求策略的日资金容量不低于 $\mathsf{F}_{\mathsf{base}}$ （称为资金容量下限），则只要保证每只个股最大资金容量对应的策略总容量不低于 $\mathsf{F}_{\mathsf{base}}$ 即可。也就是对于任意一只

个股 i，要求 $\frac{Amt_{i}\times10\%}{W_{i}}\geq F_{base}$ ，对该式变形即为要求股票 i的权重不超过 $\frac{Amt_{i}\times10\%}{F_{base}}$ 。在这个限制条件下得到的组合资金容量（以历史日均成交额测算）将不低于设定的下限$\mathsf{F}_{\mathsf{base}}$
