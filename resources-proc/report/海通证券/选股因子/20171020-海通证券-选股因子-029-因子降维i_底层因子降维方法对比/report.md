...
$$
因子_{i}=\sum_{k=1}^{K}w_{k}^{i}*子因子_{k}^{i}
$$

其中，因子 $\mathrm{~i~}$ 为降维后的因子，子因子 $^1_{\mathrm{~k~}}$ 为降维前类别内的各因子， $w_{\mathrm{k}}^{i}$ 为降维权重。降维权重是因子降维的关键，不同的降维方法会给出不同的降维权重，从而影响降维后因子的表现以及模型最终的表现。本报告主要从以下三种思路出发，构建了相关降维方法：
...
$$
w_{k}=\frac{IC_{i}}{\sum_{k=1}^{K}\lvert IC_{i}\rvert}
$$
...
$$
w_{k}=\frac{{w^{\prime}}_{k}}{\sum_{k=1}^{K}\lvert{w^{\prime}}_{k}\rvert}
$$
...
3） 对于类别内因子 IC 序列进行主成分分析并取第一主成分权重 $w^{\prime}$ ，该类别各因子降维权重计算方法如下：
