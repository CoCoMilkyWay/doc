...
（1） 对于类别 $\complement_{\mathsf{k}},$ ，计算它与行业 j的重合个股数： $\#0_{\mathsf{k}}\cap|\mathsf{nd}_{\mathrm{j}}$ 。定义重合个股数最多的行业 J为类别 k 的相似行业。然后，将类别 k 与行业 J重合的个股数除以 ${\sf C}_{\sf k}$ 包含的个股数，作为该类别与行业分类的相似度。
