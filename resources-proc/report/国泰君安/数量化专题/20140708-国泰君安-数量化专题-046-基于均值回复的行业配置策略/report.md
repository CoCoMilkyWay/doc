...
| --- | --- | --- | --- | --- | --- |
| 1 | $(\frac{1}{2},2)$ | $(\frac{1}{2},\frac{1}{2})$ | 5 | $(\frac{1}{5},\frac{4}{5})$ | $B\to A$ |
|  |  |  | 4 |  |  |
...
假设市场上有 A、B两只股票，它们的相对价格为 $(\frac{1}{2},2),(2,\frac{1}{2})\cdots$ 。显然，从长期来看，买入并持有的市场策略不可能取得任何收益，因为每只股票的累积收益在经过 2n 个时期后保持不变。然而，权重为（1/2,1/2）的 CRP策略在n 个时期内却能有 $({\frac{5}{4}})^{n}$ 倍的收益。
...
假设在t 时期投资于金融市场各证券的权重以向量 $b_{t}=(b_{t1},b_{t2},\ldots,b_{tm})$ 表示。证券不允许卖空，因此每个证券权重均为非负的，并且权重加和为 1。也就是说， $b_{_t}\in\Delta_{_m}$ ，这里 $\Delta_{\textsf{\tiny m}}=\{\textsf{b}:\textsf{b}\in\mathbb{R}_{\textsf{\tiny+}}^{\textsf{\tiny m}},\sum_{i=1}^{\textsf{\tiny m}}b_{i}=1\}$ 。从而证券组合策略可以表示为如下的映射： $b_{_1}=(\frac{1}{m},\ldots,\frac{1}{m}),\ b_{_t}:\mathbb{R}_{_+}^{^{m(t-1)}}\to\Delta_{_m}$ $t=2,3,\ldots$ ，这里 $b_{_t}=b_{_t}(x_{_1},x_{_2},\ldots,x_{_{t-1}})$ 代表t 时期分配给证券组合的权重，并且 $,b_{_t}只$ 与前t •1个时期的证券相对价格向量 $x_{1},x_{2},...,x_{t-}$ 有关。

在 t 时 期 ， 证 券 配 臵 权 重 为 $b_{_t}$ ， 则 此 时 证 券 组 合 收 益 就 以$s_{_t}=b_{_t}^{^T}x_{_t}=\sum_{_{i=1}}^{^m}b_{_{ti}}x_{_{ti}}$ 的幅度增长，经过n 个时期后，证券组合的累积收益变为

$$
S_{_{n}}=S_{_{0}}\prod_{_{t=1}}^{^{n}}s_{_{t}}=S_{_{0}}\prod_{_{t=1}}^{^{n}}b_{_{t}}^{^{T}}x_{_{t}},
$$

其中 $S_{\mathrm{~0~}}$ 代表初始投资，为了方便，我们一般假设初始投资为 1。
...
$$
\log\left(\mathsf{b}\cdot x_{_t}\right)\approx\log\left(\mathsf{b}_{_t}\cdot x_{_t}\right)+\frac{x_{_t}}{\mathsf{b}_{_t}\cdot x_{_t}}\left(\mathsf{b}-\mathsf{b}_{_t}\right)\text{。 }
$$

均值回复策略是希望使得证券组合基于上一时期相对价格的预期收益要尽可能小，从而下一时期的组合收益尽可能较高。从上述线性形式看出，也就是使得b 与 $b_{_t}$ 的偏差最小。这里，定义b 与 $b_{_t}$ 的距离为$d\left(b,b_{t}\right)=\frac{1}{2}\left\|b-b\right\|^{2}$ 。因此均值回复策略近似等同于使得 $d\left(b,b_{_t}\right)$ 达到最小。此外，该目标函数也进一步降低了策略的换手率。

因此，对于积极的均值回复策略，我们可以进行如下建模：

$$
b_{_{t+1}}=\arg\min\frac{1}{2}\left\|b-b_{_t}\right\|^2
$$

$$
\mathrm{s.t.}\quad b\in\Delta_{_m},\quad b\cdot\hat{x}_{_{t+1}}\geq\varepsilon
$$

这里 $\hat{x}_{{}_{t+1}}$ 是第t •1时期相对价格向量的预测值。 为未知参数，其取值一般大于 1。 $\boldsymbol{b}\cdot\boldsymbol{\hat{x}}_{_{t+1}}\geq\boldsymbol{\varepsilon}$ 表示使得下一时期预测的组合收益值尽可能大，这实际上为该策略加入了双重保障。

直接求解上述二次规划最优化问题得出的b 值中，会存在取值为负数的元素，即存在i ，使得 $b_{_{ti}}<0$ 。因此，我们需要做一个简单的映射，使得 $b\in\Delta_{_m}$ 。简单介绍一下所做映射的思想。给定求解最优化问题的 $b_{\phantom{\dagger}_{t}}$ 值，要得到相应的 $\tilde{b}_{t}$ ，使得

$$
\tilde{b}_{t}=\arg\min\frac{1}{2}\left\|b-b_{t}\right\|^{2}\quad s.t.\quad b\in\Delta_{m}
$$
...
（1）给定向量 $b_{_t}\in\textit{ R }^{^m}$ ，以降序重新排列 $b_{_t}$ ，得到向量 $\mu_{_t}$ ：$\mu_{_{t1}}\geq\mu_{_{t2}}\geq\cdots\geq\mu_{_{tm}}$ ；

（2）找到 $\rho$ ，使得 $\rho=\mathrm{max}\{1\leq j\leq\mathrm{n}:\mu_{_j}-\frac{1}{j}(\sum_{_{r=1}}^{^j}\mu_{_r}-1)>0\}$ ；

（3）定义 $\theta=\frac{1}{\rho}(\sum_{_{r=1}}^{^{\rho}}\mu_{_{r}}-1)$ ；

（4）得到 $\tilde{b}_{_t},\quad s.t.\tilde{b}_{_{ti}}=\max\{b_{_{ti}}-\theta,0\},\quad1\leq i\leq m$
...
一种方法是认为下期走势与上期呈倒数关系。假设证券在t 时刻的价格为 $p_{\textit{ t }}$ ，对 $t+1$ 时刻价格的预测为 $\hat{p}_{{t+1}}$ ，则令 $\hat{x}_{_{t+1}}=\frac{1}{x_{_{t}}}$ ，这里 $x_{_t}$ 代表证

券在t 时刻的相对价格， $\hat{x}_{_{t+1}}$ 代表对t •1 时刻相对价格的预测值，即tp 1ˆ tp •
tx •1ˆ tx •1tp •tp

由此可以推出，

$$
\hat{x}_{t+1}=\frac{1}{x_t}\Rightarrow\frac{\hat{p}_{t+1}}{p_t}=\frac{p_{t-1}}{p_t}\Rightarrow\hat{p}_{t+1}=p_{t-1}
$$

还有一种策略是利用滑动移动平均的思想进行预测，即

$$
\hat{p}_{_{t+1}}=MA_{_{t}}(\omega)=\frac{1}{\omega}\sum_{_{i=t-\omega+1}}^{^{t}}p_{_{i}}
$$

这里 为所选取的时间跨度。

然而，这些策略并没有完全考虑噪 $声$ 数据以及异常值带来的影响，这会导致预测的误差。为了解决上述问题，我们使用了 $L_{\mathrm{{_1}}}$ -中值的思想来预测证券下一时期的价格。假设市场有m 种证券，证券在n 个交易时期内价格的变化以相对价格向量以 $x_{1},x_{2},\ldots,x_{n}\in R_{+}^{m}$ 来表示。第t 个向量的第i 个元素 $x_{_{ti}}$ 表示在第t个时期第i 个证券的相对价格，因此在第i 个证券的投资在t时期是以 $x_{_{ti}}$ 的幅度增长的。

假设已知前t个时期的相对价格向量 $x_{1},x_{2},...,x_{t}$ ，为了预测下一时期的相对价格向量 $\hat{x}_{_{t+1}}$ ，我们构造了如下的 $L_{\mathrm{{_1}}}$ -中值问题：

$$
\mu=\underset{\mu}{\arg\operatorname*{min}}\sum_{i=0}^{k-1}\left\|x_{t-i}-\mu\right\|
$$

这里， $\left\|\cdot\right\|$ 表示向量之间的欧氏距离。简单地说， $L_{\mathrm{{i}}}$ -中值方法就是使得预 测 值 与 前 k 个 相 对 价 格 向 量 之 间 的 距 离 最 小 。该方法类似于利用中位数作为下期价格的预测。
...
均值回复策略中有一个非常关键的参数，即最优化控制问题中的 。当越小时， $\boldsymbol{b}\cdot\boldsymbol{\hat{x}}_{{}_{t+1}}\geq\boldsymbol{\varepsilon}$ 的条件便越容易满足，此时均值回复策略便越接近于 CRP 策略。当 的值越大时， $\boldsymbol{b}\cdot\boldsymbol{\hat{x}}_{_{t+1}}\geq\boldsymbol{\varepsilon}$ 使得分配给 $\hat{x}_{_{t+1}}$ 中表现好的行业越高的权重。此时，下一时期取得的收益也会越高。
