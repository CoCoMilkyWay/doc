...
具体的，对于时间序 ${\mathfrak{H}}\mathbf{\Psi})\mathbf{y}=(y_{1},\dots,y_{t})$ ，令 $\mathbf{\sigma}\cdot\mathbf{n}={\mathfrak{t}}-{\mathfrak{m}}+1$ ， 为窗口长度，并限制 $\mathrm{m}\leq\mathrm{t}/2$ （一般取 $\mathrm{.t}=2\mathrm{m},$ ,使得矩阵在列满秩的情况下尽量小，因为若 t取的过长，不仅计算速度变慢，趋势线灵敏度也会下降）。我们定义 $\mathrm{.n}\times\mathrm{m}$ 阶的移动窗口矩阵为：

$$
\begin{array}{r}{\mathcal{H}=\left(\begin{array}{ccccc}{y_{1}}&{y_{2}}&{y_{3}}&{\cdots}&{y_{m}}\\{y_{2}}&{y_{3}}&{y_{4}}&{\cdots}&{y_{m+1}}\\{y_{3}}&{y_{4}}&{y_{5}}&{\cdots}&{\vdots}\\{\vdots}&{\vdots}&{\vdots}&{\ddots}&{y_{t-1}}\\{y_{n}}&{y_{n+1}}&{y_{n+2}}&{\cdots}&{y_{t}}\end{array}\right)}\end{array}
$$
...
这里的 $V_{k}$ 由 $\mathcal{C}$ 的前 个特征向量构成。定义重构矩阵 ${\widehat{\mathcal{H}}}^{}.$ 如下：
...
$$
\alpha_{p}=\left\{\begin{array}{cc}{{p}}&{{ifp<m}}\\{{t-p+1}}&{{ifp>t-m+1}}\\{{m}}&{{otherwise}}\end{array}\right.
$$
...
$$
\mathcal{H}=\left[\begin{array}{lll}{x_{1}}&{x_{2}}&{x_{3}}\\{x_{2}}&{x_{3}}&{x_{4}}\\{x_{3}}&{x_{4}}&{x_{5}}\end{array}\right]
$$

假设对其协方差阵奇异值分解得到的第一主成分方向为 $[a_{1}\quad a_{2}\quad a_{3}]^{T}$ 则重构矩阵 为：

$$
\begin{array}{rl}&{\widehat{\mathcal{H}}=\left[\begin{array}{lll}{x_{1}}&{x_{2}}&{x_{3}}\\{x_{2}}&{x_{3}}&{x_{4}}\\{x_{3}}&{x_{4}}&{x_{5}}\end{array}\right]\left[\begin{array}{lll}{a_{1}}\\{a_{2}}\\{a_{3}}\end{array}\right][a_{1}}\\&{\quad=\left[\begin{array}{lll}{a_{1}^{2}x_{1}+a_{1}a_{2}x_{2}+a_{1}a_{3}x_{3}}&{a_{1}a_{2}x_{1}+a_{2}^{2}x_{2}+a_{2}a_{3}x_{3}}&{a_{1}a_{3}x_{1}+a_{2}a_{3}x_{2}+a_{3}^{2}x_{3}}\\{a_{1}^{2}x_{2}+a_{1}a_{2}x_{3}+a_{1}a_{3}x_{4}}&{a_{1}a_{2}x_{2}+a_{2}^{2}x_{3}+a_{2}a_{3}x_{4}}&{a_{1}a_{3}x_{2}+a_{2}a_{3}x_{3}+a_{3}^{2}x_{4}}\\{a_{1}^{2}x_{3}+a_{1}a_{2}x_{4}+a_{1}a_{3}x_{5}}&{a_{1}a_{2}x_{3}+a_{2}^{2}x_{4}+a_{2}a_{3}x_{5}}&{a_{1}a_{3}x_{3}+a_{2}a_{3}x_{4}+a_{3}^{2}x_{5}}\end{array}\right]}\end{array}
$$

从而对角化求出的趋势线为：

$$
\begin{array}{rl}&{\widehat{x_{1}}=a_{1}^{2}x_{1}+a_{1}a_{2}x_{2}+a_{1}a_{3}x_{3}}\\&{\widehat{x_{2}}=[a_{1}a_{2}x_{1}+(a_{1}^{2}+a_{2}^{2})x_{2}+(a_{1}a_{2}+a_{2}a_{3})x_{3}+a_{1}a_{3}x_{4}]/2}\\&{\widehat{x_{3}}=[a_{1}a_{3}x_{1}+(a_{1}a_{2}+a_{2}a_{3})x_{2}+(a_{1}^{2}+a_{2}^{2}+a_{3}^{2})x_{3}+(a_{1}a_{2}+a_{2}a_{3})x_{4}+a_{1}a_{3}x_{5}]/3}\\&{\widehat{x_{4}}=[a_{1}a_{3}x_{2}+(a_{1}a_{2}+a_{2}a_{3})x_{3}+(a_{2}^{2}+a_{3}^{2})x_{4}+a_{2}a_{3}x_{5}]/2}\\&{\widehat{x_{5}}=a_{1}a_{3}x_{3}+a_{2}a_{3}x_{4}+a_{3}^{2}x_{5}}\end{array}
$$
...
$$
\widetilde{x_{5}}-\widetilde{x_{4}}=\frac{1}{3}(x_{5}-x_{2})
$$
...
从择时策略的角度来说，最关心的是趋势线末端的形态和变化，要及时准确的抓住市场的趋势离不开对趋势线末端的精准判断。SSA趋势线的末端与 MA 趋势线的末端不同。从以窗口参数为 3 的 MA 和 SSA 示例中可看到 MA末端点 由价格序列末 3项等权平均组成，SSA末端点同样由价格序列末 3 项构成，但权重大小和参数 ${a}_{1},{a}_{2},{a}_{3}$ 的大小对应，具有时变特征。
...
除此之外，奇异谱分析趋势线还有另一个特点。以 SSA3 为例，相比于MA3，在上涨趋势中 SSA3 末端点 各构成项权重系数 ${a}_{1}a_{3}$ $a_{2}a_{3}$ 、都大于等于 1/3，下跌趋势中都小于等于 1/3，致使 SSA3 波幅相对 MA3要大。因此如下图所示，一旦价格趋势上涨（下跌）结束，SSA能使得价格更快下穿（上穿）趋势线，使得趋势的结束较早得到确认。
