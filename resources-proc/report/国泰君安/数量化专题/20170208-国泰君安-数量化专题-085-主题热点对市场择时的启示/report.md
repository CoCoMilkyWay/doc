...
从表3不难看出，成交活跃市场的所有样本五日收益是 0.950%，胜率是58.263%，而随着百分位水平逐步提高，无论是五日收益还是胜率都有比较明显的上升，也就是说，两者之间有正相关关系，从投资的角度讲，如果在成交活跃市场上出现主题强势的现象，应该是买入或者加仓的时机。但是，由于在实际的投资当中，我们并不知道样本数量情况，无法通过百分位数来量化地确定主题是否“足够强”，而考虑到主题相对强弱指数本身是在[0,92]这个封闭区间内波动的，因此我们可以使用一个静态阈值 $\lambda_{\mathsf{high}}$ 来表示这个“高阈值”。也就是说，实际的投资决策中，我们在主题相对强弱指数大于 $\lambda_{\mathrm{high}}$ 时发出买入信号。

为了进一步说明这一点，我们列出全样本中所有在成交活跃市场下主题相对强弱指数大于 $\cdot\lambda_{\mathrm{high}}$ 的样本，这里取 $\lambda_{h\mathrm{igh}}$ 为 40，如表 4所示（表中 Index是主题相对强弱指数，Return 是后五日收益）。可以看到，在大于 $\lambda_{h\dot{1}gh}$ 的样本中，交易日后五日收益达到1.463%，高出成交量活跃市场的平均值 0.950%；胜率达到 62.86%，高于平均水平 58.26%。

表 4：成交活跃市场下主题相对强弱指数大于 $\lambda_{high}$ 的样本
...
从表 5 也可以看出，成交萎缩市场的所有样本五日收益是-0.176%，胜率是48.29%，而随着百分位水平逐步提高（和成交活跃市场相反，这里的百分位水平越高，指的是主体相对强弱指数越低），五日收益还是胜率都开始下降，但是下降的幅度并不大，也就是说，两者之间也是正相关关系，但是相关关系不如成交活跃市场那么明显。同样，由于在实际的投资当中，我们并不知道样本数量情况，无法通过百分位数来量化地确定主题是否“足够弱”，因此我们也使用一个静态阈 $\langle\frac{\ddagger}{\Xi}\lambda_{\sf{low}}$ 来表示这个“低阈值”。也就是说，实际的投资决策中，我们在主题相对强弱指数小于 $\lambda_{\mathsf{Iow}}$ 时发出卖空信号。为了进一步说明这一点，我们也列出了全样本中所有在成交萎缩市场下主题相对强弱指数小于 ${\cdot\lambda_{\left|\right.\mathrm{ow}}}$ 的样本，这里取•low为 10，如表 6 所示。可以看到，在小于 $\cdot\lambda_{\sf dw}$ 的样本中，交易日后五日收益只有-0.395%，低于成交萎缩市场的平均值-0.176%；胜率只有42.86%，低于平均水平48.29%。

表 6：成交萎缩市场下主题相对强弱指数小于 $\cdot\lambda_{\parallel\parallel}$ 的样本
...
根据第三章的逻辑，市场根据近期成交量的变化情况可以分为三种状态，即成交活跃市场、成交萎缩市场和成交稳定市场。在成交活跃市场，主题热点越多，市场越容易形成共振，后市容易走强，具体到策略，只要主题相对强弱指数大于参数 $\lambda_{\mathsf{high}}$ ，则发出买入信号，直至市场状态改变进行平仓操作；在成交萎缩市场，市场缺乏人气，后市偏弱，因此主题强弱指数小于参数 $\lambda_{10w}$ 时，发出卖出信号，直至市场状态改变进行平仓操作；在成交稳定市场，一般没有外部资金流入，因此主题表现为轮动的特征，从而在主题强弱指数大于 $\cdot\lambda_{\mathsf{high}}$ 时发出做空信号，小于 $\boldsymbol{\cdot}\lambda_{\perp\mathrm{ow}}$ 时发出做多信号。考虑交易成本，构建出基于主题相对强弱指数的择时策略，策略曲线如图2所示。
...
如果为成交活跃市场，则主题强弱指数大于 $\cdot\lambda_{\mathrm{\scriptsize{high}}}$ 时，发出做多信号，直至市场状态改变进行平仓；如果为成交萎缩市场，则主题强弱指数小于$\lambda_{10w}$ 时，发出做空信号，直至市场状态改变进行平仓；如果为成交稳定市场，则主题强弱指数大于 $\cdot\lambda_{\mathrm{\scriptsize{high}}}$ 时发出做空信号，小于 $\cdot\lambda_{\sf dw}$ 时发出做多信号。参数选择：选择 $\lambda_{\mathrm{\Delta high}}=30$ $\lambda_{\textrm low}=10$ （4.2 节会进行参数敏感性测试，证明该选择并非必须）
...
可以看到，固定参数• $\mathtt{\Omega}_{1\mathtt{ow}}=10$ $\lambda_{\mathrm{high}}.$ 在 20 到 80 之间变化时，整体呈现出年化收益递减，最大回撤递增的现象，在 $\therefore\lambda_{high}$ 取值 30 附近（在区间[25,40]内）整体比较稳定，并在 $\lambda_{\mathrm{high}}=31$ 处取得年化收益的最大值。固定参数 $\lambda_{h\mathrm{igh}}$ =30，•low在1 到20 之间变化时，年化收益和最大回撤都没有显著的变化特征，但是 $\frac{1}{1}\pm\lambda_{10w}$ 取值 10附近（在区间[6,15]内）比较稳定。
