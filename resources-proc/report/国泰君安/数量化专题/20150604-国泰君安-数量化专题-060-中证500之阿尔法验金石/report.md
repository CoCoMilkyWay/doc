...
其中， $\boldsymbol{r}_{j}$ 表示第 $j$ 只股票的收益率； $x_{\scriptscriptstyle k}$ 表示第 j 只股票在第k 个因子上的暴露（也称为因子载荷）； $\boldsymbol{f}_{k}$ 表示第 j 只股票第k 个因子的因子收益

率（即每单位因子暴露所承载的收益率）； $\boldsymbol{u}_{\ j}$ 表示第 j 只股票的特质因子收益率。
...
$$
\begin{array}{c}\begin{array}{ccccc}{{\displaystyle\prod_{i}^{}{Var}(f_{1})}}&{{Co\nu(f_{1},f_{2})}}&{{...}}&{{Co\nu(f_{1},f_{k})}}\\{{\displaystyle\prod_{}^{}{Co\nu(f_{1},f_{2})}}}&{{Var(f_{2})}}&{{...}}&{{Co\nu(f_{2},f_{k})}}\\{{\displaystyle\hfill\begin{array}{cccccc}{{\hfill}{}}\\{{\ldots}}\\{{\ldots}}\\{{\bigcup_{}{Co\nu(f_{k},f_{1})}}}&{{Co\nu(f_{k},f_{2})}}&{{...}}&{{\ldots}}\end{array}}}&{{\displaystyle\bigcup_{i}{}}}\\{{\bigcup_{i}{Co\nu(f_{k},f_{2})}}}&{{Co\nu(f_{k},f_{2})}}&{{...}}&{{Var(f_{k})}}\end{array}\quad]\end{array}
$$
...
我们首先比较回归方程中，等权和以根号市值加权2 种方式对风险模型$R^{\mathit{\Delta}^{2}}$ 的影响，样本空间为全市场（非ST），计算结果均为滚动 12期 $R^{2}$ 均值，具体如下：
...
| 全市场（非 ST） | 中证 800 指数成分股 | $\Delta R^{^{2}}$ |
| --- | --- | --- |
...
$R^{\mathit{\Delta}^{2}}$ 高于以全市场非ST 成分作为样本空间，平均 $R^{2}$ 高出7%左右。

$R^{\mathit{\Delta}^{2}}$ 计算结果显示，不同的加权方式和样本空间对风险模型的解释力度
...
具体的，另 $R_{\mathfrak{n}t}$ 表示组合n 在第t 期的实际收益率， $\sigma_{\it{nt}}$ 表示第t •1期末对第t期组合 n的波动率预测值。

令 $b_{_{nt}}$ 表示收益率 $R_{\mathfrak{n}t}$ 与预测波动率 $\sigma_{\it{nt}}$ 的比值，即

$$
b_{_{nt}}=\frac{R_{_{nt}}}{\sigma_{_{nt}}}
$$

那么偏差统计量 $\boldsymbol{B}_{\mathfrak{n}}$ 即可以表示为，

$$
B_{_n}={\sqrt{{\frac{1}{T-1}}{\sum_{t=1}^{T}{{({b_{_nt}}-{\overline{{b_{n}}}})}^{2}}}}}
$$

其中，T 表示组合收益的观测周期长度。显然，对于周期足够长的T 而言，若组合波动率的预测是百分之百精确的，那么 $B_{n}=1$ 。（假设收益率服从正态分布）

对于检验预测波动率而言，假设组合波动率的预测精度存在统计意义上的显著性，那么对于95%的置信区间 $[1-\sqrt{2\ ⁄T},1+\sqrt{2/T}]$ ，若计算的偏差统计量 $B_{\mathfrak{n}}$ 不在置信区间内，则拒绝组合波动率预测精确的假设。反之则可称组合波动率的预测在 95%的显著性水平下精确。

更一般的，为了消除风险模型有效性本身的波动率，通常会采用年度滚动计算 $B_{\it{n}}$ 再求平均值的方式，即令

$$
B_{n}^{\tau}=\sqrt{\frac{1}{11}\sum_{t=\tau}^{\tau+11}{(b_{nt}-\overline{{b_{n}}})}^{2}}
$$

表示滚动12 个月的偏差统计量，其中 表示滚动窗口的起始时点，那么

对于组合n，在样本长度T 内共有T •11个 $\boldsymbol{B}_{n}^{\prime}$ 值，那么其均值即为

$$
\overline{{{B}}}_{n}=\frac{1}{T-11}\sum{B_{n}^{\tau}}
$$

同样，若计算的偏差统计量 $B_{\it{n}}$ 落入置信区间内，则可称组合波动率的预测在95%的显著性水平下精确。
...
我们分别计算针对上述 8种组合的滚动偏差统计量 $\overline{{B}}_{n}$ ，针对的原假设为组合波动率预测结果不精确，偏差检验结果如下表：

表 9：组合波动率偏差统计检验结果：

| 组合 | 上证50 | 沪深300 | 中证500 | 上证 180 | 中小板指 | 创业板指 | 深圳 300 | 全市场等权 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| $\overline{{B}}_{n}$ | 1.156 | 1.095 | 1.056 | 1.122 | 1.034 | 1.154 | 0.974 | 1.071 |
| 检验结果 | Reject | Reject | Reject | Reject | Reject | Reject | Reject | Reject |
| 偏差结果 | 低估 | 低估 | 低估 | 低估 | 低估 | 低估 | 高估 | 低估 |

数据来源：国泰君安证券研究

从偏差检验的结果来看，对于我们给出的8 种指数成分组合，风险模型的偏差统计检验结果均较为理想，8 个组合的 $\overline{{B}}_{n}$ 均落入 95%置信区间范

围内，各组合 $\overline{{B}}_{n}$ 的平均值为 1.082，检验结果较为理想，体现了风险模型对组合波动率预测的显著性。
...
$$
\begin{array}{rl}{Max}&{R_{r}-\lambda\sigma_{_{P}}^{^2}-TC\left(w\right)}\\{s.t.}&{\forall{k^{\prime}}\left(w^{^T}-w_{_{bench}}\right)X_{_{k^{\prime}}}=0}\\&{w^{^T}H={h^{T}}}\\&{w\geq0}\\&{\sum_{i=1}^{N}w_{i}=1}\end{array}
$$

其 中 TC w( ) 表 示 以 权 重 w 构 建 组 合 的 换 仓 成 本 ，

$$
R_{p}=\sum_{j=1}^{N}w_{n}\cdot(\sum_{k=1}^{K}x_{jk}f_{jk}+u_{j}),\sigma_{p}=\sqrt{w^{\prime}(XFX^{\prime}+\Delta)w},\lambda\ \not\exists\ \mathcal{M}|_{\mathcal{Z}}^{pk}\ ,
$$

恶系数， $X_{\mathbf{\Omega}_{k},\mathbf{\Omega}}$ 表示风险因子截面，H 为行业因子哑变量矩阵，h 表示中证500指数行业占比权重。
...
4）优化目标函数为 $\begin{array}{rl}{Max}&{{}R_{_P}-\lambda\sigma_{_P}^{^2}-TC\left(w\right)}\end{array}$ 形式，其中 $\lambda={\frac{1}{2}}$
...
$$
(w^{\textit{ T }}-w_{\textit{ b e n c h }}^{\textit{ T }})X_{\textit{ k }}
$$

其中， $X_{j}$ 为标准化风险因子截面， $w_{\it bench}$ 为对冲基准组合对应权重。组合风险因子敞口分析如下：
...
$$
(w^{\textit{ T }}-w_{bench}^{\textit{ T }})X_{\textit{ k }}\cdot f_{\textit{ k }}
$$

其中 $\boldsymbol{f}_{k}$ 为第k 类因子当期的实际因子收益率。组合因子收益归因分析如下：
