...
具体而言，如果 $P_{t}$ 是时间段 $[\mathsf{t}-\mathsf{n}_{1}~t+\mathsf{n}_{2}]$ 的市场底部, $P_{t}$ 必须满足以下条件:

(1)min Pt,其中 $n_{\scriptscriptstyle1}>0,$ s∈[t−n₁t+n2]Pmin
...
$$
\left(2\right)~\mathsf{P}_{max1}=\operatorname*{max}_{s\in\left[t-n_{1}\right]}P_{s}~\frac{P_{t}}{P_{min}}\le1+\alpha,~\frac{Pmax_{1}}{P_{min}}=1+\alpha_{1},~\alpha_{1}\gg\alpha_{o}
$$

$$
~(3)~{\mathbb P}_{max2}=\operatorname*{max}_{s\in[t~t+n_{2}]}P_{s},~\frac{{\mathcal P}max_{2}}{P_{min}}=1+\alpha_{2},~\alpha_{2}\gg\alpha_{o}
$$

条件 1 用来确认市场底部区域，其中 $P_{min}$ 是市场底部区域的最低点位，$\pmb{\alpha}$ 用来描述市场底部的波动范围。

条件 2 是对市场底部之前的下跌波段的确认，其中 $Pmax_{\Im}$ 是时间段

$[t-n_{1},t]$ 中下跌波段的最高点, $\pmb{\alpha}_{1}$ 用来描述下跌波段的下跌幅度。
...
具体的，如果 E点是 $[{\tt t}-{\tt n}_{1}$ 的 W型底部的确认点，我们通过以下步骤对 W 型底部进行确认：

对 D 点进行确认: D 点为 $[\ t-\boldsymbol{\mathrm{n}}_{2}$ 时间段的最低点,其中 $\mathtt{n}_{2}<\mathtt{n}_{1}$

$\mathbb{P}_{\mathcal{D}}=\operatorname*{min}_{s\in[t-n_{2}\ t]}P_{s}$ , D 点距离当前的时间为 $t-t_{D}$ , DE 段的涨幅为

$$
\begin{array}{r}{\mathtt{R}_{\bar{D}E}=\frac{\bar{P}_{\bar{E}}}{\bar{P}_{\bar{D}}}-1.}\end{array}
$$

对 B 点的确认： B 点为时间段 $-\mathtt{n}_{1}\mathtt{t}_{D}]\sharp\sharp$ 最低点,

$\mathrm{P}_{E}=\operatorname*{min}_{s\in[t-n_{1}~t]}P_{s},$ , B 点距离当前的时间 $t-\ t_{B}.$ 为了确保 W 型,

我们要求 B点和 D点的点位不能相差太大, $\begin{array}{r}{|\frac{\mathcal{P}_{\mathcal{D}}}{\mathcal{P}_{\mathcal{B}}}|-1\le\alpha.}\end{array}$

对 C 点的确认：C 点为时间段 $[t_{E}t_{D}]$ 的最高点， $\mathsf{P}_{c}=\operatorname*{max}_{s\in[t_{B}\ t_{C}]}P_{s}$ , BC 段的

上涨幅度 $\begin{array}{r}{\mathtt{R}_{cB}=\frac{\mathtt{P}_{C}}{\mathtt{P}_{B}}-1}\end{array}$

对 A 点的确认：A 点为时间段 $[t-\boldsymbol{\mathrm{n}}_{\perp}t_{B}]$ 的最高点, $\mathrm{P}_{{\pmb{A}}}=\operatorname*{max}_{s\in[t-n_{1}\tau_{\ B{\vec{B}}}]}P_{s}$ , AB

段的下跌幅度为 $\begin{array}{r}{\mathtt{R}_{AB}=\frac{P_{A}}{p_{B}}-1}\end{array}$ , A点距离当前的时间为 $\mathrm{-}\ t_{A}$由以上可知，确认不同的 W型底部主要由时间参数和形态参数决定.

时间类型的参数 $[\boldsymbol{\mathtt{n}}_{1},\boldsymbol{\mathtt{n}}_{2}]$ 决定了 W 型底部所在的时间区间： $\mathfrak{n}_{\mathtt{1}}$ 和 $\mathfrak{n}_{z}$ 分别决定了 W型底部中的双底 D和 B发生的时间区间.

形态类型的参数决定了 W型底部各段的涨跌幅和持续时间: a.各段的涨跌 幅 度 参 数 $[\mathrm{\sf~R}_{AB},\mathrm{~\cal~R}_{BC},\mathrm{~\cal~R}_{CD},\mathrm{~\cal~R}_{DE}]$ b. 各 段 的 时 间 长 度 参 数$[\mathsf{t}_{B}-\mathsf{t}_{A},\mathsf{t}_{C}-\mathsf{t}_{B},\mathsf{,\ t}-\mathsf{t}_{C},\mathsf{t}-\mathsf{t}_{D}].$

## 3.2. W 型底部特征的统计分布

为了能够统计历史上大多数的 W型底部各段的特征（涨跌幅度、持续时间）,我们尽量的减少对参数的限制。 对于 W型底部的时间参数, 我们仅要求 D 点发生在过去的十个交易日之内, 即 $\begin{array}{r}{\mathtt{n_{1}}=10,}\end{array}$ , 而同时 W 型底部的前底 B点发生在过去的 60个交易日之内， $\operatorname{g}\rho_{\boldsymbol{\Pi}_{2}}=60.$ . 对于 W型底部的形态特征参数，我们要求 AB段的跌幅不小于 5%，BC段的涨幅不小于 2%。此外,为了确保 D点和B点相距不大,我们要求 D点和 B点的点位之差不能超过 2%, 即 $\scriptstyle\alpha=2\%.$ . 由于 E点不仅是 W 型底部最后的确认点，也决定了基于 W型底部投资策略的买点，为了搜集尽可能多的 W型底部的样本，我们对于 DE 段的确认首先做了最低限制，仅要求当 E 点的点位高于 D点时即确认，具体的如何通过对 DE 段涨幅限制调整对 W 型底部确认以及后续走势的影响会在后面的部分详细讨论。

我们的样本包含上证综指 2005年 1月 1日到 2016年 12月31日的日频交易数据。 根据以上的参数，上证综指在 2005-2016 年区间共出现 54次 W 型底部。以下为 W 型底部 AB、BC、CD 各段的数据统计结果和相关性分析。

表 1: W 型底部特征的历史统计

| W型底各段数值 统计 | $\mathbf{R}_{AB}$ | $\mathtt{R}_{BC}$ | $\scriptstyle\mathbf{R}_{CD}$ | $\mathbf{t}_{B}-\mathbf{t}_{A}$ | $\mathbf{t}_{C}-\mathbf{t}_{B}$ | $\mathbf{t}_{D}-\mathbf{t}_{C}$ |
| --- | --- | --- | --- | --- | --- | --- |
...
| W型底各段相关 性检验 | $\mathbf{cor}(\mathbf{R}_{AE},\mathbf{R}_{BC})$ | $\mathsf{cor}(\mathbb{R}_{AE}\ \mathbb{R}_{CD})$ | $\mathsf{cor}(\mathbb{R}_{BC},\ \mathbb{R}_{CD})$ | $\mathsf{cor}(\mathsf{t}_{AB},\mathsf{t}_{BC})$ | $\mathsf{cor}(\mathsf{t}_{AB},\mathsf{t}_{CD})$ | $\mathsf{cor}\left(\mathsf{t}_{BC},\mathsf{t}_{CD}\right)$ |
| --- | --- | --- | --- | --- | --- | --- |
...
表 3: 不同形态特征下 W型底部择时策略收益 $(\mathtt{P}_{E}>\mathtt{P}_{D})$

| $\pmb{\mathrm{p}}_{E}>\pmb{\mathrm{p}}_{D}$ | 胜率 | 平均收益 | 最大收益 | 最小收益 | 样本数 |
| --- | --- | --- | --- | --- | --- |
...
表 4: 不同形态特征下 W型底部择时策略收益 $(\mathrm{R}_{ED}>0.2\mathrm{R}_{cD})$

| $\mathbf{R}_{ED}>0.2\mathbf{R}_{CD}$ | 胜率 | 平均收益 | 最大收益 | 最小收益 | 样本数 |
| --- | --- | --- | --- | --- | --- |
...
表 5: 不同形态特征下 W型底部择时策略收益 $(\mathtt{R}_{ED}>0.4\mathtt{R}_{cD})$

| $\mathtt{R}_{ED}>0.4\mathtt{R}_{CD}$ | 胜率 | 平均收益 | 最大收益 | 最小收益 | 样本数 |
| --- | --- | --- | --- | --- | --- |
...
表 6: 不同形态特征下 W型底部择时策略收益 $(\mathrm{R}_{ED}>0.6\mathrm{R}_{cD})$

| $\mathbf{R}_{ED}>0.6\mathbf{R}_{CD}$ | 胜率 | 平均收益 | 最大收益 | 最小收益 | 样本数 |
| --- | --- | --- | --- | --- | --- |
...
表 7: 不同形态特征下 W型底部择时策略收益 $(\mathtt{R}_{ED}>\mathtt{R}_{cD})$

| $\mathtt{R}_{ED}>\mathtt{R}_{CD}$ | 胜率 | 平均收益 | 最大收益 | 最小收益 | 样本数 |
| --- | --- | --- | --- | --- | --- |
...
表 8: 不同形态特征下 W型底部择时策略收益 $(\mathrm{R}_{AB}\geq10\%)$

| $\mathbf{R}_{AB}\geq10\%$ | 胜率 | 平均收益 | 最大收益 | 最小收益 | 样本数 |
| --- | --- | --- | --- | --- | --- |
...
表 9: 不同形态特征下 W型底部择时策略收益 $(\mathbb{R}_{AB}\geq15\%)$

| $\mathbf{R}_{AB}\gtrsim15\%$ | 胜率 | 平均收益 | 最大收益 | 最小收益 | 样本数 |
| --- | --- | --- | --- | --- | --- |
...
表 10: 不同形态特征下 W 型底部择时策略收益 $(\mathrm{R}_{AB}\geq20\%)$

| $\mathbf{R}_{AB}\geq20\%$ | 胜率 | 平均收益 | 最大收益 | 最小收益 | 样本数 |
| --- | --- | --- | --- | --- | --- |
...
表 11: 不同形态特征下 W 型底部择时策略收益 $(\mathrm{R}_{AB}\geq5\%~\mathrm{R}_{CB}\geq5\%)$
...
表 12: 不同形态特征下 W 型底部择时策略收益 $(\mathrm{R}_{AB}\geq10\%~\mathrm{R}_{CB}\geq5\%)$
...
表 13: 不同形态特征下 W 型底部择时策略收益 $(\mathrm{R}_{AB}\geq15\%\ \mathrm{R}_{CB}\geq5\%)$
...
表 14: 不同形态特征下 W 型底部择时策略收益 $(\mathrm{R}_{AB}\geq10\%\mathrm{R}_{CB}\geq10\%)$
