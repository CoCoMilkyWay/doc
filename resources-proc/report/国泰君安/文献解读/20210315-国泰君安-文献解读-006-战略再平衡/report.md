...
我们可以引入股票与债券的平均收益率 $R_{t}^{Avg}\;=\;(R_{t}^{B}\;+\;R_{t}^{S})/2$ 来分别改写股票与债券的单期收益率，将股票与债券的相对收益率记作 $K_{t}=$ $R_{t}^{S}-R_{t}^{B}$ 。股票与债券收益率可以以相对收益的形式表达如公式3与公式 4

$$
R_{t}^{S}\;=\;(R_{t}^{Avg}\;+\;0.5\;K_{t})\tag{3}
$$
...
$$
\begin{aligned}R_{t}^{mom(N)}&=0.5mom_{t-1}^{S}(N)\frac{\sigma^{Ref}}{\sigma_{t-1}^{S}}R_{t}^{S}+\\&\quad0.5mom_{t-1}^{B}(N)\frac{\sigma^{Ref}}{\sigma_{t-1}^{B}}R_{t}^{B}\quad+\quad0.8R_{t}^{F}\end{aligned}\tag{7}
$$
