...
$$
\mathrm{L=}\frac{E(R_{p})}{Std(R_{p})}\tag{1}
$$

其中， $E(R_{p})$ 和 ${\mathfrak{r}}t{d}(R_{p})$ 是投资组合收益的期望和标准差的估计。具体来说，对于 $\mathbf{t}=\{1,\dots,\mathrm{T}\}$ 的交易期，我们可以最大化以下目标函数：
...
其中 $r_{i,t}$ 为资产 i 在第 t 期的收益， $w_{i,t}$ 为资产的分配比率，满足 $.w_{i,t}\in$ $\scriptstyle[0,1],\sum_{i=1}^{n}w_{i,t}=1$ 。在本文中，作者采用参数为θ的神经网络 f 对多头投资组合中的 $w_{i,t}$ 进行建模：

$$
w_{i,t}=f(\boldsymbol{\theta}|\boldsymbol{x}_{t})\tag{5}
$$

其中， $x_{t1}$ 表示当前的市场信息，模型绕过经典的预测步骤，将输入与头寸联系起来，以在交易期 T 中最大化夏普比，即 $L_{T}$ 。为了对一个多头的投资组合施加约束，要求每个资产权重非负，且求和为 1，本文使用softmax 输出来满足这些要求：
...
为了构造一个纯多头组合，该框架对输出层使用 softmax 激活函数，它对输出施加约束，以保持组合中每个资产的权重为正，且求和为 1。输出节点的数量 $(w_{1},\ldots,w_{n})$ 等于所构建投资组合中的资产数量。可以通过将这些投资组合中的资产权重与相关资产的回报 $\left(r_{1},\ldots,r_{n}\right)$ 相乘来计算最终实现的投资组合收益 $.R_{p}.$ 。一旦得到这个收益值，我们就可以求解出夏普比率，并计算出夏普比与模型参数相关的梯度，并使用梯度上升来迭代更新参数。
...
$$
R_{p,t}=\sum_{i=1}^{n}\frac{\sigma_{tgt}}{\sigma_{i,t-1}}w_{i,t-1}r_{i,t}-M_{t}\tag{8}
$$
...
其中 $\sigma_{tgt}$ 为目标波动率， $\sigma_{i,t-1}$ 为资 $\cdot 产$ i 的预期波动率，用指数加权移动均值的标准差表示。使用资产交易价值的每日变化来表示交易成本，这是由公式 表示。 为成本率，作者取不同的成本率以反映该模型在不同交易成本下的表现。
