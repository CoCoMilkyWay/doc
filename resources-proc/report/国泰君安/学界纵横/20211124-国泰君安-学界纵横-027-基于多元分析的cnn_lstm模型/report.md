...
$$
\tilde{C}_{t}=tanh(W_{c}.[h_{t-1},x_{t}]+b_{c})\tag{3}
$$
...
下面，在本次处理过程 t 中，输出值 $\boldsymbol{h}_{t-1}$ 和输入值 $x_{t}$ 作为输出门的输入，在输出门中由以下公式计算：
...
其中 $\hat{y}_{i\cdot}$ 是通过模型得到的 i 时刻的预测值， $y_{i}$ 为 i 时刻该变量的真实值。
