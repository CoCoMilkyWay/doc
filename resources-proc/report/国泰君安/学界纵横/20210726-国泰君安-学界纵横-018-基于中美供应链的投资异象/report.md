...
$$
\begin{aligned}RET_{t}=\alpha+&\gamma CRET_{t-1}+\beta_{1}INDRET_{t-1}+\beta_{2}RET_{t-1}+\beta_{3}SIZE_{t-1}\\&+\beta_{4}B/M_{t-1}(+\beta_{5}CINDRET_{t-1}+\beta_{6}CRET_{t-2})+\varepsilon_{t}\end{aligned}\tag{1}
$$

其中，因变量 $iRET_{t}$ 指供应商公司在 t 月的股票回报率。自变量 $CRET_{t-1}$ 指供应商的主要中国客户公司在 t-1 月的股票回报率。其余指标， $RET_{t-1}$ 指供应商公司在 t-1 月的回报率， $INDRET_{t-1}$ 指供应商公司股票所在行业在 t-1 月的回报率， $SIZE_{t-1}$ 指滞后的公司市场价值， $B/M_{t-1}$ 即滞后的B/M 比值。 $CINDRET_{t-1}$ 指滞后的客户公司所在行业的（平均）回报率，$CRET_{t-21}$ 指 t-2 月的客户公司的回报率。
...
| $SIZE_{t-\mathbf{1}}$ -0.003 | -0.002 0.001 | 0.002 | 0.002 -0.001 |
| --- | --- | --- | --- |
...
|  | (1.025) |  | (-1.045) |
| $\mathbf{A}\mathbf{D}\mathbf{J}{\cdot}\mathbf{\boldsymbol{R}}^{2}$ 0.149 | 0.153 0.170 | 0.134 | 0.165 0.187 |
...
$$
p(X_{p,t})=prob(Customer_{p,t}=1|X_{p,t})=\frac{\exp(\beta_tX_{p,t})}{1+\exp(\beta_tX_{p,t})}\tag{2}
$$
...
$$
\begin{aligned}RET_{t}=\alpha+\gamma CRET_{t}^{p}+\beta_{1}INDRET_{t-1}+\beta_{2}RET_{t-1}+\beta_{3}SIZE_{t-1}\\+\beta_{4}B/M_{t-1}(+\beta_{5}CINDRET_{t-1}^{p}+\beta_{6}CRET_{t-2}^{p})+\varepsilon_{t}\end{aligned}\tag{3}
$$
...
| $SIZE_{t-\mathbf{1}}$ | 0.001 | -0.001 | 0.001 | 0.001 | -0.001 | -0.001 |
|  | (0.350) | (-0.211) | (0.308) | (0.258) | (-0.024) | (-0.263) |
| $B/M_{t-1}$ | -0.003 | -0.005 | -0.003 | -0.006 | -0.006 | -0.005 |
|  | (-0.611) | (-0.851) | (-0.485) | (-0.997) | (-0.846) | (-0.802) |
| $\underline{{CINDRET_{t-1}^{p}}}$ |  | -0.250 | -0.272** |  | 0.042 | 0.128 |
|  |  | (-1.489) | (-2.187) |  | (0.178) | (0.622) |
| $\boxed{CRET_{t-2}^{p}}$ |  |  | -0.214** |  |  | 0.054 |
|  |  |  | (-2.161) |  |  | (0.518) |
| $\mathbf{A}\mathbf{D}\mathbf{J}{\cdot}\mathbf{\boldsymbol{R}}^{2}$ | 0.132 | 0.142 | 0.158 | 0.119 | 0.159 | 0.185 |
...
$$
\begin{aligned}CRET_{t}=\alpha+\gamma RET_{t-1}+\beta_{1}CINDRET_{t-1}+\beta_{2}CRET_{t-1}+\beta_{3}CSIZE_{t-1}\\+\beta_{4}CB/M_{t-1}(+\beta_{5}INDRET_{t-1}+\beta_{6}RET_{t-2})+\varepsilon_{t}\quad(4)\end{aligned}
$$
...
|  |  |  | (1.383) |  |  | (-0.687) |
| $\mathbf{A}\mathbf{D}\mathbf{J}{\cdot}\mathbf{R}^{2}$ | 0.141 | 0.170 | 0.199 | 0.131 | 0.165 | 0.176 |
