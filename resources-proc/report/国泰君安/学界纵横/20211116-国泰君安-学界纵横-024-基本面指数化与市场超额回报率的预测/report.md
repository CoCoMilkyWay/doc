...
首先，构建财务统计量 $\cdot S_{j,t},$ ，该指标反映了公司j在时间t的财务状况。根据 Piotroski（2000）的方法，本文从盈利能力、财务杠杆及流动比率、股本发行和运营效率这五个维度，选取了能够衡量公司财务状况 10 个财务指标。接下来以 0 为临界点，对各指标打分，得到 10 个哑变量，最后将其加总。

表 1：统计量 $\cdot s_{j,t}$ 的构建
...
其中， $t=1,\dots,T$ 。这样可以得到参数估计值 $\hat{\alpha}_{j}=\big(\hat{\alpha}_{j,0},\hat{\alpha}_{j,1}\big)^{\prime},j=$ $1,\ldots,N.$ 。其中 $\alpha_{j,1}$ 反映了 $R_{t}{\vec{\times}}\Psi$ 由 $S_{j,t-1}$ 构建的基本面指数的敏感程度。第二阶段的回归是用每个月的 $S_{j,t}$ 对刚刚得到的因子 $\hat{\alpha}_{j,1}$ 进行横截面回归：

$$
S_{j,t}=\gamma_{t}+FUND_{t}^{PLS}\hat{\alpha}_{j,1}+\psi_{j,t}\tag{2}
$$

其中， $j=1,\dots,N.$ 。这样，我们就得到了基本面指数 $FUND_{t}^{PLS}$ 的估计值。公式（1）提取出 $S_{j,t-1}$ 与预测超额市场回报率 $R_{t}$ 相关的部分，并剔除了不相关的部分（Huang et al., 2015）
...
| 指数 | 定义 |
| --- | --- |
| $\overline{{FUND^{PIOT}}}$ | 所有股票的 $S_{j.t}{\sharp}\sharp^{\sharp}$ 等权加总 |
...
首先，作者检验了基本面指数 $FUND^{PLS}\rangle$ 对超额市场回报率 $R_{t}$ 的预测能力。作者通过构建了以下回归方程，发现 $FUND^{PLS}\Re$ 对超额市场回报率 $R_{t}$ 的解释能力高达 11%，显著优于其他的指标。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| DJIA | Slope | t-stat | Intercept | t-stat | $R^{2}$ | $R_{\mathrm{~}\mathrm{~U~}}^{2}$ | $R_{\mathrm{~D~}}^{2}$ | $R_{\mathrm{~\tiny~BH}}^{2}$ | $R_{\mathrm{~BL~}}^{2}$ | $R_{\mathrm{\mathrm{~HH~}}}^{2}$ | $R_{\mathrm{~HL~}}^{2}$ | $R_{\mathrm{~\ E~}}^{2}$ | $R_{\mathrm{~\tiny~R~}}^{2}$ |
...
| S&P500 | Slope | t-stat |  | Intercept | t-stat | R² | $R_{\mathrm{~U~}}^{2}$ | $R_{\mathrm{~D~}}^{2}$ | $R_{\mathrm{~BH}}^{2}$ | $R_{\mathrm{~\tiny~BL}}^{2}$ $R_{\mathrm{~HH}}^{2}$ | $R_{\mathrm{~\bf~m~}}^{2}$ | $R_{\mathrm{~E~}}^{2}$ | $R_{\mathrm{~\tiny~R~}}^{2}$ |
...
其中， $X=UP,DOWN$ $D_{X,t}$ 是一个指示函数（当t时期的市场属于X状态时为 1，否则为 0）。 $\varepsilon_{X,t}^{2}$ 是回归式（3）的残差平方， $\overline{{R_{M}}}.$ 是超额回报率 $\cdot R_{M,t}$ 的均值。估计结果见表 4 的第 7-8 列。数据显示， $FUND^{PLS}$ 在市场下行阶段表现出更有效的预测能力 $(R_{D}^{2}=10\%VSR_{U}^{2}=0.47\%)$ ）,并且这种状态的显著的差异是基准指数所不具备的。
...
预测结果的检验则是选择了统计量 $R_{OS}^{2}$ （Campbell & Thompson, 2008）、调整后的 MSFE（Clark & West, 2007）以及 CW test。预测结果见表 5。

表 5: 样本外检验

| DJIA | $R_{\mathrm{~\tiny~OS~}}^{2}$ | MSFE | CW test | $R_{\mathrm{~}}^{2}{\bf{u}}$ | MSFE | CW test | $R_{\mathrm{~D~}}^{2}$ | MSFE | CW test | $R_{\mathrm{~\tiny~EXP}}^{2}$ | MSFE | CW test | $R_{\mathrm{~REC}}^{2}$ | MSFE | CW test |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| DJIA $R_{t+1}$ | 0.545 | 4.72 |  | stat $R^{2}$ 2.48 10.7% | $R_{\mathrm{~}}^{2}\mathrm{{_{os}}}$ 5.89% | $R_{\mathrm{~L~U~}}^{2}$ 0.5% | $R_{\mathrm{~L~U~}}^{2}$ 10.5% | $R_{\mathrm{~\tiny~C~U~}}^{2}$ 0.6% | $R_{\mathrm{~}}^{2}{}_{\mathrm{cD}}$ 9.3% | $R_{\mathrm{~BH}}^{2}$ 12.1% | $R_{\mathrm{~BL~}}^{2}$ 0.6% | $R_{\mathrm{~HH}}^{2}$ 11.2% | $R_{\mathrm{~\tiny~HL~}}^{2}$ |  | $\scriptstyle R_{\mathrm{~E~}}^{2}$ | $\scriptstyle{R_{\mathrm{~R~}}^{2}}$ |
...
| S&P500 | Slope | t-stat | Intercept |  | t-stat | $R^{2}$ | $R_{\mathrm{~\bf~os~}}^{2}$ |  | $R_{\mathrm{~LU~}}^{2}$ | $R_{\mathrm{~\small~c~U~}}^{2}$ |  |  |  | 0.6% | 0.1% | $R_{\mathrm{~R~}}^{2}$ |
| Rt+ 1 | 0.685 | 5.76 | 0.0069 |  |  |  |  | $R_{\mathrm{~LU~}}^{2}$ |  | $R_{\mathrm{~}}^{2}{\mathrm{~c~p~}}$ | R BH | $R_{\mathrm{~\tiny~BL~}}^{2}$ | RHH | R2HL | R²E |  |
