...
$$
\begin{array}{rl}&{r_{\mathrm{i},\tau}-r_{f,\tau}=\alpha_{t}^{7F}+b_{i,t}\big(MKT_{\tau}-r_{f,\tau}\big)+s_{i,t}SMB_{\tau}+h_{i,t}HML_{\tau}+}\\&{p_{i,t}UMD_{\tau}+\sum_{j=1}^{3}\gamma_{t}^{j}IND_{\tau,t}^{j}+\nu_{\mathrm{i},\tau},\tau\in\{t-\frac{m}{12},\dots,t=\frac{1}{12}\}}\end{array}
$$
...
$$
{\boldsymbol{\Delta}}_{i,t}={\frac{1}{m}}\sum_{\tau=t-{\frac{m}{12}}}^{t-{\frac{1}{12}}}\left(s_{i,t}SMB_{\tau}+h_{i,t}HML_{\tau}+p_{i,t}UMD_{\tau}+\sum_{j=1}^{3}\gamma_{i,t}^{j}IND_{\tau,t}^{j}\right)
$$
...
采用条件双重排序法——在每个横截面 t，先根据因子调整收益 $\boldsymbol{\alpha}^{7F}$ 进行分位数分组，再在每个组内依据平均因子收益Δ进行分组，在组内计算平均规模增量Flow，进行显著性检验。实证结果如表 1 所示。结论为：控制了α收益后，越高的历史因子暴露收益带来越高的基金规模增量。

由于基金的规模增量反应了投资者对基金管理能力的评价，该结论表明，投资者把本该归因为估值、规模、动量、行业等因子带来的暴露收益，错归为基金经理的主动管理能力，从而导致了这一类基金的规模过高，产生错配。

表 1：历史因子暴露收益与规模流量

| $\mathrm{A_{s}}$ |  | $\mathrm{A}_{\mathrm{e}}$ Flow |  | $\Delta$ $\alpha^{\mathrm{CAPM}}$ |  | $\mathrm{A_{s}}$ | $\mathrm{A}_{\mathrm{e}}$ | Flow | $\Delta$ | $\alpha^{\mathrm{CAPM}}$ | $\mathrm{A_{s}}$ | $\mathrm{A}_{\mathrm{e}}$ | Flow | ∆ | $\alpha^{\mathrm{CAPM}}$ |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
$$
C=cD^{2}\sum_{j=1}^{N}\frac{\omega_{\mathrm{j}}^{2}}{M_{j}}
$$
...
$$
\tilde{C}=\frac{C}{A}=c^{\prime}AT^{2}\sum_{j=1}^{N}\frac{\omega_{j}^{2}}{M_{j}}
$$
...
|  | Top-Tercile ∆ |  |  |  | Others |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  | Ret | $\alpha^{\mathrm{CAPM}}$ | $\alpha^{\mathrm{FFC}}$ | $\alpha^{\mathrm{7F}}$ | Ret | $\alpha^{\mathrm{CAPM}}$ | $\alpha^{\mathrm{FFC}}$ | $\alpha^{\mathrm{7F}}$ |
...
| Ret(%) | $\alpha^{\mathrm{CAPM}}(\%)$ | $\alpha^{\mathrm{FF3}}(\%)$ | $\alpha^{7\mathrm{F}}(\%)$ |
| --- | --- | --- | --- |
| FIT low 11.08 | $2.74^{*}$ (1.67) | 1.21 (1.39) | $3.10^{***}$ (1.17) |
...
|  | $A_{s}$ | $A_{e}$ | Flow | ∆ | $\mathsf{\alpha}^{5F}$ |
| --- | --- | --- | --- | --- | --- |
...
随着因子暴露收益的增加，规模流量呈递增趋势， $\mathsf{\alpha}^{5F}$ 呈递减趋势。多空组合（H-L）的规模流量为 0.454 亿元，而α收益为负。表 7 的结果表明，基金的规模与主动管理能力错配的现象在 A股市场同样存在。基金规模的增量一部分归因为历史因子暴露的收益，并且高因子暴露收益伴随着更低的主动管理能力。

表 8：不同排序组合的α收益（控制绝对规模）
![](images/a27b154f1a80042c0f12d1e36e924b9c54ece9bd0786db9ce5153749ea55232c.webp)
数据来源：Wind
...
为了检验 FRR 因子在 A 股的预测能力，以季度换仓的方式进行滚动回测，测试区间为 2020Q1 年至 2022Q1。在每一个季度横截面，取其前 $\mathbf{\dot{m}=}$ 24个月的数据进行多因子归因，计算相应的历史因子暴露收益、α收益、平均规模和规模增长率。在因子构造方面，考虑 2 种构造方式：

$$
\begin{array}{l}{{\displaystyle Factor1=\mathrm{\Lambda}_{i,t}=\frac{1}{m}\sum_{\tau=t-\frac{m}{12}}^{t-\frac{1}{12}}\left(s_{i,t}SMB_{\tau}+h_{i,t}HML_{\tau}+p_{i,t}UMD_{\tau}\right)}}\\{{\mathrm{}}}\\{{\displaystyle Factor2=\frac{1}{2}\mathrm{\Lambda}_{i,t}+\frac{1}{2}A\dot{U}M\mathrm{},\quad\mathrm{\large~\left.\nabla\xi,\Omega\right.\left.\dot{H}\right.d\dot{U}M=\frac{AUM_{0}-AUM_{-m}}{AUM_{-m}}.}}}\end{array}
$$
