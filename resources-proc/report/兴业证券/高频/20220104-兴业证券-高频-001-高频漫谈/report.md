...
$$
\begin{array}{rl}{\frac{1}{|\mathbf{x}|}+\frac{1}{|\mathbf{y}|}-\mathbb{E}\int\hat{\mathcal{X}}\equiv\mathbb{J}[\mathbf{\Xi}]\mathbb{X}\frac{1/2}{\hat{\mathcal{\hat{A}}}\hat{\mathcal{D}}}\frac{\Xi^{2}}{\hat{\mathcal{F}}}\hat{\mathcal{X}}\equiv\frac{1}{T}\sum_{t=1}^{T}[\sum_{i=1}^{\mathrm{N}}\bigl(r_{M,i,t}\bigr)^{2}]}&{}\\{\big/\big|\hat{\mathcal{X}}\equiv\frac{1}{|\mathbf{X}|}+\mathbf{\Xi}]\big|\hat{\mathcal{X}}\equiv\frac{1}{\hat{\mathcal{P}}}\frac{\Xi^{2}}{\hat{\mathcal{H}}}\hat{\mathcal{H}}\equiv\frac{1}{T}\sum_{t=1}^{T}\bigl(r_{D,t}-{\overline{{r_{D,t}}}}\bigr)^{2}}&{}\end{array}
$$
...
遗传规划算法η最终会随着模型检验通过的因子数量 $F_{total}$ 上升而下降，真实样本外有效的因子 $F_{effective}.$ 在这个过程中可能增加得并不多。
...
$$
g=f(Reorder(dat{a}))=f(d{a}t{a})
$$

指标 $;g\colon$ 通过数据data构造而成，且对于时序重排序函数Reorder不敏感，如已实现方差： $E(Reorder([r_{1}^{2},r_{2}^{2},r_{3}^{2}]))=E([r_{2}^{2},r_{1}^{2},r_{3}^{2}])$ 。我们认为提取信息的函数具有不变的数学性质，输入不同的数据，能够提炼出对应部分的信息，如分钟收益率偏度、分钟收益率峰度、分钟成交占比标准差，分钟成交量占比信息熵等等，都是通过分布信息函数挖掘出数据所含的分布信息。构建高频指标本质就是从高频数据中提取对应的信息，在这个基础上，我们可以借鉴机器学习中的特征工程方法，学习分布信息函数的构造方法。
...
指标g对于时序重排序函数Reorder敏感，数据在时序上的变化会影响指标值，如收益率自相关性 $\rho(r_{t},r_{t-1})$ 依赖于收益率序列的排序，重新排列收益率序列后，指标值会发生变化。当然收益率自相关性在一定程度上也用到了收益率的分布信息，单独把时间信息列出来是因为许多时序方法能够提取出序列的时间信息，如DTW 能比较时间序列之间的相似性，LSTM 能提取时间序列的不变因素，时序异常值检验能发现时序上的异常值。这些函数本质上能提取数据的时间信息，我们希望对这些函数进行搜索以更有效率的方式生成因子。

## 3) 关联信息

$$
g=f{\big(}data_{x},Reorder(data_{y}{\big)}{\big)}\neq f{\big(}data_{x},data_{y}{\big)}
$$

指标g对于数据 $d\mathbf{a}ta_{x},d\mathbf{a}ta_{y}$ 的一一对应关系敏感，改变 $d\mathbf{a}ta_{x},d\mathbf{a}ta_{y}$ 的对应关系会改变指标g的取值。如量价相关性ρ(r,volume)，改变对应收益率水平下的成交量会极大地改变量价相关性的取值。本质上关联信息是利用了随机变量之间的联合分布关系，在某个随机变量取较大的值时，另一个随机变量也容易出现较大的值。许多因子都利用了关联信息，如大单对应收益率，每笔成交额，价差等。

## 4) 另类信息

$$
g=f(data,Information),Information\notin data
$$

指标 ${\bf\nabla}\cdot\mathbf{\nabla}g\cdot\mathbf{\nabla}$ 依赖于不属于数据data的其他信息，如研究人员事先规定的概念，尾盘、开盘、大单等等。如果参数是通过其他信息确定的，那么指标 $_g$ 含有另类信息。
...
这种因子表现随着等权天数而变化的现象并不少见，在许多情况下，因子都存在一个最优等权平均天数n∗。可以把因子在时序上的等权平均看作组合优化问题，对于因子 $G=[g_{1};g_{2};g_{3};\ldots;g_{n}]$ 而言，角标代表高频指标对应的天数，那么对于 $\cdot t=n+1$ 的截面收益率 $R_{t+1}$ （列向量）而言，n 天平均的因子收益率 $R_{G_{t+1}}$ 等于

$$
\begin{array}{c}{{R_{G_{n+1}}=mean(G^{T})R_{t+1}}}\\{{=(g_{1}+g_{2}+g_{3}+\cdots+g_{n})^{T}R_{t+1}/n}}\\{{=(g_{1}^{T}R_{t+1}+g_{2}^{T}R_{t+1}+g_{3}^{T}R_{t+1}+\cdots+g_{n}^{T}R_{t+1})/n}}\end{array}\tag{2}
$$
...
下面来说明这一点，若因子截面值 $\mathbf{\nabla}_{\pmb{\mathcal{g}}}$ 与收益率截面值R都服从标准正态分布，Haldane(1942)指出，它们的皮尔逊相关系数 $_{\textrm{ \tiny . }\rho}$ （即 IC），前两阶中心距可以被 ${}_{\cdot\rho.}$ 表示：

$$
\mu=\rho,\sigma^{2}=1+\rho^{2}\tag{3}
$$

所以因子收益率与 IC 正相关，组合夏普比率 $\scriptstyle sharpe={\frac{\mu}{\sigma}}={\frac{\rho}{\sqrt{1+\rho^{2}}}}$ 也与 IC 正相关，那么组合优化问题的目标函数就转化为最大化组合 IC（若 IC 小于 0，那么因子乘以-1）。
...
随着等权平均天数n的增加，式(5)的分子与分母都在变大。由于单因子的IC衰减规律， $cov(g_{n},R_{t+1})$ 会不断减小，表现为分子增长的速度逐渐变慢；但因子截面标准差一般是比较稳定的，因此std $(g_{1}+g_{2}+\cdots+g_{n})$ 增长速度一般变化不大，表现为分母增长的速度几乎不变。若一开始分子增加得比分母快，就会出现组合 $\cdot IC_{n}$ 先变大，后来逐渐变小的情况。具体来说，以 $\Sigma_{n}$ 表示 n 天因子值组成的协方差矩阵，I表示单位向量
...
当式(6)在 n 比较小的时候成 $\dot{\underline{{\sigma}}}$ ，那么组合 $IC_{n}$ 会递增，之后随着$cov(g_{n},R_{t+1})$ 逐渐衰减组合 $IC_{n}$ 递减。

因此在很多情况下会存在一个最优等权平均天数 $n^{*}$ ，使得因子表现最好。这个最优等权天数 $n^{*}$ 使得多增加一天平均减少的分子与增加的分母对组合 $IC_{n}$ 的边际效应为 0，即式(6)两侧取等号时取得 $n^{*}$ 。实际显式计算 ${\boldsymbol{\cdot}}{\boldsymbol{n}}^{*}$ 比较困难，我们采用一个近似算法，取两个时间段 $\scriptstyle[0,\mathtt{n}]$ ，[n,2n]，分别计算这两个时间段对应的$IC_{1},IC_{2}$ ，那么根据式(6)最优 $n^{*}$ 应满足如下公式

$$
\frac{IC_{2}}{IC_{1}}=\sqrt{2+2\rho(g_{n},g_{2n})}-1\tag{7}
$$

其中 $\rho(g_{n},g_{2n})$ 是时间段 $[0,\mathfrak{n}]$ ，[n,2n]的因子截面自相关性。但在实际操作过程中，如果 $n^{*}$ 所在的区间能够被大致的确定，那么更方便的做法其实是遍历这个区间，比如[10,30]，搜索得到最优 $\boldsymbol{\cdot}\boldsymbol{n}^{*}$ 。

接下来，我们考察权重不等的加权平均。对于加权平均，最大化 IC 等价于找到因子组合的最优权重 $w^{*}$ ，在每期因子值标准差相等的假设下，对于max ρ而言，最优 $.w^{*}$ 存在显示解
...
$$
\sigma_{t-j}=std\bigl(\bigl[\mu_{j},\mu_{j+1},\dots,\mu_{t}\bigr]\bigr)\tag{10}
$$
...
(11)式等于统计量 ${\mu}_{t}$ 的标准误，标准误衡量统计量与总体分布的参数值是否接近，标准误越小，样本对整体分布的代表性越好。事实上，动量因子的标准差就是波动率因子，波动率因子衡量了动量因子在时序上的差异，这种差异来自于动量因子在时序上分布的差异。波动率越大，股票含有的风险可能越大，因此需要更高的预期收益来补偿，所以波动率因子对股票截面收益率有预测能力。
...
$$
\begin{array}{c}{{max_{w}\displaystyle\frac\lambda{\sigma(\lambda)}}}\\{{s.t.w^{T}{\bf1}=0,w^{+}{\bf1}^{T}=1,{w^{-}}{\bf1}^{T}=-1}}\end{array}\tag{12}
$$

σ(λ)是多期值，我们用σ(w)替代，因为权重分配得越集中，那么因子收益率的风险就会越大，因子收益率的波动率也会变大；相反，若权重分配得约平均，$\begin{array}{r}{w_{i}=\frac{1}{N},}\end{array}$ ，N表示截面上股票个数，那么因子收益率对冲的风险会越多，因子收益率的波动率也会降低。若因子值 F与因子当期收益率是正相关（注意这是下面结论成立的假设），那么 $\lambda=w^{T}\pmb{F}$ ，最优 ${\bf\nabla}_{W}^{*}\propto F.$ 。按照因子值中位数正负进行划分的F+，F−，那么w应满足
...
例如存在两个投资者，他们在每一期随机地购买股票，权重为 $F_{A},F_{B}$ ，假设$F_{A},F_{B}$ 独立同分布，且与股票收益率R相互独立，那么因子截面相关性$\rho_{cross-section}=0$ ，每一期投资者的收益期望也为 0，即 $E[F_{A}R]=0,E[F_{B}R]=0$ 但他们的时序相关性等于1

$$
\rho_{time-series}=\frac{E[F_{A}RF_{B}R]}{\sigma(F_{A}R)\sigma(F_{B}R)}=\frac{E[(FR)^{2}]}{\sigma(FR)\sigma(FR)}=1\tag{15}
$$
...
我们考察两种基于因子截面值构造出的相关性指标。第一种是因子截面相关性。因子截面相关性衡量因子间的相似程度，因子 $F_{i},F_{j}$ 第t天的截面相关性被定义为

$$
\rho_{cross-section}^{(\mathrm{t)}}=\frac{cov\big(F_{i},F_{j}\big)}{\sigma_{F_{i}}\sigma_{F_{j}}}\tag{16}
$$

d天平均相关性为

$$
\rho_{cross-section}=\sum_{t=1}^{d}{\rho^{(t)}}_{cross-section}/d\tag{17}
$$

除了因子截面相关性之外，因子交易相关性也是基于因子截面值构造的。因子交易相关性被定义为因子截面换手相似性，因子截面换手 $Tor=F_{i}^{(t)}-F_{i}^{(t-1)}$ 那么

$$
\overline{{\rho^{(t)}{}_{trading}}}=\frac{cov\big(Tor_{i},Tor_{j}\big)}{\sigma_{Tor_{i}}\sigma_{Tor_{j}}}\tag{18}
$$
...
对冲后得到的截面因子值为 $F_{H}$ ，因为因子值最终要处理成因子权重，不失一般性的，我们假设因子截面加和等于 1，即∑ $\begin{array}{r}{F=\sum MOM=\sum F_{H}=1}\end{array}$ 。令 $\beta=$ $\frac{\left[F,MOM\right]}{\left[MOM,MOM\right]}$ ，上式两边截面上求和，因为 $F_{H}$ 需要乘以 $\frac{1}{1-\beta}$ 才能满足∑ $F_{H}=1$ ，所以我们调整式(20)为
...
$$
\mathfrak{iL}R_{F_{t}^{H}}=F_{H}R_{t},R_{F^{H}}=[R_{F_{1}^{H}},R_{F_{2}^{H}},\dots,R_{F_{t}^{H}}],\mathfrak{H}\measuredangle
$$
...
$\begin{array}{r}{\pmb{{B}}=diag\left(\frac{\beta_{1}}{1-\beta_{1}},\frac{\beta_{2}}{1-\beta_{2}},\dots,\frac{\beta_{t}}{1-\beta_{t}}\right)}\end{array}$ ，可以看到截面对冲其实没有完全消除动量因子 MOM对因子 F收益率的影响。对冲后得到的因子 F收益率实际上是在原因子F 收益率附近移动，具体的取值取决于因子 F 收益率与动量因子收益率的差值。这样就可能造成一种现象，即动量因子收益较好时，对冲之后因子 F收益率反而降低，本来是想消除动量因子对 F的影响，但实际上动量因子的影响还存在。

若B不随着时间变化而变化，我们可以更清楚的推导出时序相关性 $5$ 截面相关性的关系，令B = b

$$
\begin{array}{c}{{\displaystyle{\rho}(R_{F^{H}},R_{MOM})=\frac{(1+b)cov(R_{F},R_{MOM})-cov(R_{MOM},R_{MOM})}{\sigma\left(R_{F}+b(R_{F}-R_{MOM})\right)\sigma\left(R_{MOM}\right)}}}\\{{\displaystyle{\rho}(R_{F},R_{MOM})=\frac{cov(R_{F},R_{MOM})}{\sigma\left(R_{F}\right)\sigma\left(R_{MOM}\right)}}}\end{array}\tag{23}
$$

整理可以得到

$$
\rho^{H}=\left(1+\mathfrak{b}\right)\frac{\sigma(\mathbf{R}_{\mathrm{F}})}{\sigma({R_{F}}^{H})}\rho-\frac{\sigma(\mathbf{R}_{\mathrm{M}0\mathbf{M}})}{\sigma({R_{F}}^{H})}\tag{24}
$$

$\rho^{H}$ 是对冲后得到的时序相关性， $\rho$ 是对冲前的时序相关性，若 $\sigma({\bf R_{F}})=$ $\sigma(R_{F^{H}})=\sigma(\operatorname{R}_{\operatorname{M}0\operatorname{M}})$ ，对于同类的因子，这种假设可能成立，那么

$$
\overline{{\rho^{H}\approx\frac{1}{1-\beta}\rho-1}}\tag{25}
$$
...
总体来说， $\frac{\dot{\pi}}{\vert\nabla\vert}$ 的截面相关性代表了高的时序相关性，截面对冲会降低因子时序相关性。但是在高截面相关性的情况下，截面对冲会对时序相关性的影响降低，。

因此，在进行截面对冲时不能任意选择对冲因子，不然因子仍然会在时序上暴露风险。基于目前的线性对冲方法，我们认为：（1）对冲的因子应该是风险因子，此时 ${R}_{MOM}$ 的取值相对较小，所以对原因子收益率的扭曲越小。（2）选取因子进行对冲操作时，应考虑截面相关性大小，选取截面相关性适中的两个因子进行对冲，这样可以使得因子时序相关性更接近 0。
...
由于市场上许多投资者对风险、风险因子、因子这些概念认知混淆，在识别风险之前，我们先对它们作区分。以λ表示因子时序收益率，λ由提供收益的 $\lambda_{p}$ 与提供波动率的 $\lambda_{r}$ 组成， $E\big[\lambda_{p}\big]>0,E\lbrack\lambda_{r}\rbrack=0,D\big[\lambda_{p}\big]<D[\lambda_{r}]$

$$
\lambda=\lambda_{p}+\lambda_{r}\tag{26}
$$

只看因子收益率 $E[\lambda]=E\big[\lambda_{p}\big]$ ，是发现不了因子内的 $\lambda_{r}$ ，但是考察因子波动率 $D[\lambda]=D\big[\lambda_{p}\big]+D[\lambda_{r}]+2cov\big(\lambda_{p},\lambda_{r}\big)$ 可以发现 $\mathcal{A}_{r}$ 为因子提供了额外的波动率，导致因子λ的风险收益比降低，如果能找到一种方法从因子λ中去掉 $\cdot\lambda_{r},$ ，那么因子λ表现会变好。在这个分析中，风险指D[λ]，若在多因子组合中分配w给因子λ，$D[\lambda]\propto w^{2}$ ,E[λ] ∝ w，那么承担越大的风险，能获得越高的收益。风险因子指 $\lambda_{r}$ ，即不提供收益率，而提供因子时序波动率的因子，去掉这一部分的风险能提高投资组合的风险收益比。

## 4.2、高频因子风险识别方法

$\frac{\dot{\pi}}{\vert\nabla\pmb{\vert\tau}\vert}$ 频因子风险识别的难点在于如何定义风险因子，有两种思路可以供大家参考。

## 1) 自上而下的风险识别

这种方法认为风险类别在高频、低频中是相同的，区别只是在于风险构造的数据源不同，如果用 $\frac{\dot{\pi}}{\vert\nabla\pmb{\vert\tau}\vert}$ 频数据重构 Barra 风险因子，那么可能识别出 $\frac{\dot{\pi}}{\vert\nabla\pmb{\vert\tau}\vert}$ 频因子内的共性风险。
...
可以证明含有风险因子 $\cdot\lambda_{r}$ 时，计算因子 $\lambda_{1},\lambda_{2}+\lambda_{r}$ 的时序相关性约等于因子$\lambda_{1},\lambda_{2}$ 相关性与因子 $\lambda_{1},\lambda_{r}$ 的加权和。其中权重 $\omega.$ 与因子收益率的标准差正相关，一般 $\smash{\mathcal{F}_{0}}$ 言，风险因子标准差大于因子标准差，所以风险因子对组合时序相关性影响较大。
