...
其中， 为 内的 情数据，如分钟级收盘价、成交量等。指标g对于时序重排序函数Reorder敏感，数据data在时序上的变化会影响指标值，如收益率 相关性 $\rho(r_{t},r_{t-1})$ 依赖于收益率序列的排序，重新排列收益率序列后，指标值会发 变化。在此基础之上，我们衍 出基于时序信息相关因 。在本 中，我们将时序信息因 分为两类构建 式：序列 相关性与条件分布。
...
具体来为了更好体现 频因 对于短期 情信息把握的优势，我们采取 频调仓的 式测量因 收益率。说， $F_{Dj}$ 是第i天 频调仓因 ， $F_{j}$ 是第j天的 频指标， $n_{\mathrm{~S~}}$ 取15 ：

$$
F\_Mean_{Di}=\frac{1}{n_{s}}\sum_{j=1}^{n_{s}}F_{j}.\tag{2}
$$

除此之外，我们也可以在时序上取标准差的 式构建因 ，以衡量个股在该指标上的离散程度。

$$
F\_STD_{Di}=\frac{1}{n_{s}}\sum_{j=1}^{n_{s}}\left(F_{j}-\frac{1}{n_{s}}\sum_{j=1}^{n_{s}}F_{j}\right)^{2}\tag{3}
$$
...
$$
\mathbf{rtn}\mid Condition\sim N(\mu,\sigma^{2})\tag{4}
$$
...
其中， $P_{t+\eta}$ 为未来时刻的股票价格， $P_{t}$ 为当前时刻的股票价格。我们设置n=1，即价格时差为1分钟。由于需要考虑到分钟级股价存在 相关性，我们进 步假设其 $InP_{\;t+T}$ 与 $InP_{t}$ 之间的相关系数为 $1\rho_{\mathrm{c}}$

在此基础之上，我们假设 $InP_{\textit{ t + 1 }},InP_{\textit{ t })}$ 服从 元正态分布，且两者具有 样的均值和标准差。由此，我们可得

$$
(\ln P_{t+1},\ln P_{t})\sim\mathrm{N}(\mu,\mu,\sigma^{2},\sigma^{2},\rho)\tag{6}
$$

我们构建了前后对数价格的二元正态分布 $\mathbf{i}X=\begin{pmatrix}\ln P_{t+1}\\\ln P_t\end{pmatrix}$ ，其均值 $\mathbf{\mu}=\left(\begin{matrix}{\mathbf{\mu}}\\{\mathbf{\mu}}\end{matrix}\right)$ $\mathbf{\Sigma}=$ $\sigma^{2}\left[\begin{matrix}{1}&{\mathsf{p}}\\{\mathsf{p}}&{1}\end{matrix}\right]$

进一步，我们计算对数收益率 $R_{t}=\ln P_{t+1}-\ln P_{t}$ ，由此得到

$$
\boldsymbol{Y}=\binom{R_t}{\ln P_t}=\binom{\ln P_{t+1}-\ln P_t}{\ln P_t}=\begin{bmatrix}\mathbf{1}&-\mathbf{1}\\\mathbf{0}&\mathbf{1}\end{bmatrix}\boldsymbol{X}=\boldsymbol{C}\boldsymbol{X}\tag{7}
$$

其中 $\pmb{C}=\left[\begin{matrix}{1}&{-1}\\{0}&{1}\end{matrix}\right]$ 。由此，我们得到Y的分布为 $N{\sim}(C\pmb{\mu},C\pmb{\Sigma}\mathbf{C}^{\prime})$ 。由此，我们在考虑了对数价格的前后自相关性后，得到了收益率与对数股票价格的二元正态分布。最后，在股票价格ln $P_{t}$ 已知的情况下，我们最终得到在考虑了股票价格自相关性下的 VaR。其中， $\mathbf{p}.$ 为对数价格的前后分钟相关性， $\mu,$ 为日内分钟级对数价格的均值， $\sigma_{\cdot}$ 为日内分钟级对数价格的标准差，同时我们假设 $P_{t}$ 为当日收盘价， $Z_{\alpha}=$ 1.96。
...
| $\mu-\ln P_{t}>0$ |  | $\mu-\ln P_{t}<0$ |
| --- | --- | --- |
| $\begin{array}{r}{\rho>0,}\end{array}$ $\|\mathsf{p}\|=a$ | 风险偏大 | 风险偏小 |
| $\begin{array}{r}{\rho<0,}\end{array}$ $\|\mathsf{p}\|=a$ | 风险小 | 风险大 |
