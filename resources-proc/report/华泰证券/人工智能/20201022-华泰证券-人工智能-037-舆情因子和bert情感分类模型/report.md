...
$$
S_{i,t}=正面新闻数量-负面新闻数量
$$

2. 在每个交易日 T，针对每只个股 i，取过去 T-30 个自然日的情感得分，按照时间先后对个股情感得分求线性衰减加权和，得到新闻舆情因子 $F_{i,T}$

$$
F_{i,T}=\sum_{t=T-29}^{T}w_{t}S_{i,t},\quad w_{t}=\frac{30-(T-t)}{30}
$$
...
a) 中位数去极值：设第 T 期某因子在所有个股上的暴露度向量为 $D_{i}$ ， $D_{M}$ 为该向量中位数， $D_{M1}$ 为向量 $|D_{i}-D_{M}|的$ 中位数，则将向量 $D_{i}$ 中所有大于 $D_{M}+5D_{M1}$ 的数重设为 $D_{M}+5D_{M1}$ ，将向量 $D_{i}$ 中所有小于 ${{\cdot}{D}_{M}}-5{{D}_{M1}}$ 的数重设为 $D_{M}-5D_{M1}$
...
4. 多空组合收益计算方法：用 Top组每天的收益减去 Bottom 组每天的收益，得到每日多空收益序列 $[r_{1},r_{2},\cdots,r_{n}$ ，则多空组合在第n天的净值等于 $(1+r_{1})(1+r_{2})\cdots(1+r_{n})$
...
$$
Attention\big(Q,\ K,\ V\big)=softmax\big(\frac{QK^T}{\sqrt{d_k}}\big)V
$$

在上面的公式中，通过计算点积 $QK^{T}$ 得到 Query 和 Key 的相似度，再将相似度输入softmax 函数规范化到(0，1)范围内，得到Key对应Value 的权重，并乘上V得到最终的注意力向量。

我们以句子“证券大涨带动做多情绪升温”为例，说明其前三个字符的自注意力计算流程。前三个字符经过预处理编码后可得向量 X ，X ，X ，对其做线性变换后得到 K，Q，V 向量。图表 19 展示了使用 $\mathbf{Q}_{2}$ 作为查询向量的子注意力计算流程。针对句子中的每个字符的向量，都重复图表 19中的过程，就可得到整个句子的注意力输出序列。
...
1.将输入 X与 8个不同的权重矩阵 $.W_{i}$ 相乘，构成输入向量 $W_{i}X$ ，形成 $Q_{i},\quad K_{i},\quad V_{i},\quad\mathrm{i}=1$ . . . ，8，

2.计算注意力权重：

$$
Z_{i}=softmax(Q_{i}K_{i}^{T}/\sqrt{d_{k}})V_{i}\quad,
$$

3.将 8个注意力头的结果合并。

$$
Z^{C}=concact(Z_{1},\quad\dots,\quad Z_{8}),\quad i=1,\quad\dots,\quad8
$$
