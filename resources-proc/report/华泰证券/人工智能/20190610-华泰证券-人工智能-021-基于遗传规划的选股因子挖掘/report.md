...
$$
y=(+\big(-(-x{{X}_{0}}{{X}_{0}})(\times3{{X}_{1}})\big)0.5)
$$
...
a) 中位数去极值：设 $F_{M}$ 为该向量中位数， $F_{M1}$ 为向量 $\lvert F_{t}-F_{M}\rvert$ 的中位数，则将向量 $F_{t}$ 中所有大于 $F_{M}+5F_{M1}$ 的数重设为 $F_{M}+5F_{M1}$ ，将向量 $F_{t}$ 中所有小于 $F_{M}-$ $5F_{M1}$ 的数重设为 $F_{M}-5F_{M1}$ ；
...
$$
1)z\dot{2}\acute{\alpha}-2\dot{2}-2\dot{2}-\dot{2}\dot{2}=0
$$
...
其中 $SE(a^{T})$ 代表回归系数 $\boldsymbol{a}^{T}$ 的标准差的无偏估计量。一般 t值绝对值大于 2 我们就认为本期回归系数 $\boldsymbol{a}^{T}$ 是显著异于零的(也就是说，本期因子 X对下期收益率具有显著的解释作用)。注意，我们在回归模型中加入了市值、行业因子，能在一定程度上规避市值、行业因素对财务质量因子的影响。
