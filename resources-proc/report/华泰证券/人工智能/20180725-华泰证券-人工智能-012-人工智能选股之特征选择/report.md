...
$$
\mathrm{F=}\frac{\overline{{SSA}}}{\overline{{SSE}}}\mathrm{=}\frac{\frac{\overline{{SSA}}}{k-1}}{\frac{\overline{{SSE}}}{n-k}}
$$
...
使用 F 值对分类模型进行特征选择时，我们假设不同特征对分类结果贡献程度的差异，主要源于各个特征在不同标签下的组间离散程度与组内离散程度之比存在差异。对每个特征，计算F值并得到对应的p值。F值越大，该特征的组间离散程度越大而组内离散程度越小，特征与标签的关联度越高。如下表所示，对于模拟的原始特征集合，我们选择关联度排名前三的特征，最终得到新的特征子集 $X'=[X_1,X_3,X_4]$
...
$$
\mathrm{I}(\mathrm{X};\mathrm{Y})=\sum_{y\in Y}\sum_{x\in X}p(x,y)\log(\frac{p(x,y)}{p(x)p(y)})
$$
...
$$
\mathrm{I}(\mathrm{X};\mathrm{Y})=\int_{Y}\int_{X}p(x,y)\log(\frac{p(x,y)}{p(x)p(y)})dxdy.
$$
...
| --- | --- | --- |
| 真实情 $况=H_{0}$ 为真 | 正确判断（1-α） | 第一类错误，假阳性（α） |
...
|  | 预测结果=不拒绝 $H_{0}$ | 预测结果=拒绝 $\mathbf{.}\mathbf{\boldsymbol{H}_{0}}$ | 总计 |
| --- | --- | --- | --- |
| 真实情况=H0为真 | U | V | m₀ |
| 真实情况=H0为假 | T | S | m1 |
| 总计 | W | R | m |
...
$$
\mathrm{FWE}=\mathrm{P}(\mathrm{V}\geq1)
$$
