...
SinGAN 由多个 GAN 以金字塔形式组成，各层级 GAN 分别实现从全局特征到局部纹理的学习。模型架构如下图所示，左侧 G 、G 、…、G 为 N+1 层生成器，右侧 ${\sf D}_{0}、{\sf D}_{1}、$ DN为 N+1 层判别器。训练过程如左侧箭头所示，由下至上依次训练：首先训练 $(\mathsf{G}_{\mathsf{N}},\mathsf{D}_{\mathsf{N}})$ 学习粗糙的全局纹理（低频信息）；最后训练 $(\mathsf{G}_{0},\mathsf{D}_{0})$ 学习精细的局部纹理（高频信息）。
...
SinGAN 第 n 层生成器 $\mathbf{G_{n}}$ 的输入包含两部分：1）噪音 $\mathbf{z}_{\mathbf{n}},$ ，2）上一层生成器 $\mathbf{G}_{\mathbf{n}+1}$ 输出的假样本 $\widetilde{x}_{n+1}$ 经上采样得到的假样本 $\widetilde{x}_{\pmb{n+1}}\uparrow^{\pmb{r}}$ 。上采样的目的是使两项输入的尺寸匹配。如下图所示，两者相加后输入生成器 $\mathbf{G}_{\mathsf{n}},\mathbf{G}_{\mathsf{n}}$ 的输出再与上一层假样本 $\tilde{x}_{n+1}$ ↑r相加，得到当前层的假样本 ${\tilde{x}}_{n}$ 。此处应用了残差学习的思想，即每层生成器 $\mathbf{G}_{\mathsf{n}}$ 学习的是上一层生成器 $\mathbf{G}_{\mathsf{n}+1}$ 没有学会的细节。

图表2： SinGAN 第 n 个生成器 $\mathbf{G_{n}}$ 示意图
![](images/c1988e7dddfc1d84633e7da7850e100b90a4fee8791b22ac8b40643238e1e20d.webp)
资料来源：Shaham, Dekel, & Michaeli. (2019). SinGAN: learning a generative model from a single natural image，华泰研究

SinGAN 第 n 层判别器 $\mathsf{D_{n}}$ 的输入为真或假样本。假样本即同层生成器 $\mathbf{G}_{\mathsf{n}}$ 的输出 ${\tilde{x}}_{n}$ 。真样本由原始数据经固定采样率的下采样得到，每一层 GAN 的学习目标是生成不同分辨率下的原始数据。以图像生成为例：训练初始，低层级 GAN（如 GN和 DN）学习生成“高糊”原始图像；训练尾声，高层级 GAN（如 $\mathbf{G}_{0}$ 和 D0）学习生成“高清”原始图像。
...
$$
GAN总层数=\left[log_{\frac{1}{下采样率}}\frac{原始样本长度}{最低层样本长度}\right]+1=\left[log_{\frac{1}{0.8}}\frac{308}{15}\right]+1=15
$$

其中⌈a⌉代表对实数 a 向上取整。

理论上，最高层即第0层样本长度为原始样本长度308；第1层样本长度为 $308\times0.8=246.5$ 向下取整为 246；第 2 层样本长度为 $308\times0.8^{2}=197.1$ ，向下取整为 197；以此类推，第14 层样本长度为 $308\times0.8^{14}=13.5$ ，向下取整为 13，与我们设定的最低层样本长度 15 接近，但不完全相同。
...
$$
调整后下采样率=1\Bigg/\left|\frac{原始样本长度^{\frac{1}{GAN总层数}}}{最低层样本长度}\right|=1\Bigg/\left|(\frac{308}{15})^{\frac{1}{15}}\right|\approx0.8175
$$
...
1. 首先生成长度为 15 的噪音，左右两边边缘各填充 5个 0，得到长度为 25 的带 0噪音。随后输入至第15层生成器 $\mathbf{G}_{15},$ ，卷积后长度减少10，输出长度为15的第15层假样本。

2. 原序列长度为 308，下采样后得到长度为 15 的第 15 层真样本。将长度为 15 的真样本或假样本输入至第 15 层判别器 $\mathbf{D}_{15}$ ，得到长度为 5 的序列，最终求均值得到长度为 1的判别值。求均值操作属于马尔可夫判别器范畴，后文将展开介绍。
...
下面我们推导第14层GAN网络，该层样本长度为18，学习原始数据中次粗糙的全局特征。1. 首先将第 15 层生成器 $\mathbf{G}_{15}$ 输出的长度为 15 的假样本，先经上采样（Cubic 插值）得到长度为 18 的假样本，再进行边缘填充，得到长度为 28 的带 0假样本。

2. 生成长度为 18的噪音，左右两边边缘各填充 5个 0，得到长度为 28的带 0噪音。

3. 将假样本与噪音相加（权重可通过超参数调整，后文将展开介绍），输入至第 14 层生成器 $\mathbf{G}_{14},$ ，卷积后长度减少 10，输出长度为 18 的第 14 层假样本。由于生成器实际学习的是残差，生成器输出结果还会和前一层假样本相加，得到最终输出。公式表示如下：

$$
\tilde{x}_{n}=(\tilde{x}_{n+1})\uparrow^{r}+G_{n}(z_{n},(\tilde{x}_{n+1})\uparrow^{r})
$$

4. 原序列长度为 308，下采样后得到长度为 18 的第 14 层真样本。将长度为 18 的真样本或假样本输入至第 14 层判别器 $\mathbf{D}_{14}$ ，得到长度为 8 的序列，最终求均值得到长度为 1的判别值。

此后每层 GAN 网络均和第 14 层 GAN 网络类似。直至最高层即第 0 层，该层样本长度为308，等同于原序列长度，学习原始数据中最精细的局部纹理。和此前层的最大区别在于，训练第 0层判别器 $\mathsf{D}_{0}$ 时，原序列无需经下采样，而是直接作为真样本输入至判别器。
...
$$
\min_{G_{n}}\max_{D_{n}}\ L_{adv}(G_{n},D_{n})+\alpha L_{rec}(G_{n})
$$
...
$$
L_{adv}=D_{n}(\tilde{x}_{n})-D_{n}(x_{n})+\lambda(\left\|\nabla_{\tilde{x}_{n}}D_{n}(\tilde{x}_{n})\right\|_{2}-1)^{2}
$$
...
我们选择 $\{z_{N}^{rec},z_{N-1}^{rec},\dots z_{0}^{rec}\}=\{z^{*},0,\dots,0\}$ 作为这组特定噪 $声$ ，其中 $z^{*}$ 为训练前随机选取的噪声序列，并且在训练过程中保持不变。若 $\tilde{x}_{n}^{rec}$ 为第 n 层生成器 $\mathbf{G}_{\mathsf{n}}$ 生成样本，则重构损失可以表示为：

$$
L_{rec}=\left\{\begin{aligned}&\|G_n(0,(\tilde{x}_{n+1}^{rec})\uparrow^r)-x_n\|^2,\ 当n<N\\&\quad\|G_n(z^*)-x_N\|^2,\ 当n=N\end{aligned}\right.
$$
...
每层 GAN 迭代次数 T，每次迭代判别器训练次数 $\mathbf{\mathsf{T}}\mathbf{\mathsf{e}},$ 每次迭代生成器训练次数 $\mathsf{T}\mathbf{{\boldsymbol{\mathsf{{D}}}}},$ 重构损失权重 α
1根据 min_size、real_size 和 r_init，计算实际下采样率 r 和 GAN 总层数 N+1
2 for n • N to 0 do # 训练第n层 GAN
3随机初始化判别器 $.D_{n}$ 网络参数和生成器 $G_{n}$ 网络参数
4当 $n=N时$ ，随机选取噪音 $z^{*}$ （计算重构误差使用，训练过程中保持不变）
5原始序列x以采样率 $\cdot r^{n}!$ 进行下采样，得到真样 $\dot{x}_{n}$
6第n +1层生成器生成假样 $本\tilde{x}_{n+1},$ 再进行上采样，得到前一层假样 $\mathbf{\hat{x}}(\tilde{x}_{n+1})$ ↑r
7 for t • 1 to T do # 迭代训练
8随机选取噪音 $z_{n}$
9 $若n=N$ ，则生成假样本 $\cdot\tilde{x}_{n}=G_{n}(z_{n});$ $n<N$ ，则生成假样本 $\cdot\tilde{x}_{n}=(\tilde{x}_{n+1})\uparrow^{r}+G_{n}(z_{n},(\tilde{x}_{n+1})\uparrow^{r})$
10 for $t_{D}$ • 1 to $T_{D}$ do
11根据真样本 $\cdot x_{n}$ 和假样 $氏\tilde{x}_{n},$ 计算对抗损失 $L_{adv}$ 和重构损失 $\_L_{rec}$
12使用优化器更新判别器 $D_{n}$ 参数，优化目标为max $L_{adv}(G_{n},D_{n})+\alpha L_{rec}(G_{n})$
Dn
13 for $t_{G}$ • 1 to $T_{G}$ do
14 $若n=N,$ ，则生成假样本 $\therefore\tilde{x}_{n}=G_{n}(z_{n});$ ；若 $n{<}N,$ ，则生成假样 $\tilde{x}_{n}=(\tilde{x}_{n+1})\uparrow^{r}+G_{n}(z_{n},(\tilde{x}_{n+1})\uparrow^{r})$
15根据真样 $本x_{n}$ 和假样本 $\widetilde{x}_{n},$ 计算对抗损失 $.L_{adv}$ 和重构损失 $\llcorner L_{rec}$
16使用优化器更新生成器 $\cdot G_{n}乡$ 数，优化目标为minmax $L_{adv}(G_{n},D_{n})+\alpha L_{rec}(G_{n})$
Gn Dn
输出：N+1 层生成器 $\mathbb{G}_{0},\mathbb{G}_{1},...,\mathbb{G}_{N}$
```
资料来源：华泰研究

生成阶段，基于各层随机噪音 $\left\{z_{N},z_{N-1},\ldots z_{0}\right\}$ ，依次输入至各层生成器。对于第 n层生成器：1. 若n＝N，则生成该层假样本 $\tilde{x}_{n}=G_{n}(z_{n})$ ；

2. 若 $n{\leq}N$ ，则生成该层假样本 $\tilde{x}_{n}=(\tilde{x}_{n+1})\uparrow^{r}+G_{n}(z_{n},(\tilde{x}_{n+1})\uparrow^{r})$

按上述方式迭代，最终得到第 0 层生成器的输出 ${\tilde{x}}_{0}$ ，即与原始序列长度相同的假样本。
...
3. 如果原始序列长度 T 较小，为保证合理切片长度，那么就需要牺牲样本量。当样本量较小时，GAN 训练质量将受到损伤，如出现模式崩溃现象。这点在部分上市时间较短的资 $产$ （如科创 50 指数）体现尤为明显。
...
我们对“噪音项初始权重”超参数作展开介绍。前文我们提到，第 n 层（n＜N）生成器 Gn的输入有两项：1）第 n 层噪音 z ，2）第 n+1 层假样本上采样后得到长度匹配的假样本$(\tilde{x}_{n+1})$ ↑r。这两项相加时，会赋予噪音项一定权重。各层噪音项权重有差异，第 n 层噪音项权重为 1）初始权重和 2）第 n层重构损失平方根这两项的乘积。
